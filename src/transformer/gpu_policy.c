...
int bn_transformer_gpu_all2_q4q6_moe_model(const BnConfig *c,
    ...
    for (int l = 0; l < c->n_layers; l++) {
        ...
        if (!lw->moe.router_weight)
            continue;
        if (bn_transformer_gpu_all2_q4q6_moe_layer(c, lw, c->dim))
            return 1;
    }
    return 0;
}
...
int bn_transformer_gpu_all2_q4q6_moe_cpu_attn_safe_default(
    const BnConfig *c,
    const BnWeights *w) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_cpu_attn_safe_default(c, w);
}
...
int bn_transformer_gpu_all2_q4q6_moe_cpu_attn_fallback_enabled(
    ...
    const BnConfig *c,
    const BnWeights *w) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_cpu_attn_fallback_enabled(
        gpu, c, w);
}
...
int bn_transformer_gpu_cuda_small_dense_q8_cpu_attn_safe_default(
    const BnConfig *c,
    const BnWeights *w) {
    return bn_transformer_gpu_small_dense_q8_cpu_attn_safe_default(c, w);
}
...
int bn_transformer_gpu_cuda_small_dense_q8_cpu_attn_fallback_enabled(
    ...
    const BnConfig *c,
    const BnWeights *w) {
    return bn_transformer_gpu_small_dense_q8_cpu_attn_fallback_enabled(
        gpu, c, w);
}
...
int bn_transformer_gpu_cuda_small_dense_exact_q4_q8_default(
    ...
    const BnConfig *c,
    int q4_q8_from_layer) {
    return bn_transformer_gpu_small_dense_exact_q4_q8_default(
...
int bn_transformer_gpu_cuda_small_dense_exact_q4_q8_to_layer(
    ...
    int exact_q4_q8_default,
    int q4_q8_to_layer) {
    return bn_transformer_gpu_small_dense_exact_q4_q8_to_layer(
...
int bn_transformer_gpu_cuda_small_dense_exact_q4_q8_ffn_down_enabled(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_small_dense_exact_q4_q8_ffn_down_enabled(
        gpu, c);
}
...
int bn_transformer_gpu_cuda_small_dense_prefill_decode_fallback_requested(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_small_dense_prefill_decode_fallback_requested(
        gpu, c);
}
...
int bn_transformer_gpu_cuda_large_hybrid_prefill_decode_fallback_default(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_large_hybrid_prefill_decode_fallback_default(
        gpu, c);
}
...
int bn_transformer_gpu_large_hybrid_cpu_attn_fallback_enabled(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_cuda_large_hybrid_cpu_attn_fallback_enabled(
        gpu, c);
}
...
int bn_transformer_gpu_cuda_large_hybrid_prefill_chain_disabled_default(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_large_hybrid_prefill_chain_disabled_default(
        gpu, c);
}
...
int bn_transformer_gpu_cuda_prefill_direct_kv_allowed(
    ...
    if ((bn_gpu_policy_cpu_decode_fallback_requested() ||
         bn_transformer_gpu_all2_q4q6_moe_cpu_attn_fallback_enabled(
             gpu, c, w) ||
         bn_transformer_gpu_small_dense_q8_cpu_attn_fallback_enabled(
             gpu, c, w) ||
...
int bn_transformer_gpu_cuda_small_dense_q8_logits_refine_enabled(
    ...
    const BnConfig *c,
    int tensor_type) {
    return bn_transformer_gpu_small_backend_q8_logits_refine_enabled(
        gpu, c, tensor_type);
}
...
int bn_transformer_gpu_all2_q4q6_moe_cpu_moe_safe_default(
    const BnConfig *c,
    const BnWeights *w) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_cpu_moe_safe_default(c, w);
}
...
int bn_transformer_gpu_moe_exact_attention_enabled(
    const BnGPUBackend *gpu,
    const BnConfig *c) {
    return bn_transformer_gpu_cuda_moe_exact_attention_enabled(gpu, c);
}
...
int bn_transformer_gpu_cuda_large_hybrid_argmax_blocked(
    ...
    const BnWeights *w,
    int want_argmax) {
    return bn_transformer_gpu_large_hybrid_argmax_blocked(
        gpu, c, w, want_argmax);
}
...
int bn_transformer_gpu_moe_route_topk_enabled(
    ...
    int all2_q4q6_moe,
    int all2_q4q6_moe_gpu_route_layer_selected) {
    return bn_transformer_gpu_cuda_moe_route_topk_enabled(
...
int bn_transformer_gpu_moe_cpu_route_resident_ffn_enabled(
    ...
    int gpu_route_topk,
    int moe_routed_q8) {
    return bn_transformer_gpu_cuda_moe_cpu_route_resident_ffn_enabled(
...
int bn_transformer_gpu_moe_routed_ffn_enabled(
    ...
    int moe_hidden,
    int dim) {
    return bn_transformer_gpu_cuda_moe_routed_ffn_enabled(
...
bn_transformer_gpu_moe_decode_route_policy(
    ...
    policy.all2_q4q6_moe =
        bn_transformer_gpu_all2_q4q6_moe_layer_enabled(
            gpu, c, lw, dim);
    policy.route_layer_selected =
        bn_transformer_gpu_all2_q4q6_moe_route_layer_selected(
            layer, from_layer, to_layer);
    policy.exact_gpu_route =
        bn_transformer_gpu_all2_q4q6_moe_exact_gpu_route_enabled(
            policy.all2_q4q6_moe, policy.route_layer_selected);
    policy.router = bn_transformer_gpu_all2_q4q6_moe_router(
...
int bn_transformer_gpu_all2_moe_direct_route_enabled(
    ...
    void *router_diff,
    void *moe_gate_all) {
    return bn_transformer_gpu_cuda_all2_moe_direct_route_enabled(
        c, router_diff, moe_gate_all);
}
...
int bn_transformer_gpu_all2_q4q6_moe_route_layer_selected(
    ...
    int route_from_layer,
    int route_to_layer) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_route_layer_selected(
...
int bn_transformer_gpu_all2_q4q6_moe_exact_gpu_route_enabled(
    int all2_q4q6_moe,
    int route_layer_selected) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_exact_gpu_route_enabled(
...
void *bn_transformer_gpu_all2_q4q6_moe_router(
    ...
    int route_layer_selected,
    int exact_gpu_route) {
    return bn_transformer_gpu_cuda_all2_q4q6_moe_router(
...
BnTransformerGPUMoEDebugPolicy bn_transformer_gpu_moe_decode_debug_policy(
    ...
    return bn_transformer_gpu_moe_debug_policy(
        bn_transformer_gpu_all2_q4q6_moe_cpu_moe_safe_default(c, w),
