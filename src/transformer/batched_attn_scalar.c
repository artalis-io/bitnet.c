...
static inline float batched_dot(const float *a, const float *b, int n) {
    ...
    int i = 0;
    for (; i + 15 < n; i += 16) {
        lane[0] = fmaf(a[i + 0],  b[i + 0],  lane[0]);
        lane[1] = fmaf(a[i + 1],  b[i + 1],  lane[1]);
        lane[2] = fmaf(a[i + 2],  b[i + 2],  lane[2]);
        lane[3] = fmaf(a[i + 3],  b[i + 3],  lane[3]);
        lane[0] = fmaf(a[i + 4],  b[i + 4],  lane[0]);
        lane[1] = fmaf(a[i + 5],  b[i + 5],  lane[1]);
        lane[2] = fmaf(a[i + 6],  b[i + 6],  lane[2]);
        lane[3] = fmaf(a[i + 7],  b[i + 7],  lane[3]);
        lane[0] = fmaf(a[i + 8],  b[i + 8],  lane[0]);
        lane[1] = fmaf(a[i + 9],  b[i + 9],  lane[1]);
        lane[2] = fmaf(a[i + 10], b[i + 10], lane[2]);
        lane[3] = fmaf(a[i + 11], b[i + 11], lane[3]);
        lane[0] = fmaf(a[i + 12], b[i + 12], lane[0]);
        lane[1] = fmaf(a[i + 13], b[i + 13], lane[1]);
        lane[2] = fmaf(a[i + 14], b[i + 14], lane[2]);
        lane[3] = fmaf(a[i + 15], b[i + 15], lane[3]);
    }
    for (; i + 3 < n; i += 4) {
        lane[0] = fmaf(a[i + 0], b[i + 0], lane[0]);
        lane[1] = fmaf(a[i + 1], b[i + 1], lane[1]);
    ...
    float sum = (lane[0] + lane[1]) + (lane[2] + lane[3]);
    for (; i < n; i++) sum = fmaf(a[i], b[i], sum);
    return sum;
}
...
static inline float batched_dot_fp16(const float *a, const uint16_t *b, int n) {
    ...
    int i = 0;
    for (; i + 15 < n; i += 16) {
        lane[0] = fmaf(a[i + 0],  bn_fp16_to_fp32(b[i + 0]),  lane[0]);
        lane[1] = fmaf(a[i + 1],  bn_fp16_to_fp32(b[i + 1]),  lane[1]);
        lane[2] = fmaf(a[i + 2],  bn_fp16_to_fp32(b[i + 2]),  lane[2]);
        lane[3] = fmaf(a[i + 3],  bn_fp16_to_fp32(b[i + 3]),  lane[3]);
        lane[0] = fmaf(a[i + 4],  bn_fp16_to_fp32(b[i + 4]),  lane[0]);
        lane[1] = fmaf(a[i + 5],  bn_fp16_to_fp32(b[i + 5]),  lane[1]);
        lane[2] = fmaf(a[i + 6],  bn_fp16_to_fp32(b[i + 6]),  lane[2]);
        lane[3] = fmaf(a[i + 7],  bn_fp16_to_fp32(b[i + 7]),  lane[3]);
        lane[0] = fmaf(a[i + 8],  bn_fp16_to_fp32(b[i + 8]),  lane[0]);
        lane[1] = fmaf(a[i + 9],  bn_fp16_to_fp32(b[i + 9]),  lane[1]);
        lane[2] = fmaf(a[i + 10], bn_fp16_to_fp32(b[i + 10]), lane[2]);
        lane[3] = fmaf(a[i + 11], bn_fp16_to_fp32(b[i + 11]), lane[3]);
        lane[0] = fmaf(a[i + 12], bn_fp16_to_fp32(b[i + 12]), lane[0]);
        lane[1] = fmaf(a[i + 13], bn_fp16_to_fp32(b[i + 13]), lane[1]);
        lane[2] = fmaf(a[i + 14], bn_fp16_to_fp32(b[i + 14]), lane[2]);
        lane[3] = fmaf(a[i + 15], bn_fp16_to_fp32(b[i + 15]), lane[3]);
    }
    for (; i + 3 < n; i += 4) {
        lane[0] = fmaf(a[i + 0], bn_fp16_to_fp32(b[i + 0]), lane[0]);
        lane[1] = fmaf(a[i + 1], bn_fp16_to_fp32(b[i + 1]), lane[1]);
    ...
    float sum = (lane[0] + lane[1]) + (lane[2] + lane[3]);
    for (; i < n; i++) sum = fmaf(a[i], bn_fp16_to_fp32(b[i]), sum);
    return sum;
}

static inline void batched_acc_fp16(float *out, float w, const uint16_t *v, int n) {
    for (int i = 0; i < n; i++)
        out[i] = fmaf(w, bn_fp16_to_fp32(v[i]), out[i]);
}
        ...
        for (int t = 0; t < n_tokens; t++) {
            ...
            for (int i = 0; i < n_kv; i++) {
                ...
                if (kv_f16) {
                    ...
                    for (int d = 0; d < hs; d++)
                        out_buf[d] = fmaf(w, vi[d], out_buf[d]);
                }
            }
