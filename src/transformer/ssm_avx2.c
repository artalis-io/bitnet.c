...
void bn_transformer_ssm_gate_avx2_range(void *ctx, int start, int end) {
    ...
    for (int hv = start; hv < end; hv++) {
        ...
        for (int d = 0; d < hd; d += 8) {
            ...
            // SiLU(z) per-element (no SIMD shortcut for expf)
            float g[8], sv[8];
            _mm256_storeu_ps(g, _mm256_loadu_ps(zh + d));
            for (int i = 0; i < 8; i++)
                sv[i] = g[i] / (1.0f + expf(-g[i]));
            _mm256_storeu_ps(oh + d, _mm256_mul_ps(o, _mm256_loadu_ps(sv)));
        }
    }
}
