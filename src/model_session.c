...
size_t bn_model_session_arena_size(const BnConfig *c, const BnWeights *w) {
    ...
    if (checked_mul_size((size_t)c->n_heads, (size_t)c->seq_len, &att_size) != 0)
        return 0;
    int n_attn_layers = bn_model_session_policy_attention_layer_count(c);
    int n_ssm_layers = bn_model_session_policy_ssm_layer_count(c);
    ...
    if (bn_model_session_policy_uses_hybrid_layer_layout(c)) {
        size_t qkv_tmp = 0;
    ...
    int shared_expert_hidden =
        bn_model_session_policy_shared_expert_hidden_dim(c);
    ...
    if (bn_model_session_policy_uses_moe(c) &&
        route_policy.expert_hidden_dim > x_q_size)
        x_q_size = route_policy.expert_hidden_dim;
    int per_layer_dim =
        bn_model_session_policy_per_layer_embedding_dim(c);
    if (per_layer_dim > 0) {
...
int bn_model_alloc_session_buffers(const BnConfig *c, const BnWeights *w,
    ...
    bn_model_session_policy_init_rope_frequencies(c, s->rope_freq,
                                                  half_head);
