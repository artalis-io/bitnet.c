                                             ...
                                             int allow_prepared)
{
    ...
    if (allow_prepared && bn_gpu_policy_metal_native_quant_prepared_upload_enabled() &&
        (rows % 4) == 0) {
        int n_groups = rows / 4;
                                      ...
                                      uint32_t n_tokens)
{
    ...
    if (ctx->native_quant_barriers_enabled) {
        id<MTLBuffer> bufs[2] = { ctx->native_quant_buf, ctx->native_quant_scales_buf };
    ...
    uint32_t n_tokens)
{
    ...
    [enc dispatchThreadgroups:grid threadsPerThreadgroup:tpg];
    if (ctx->native_quant_barriers_enabled) {
        [enc memoryBarrierWithResources:&ctx->native_quant_buf count:1];
        id<MTLBuffer> bufs[2] = { ctx->native_quant_scales_buf, ctx->native_quant_block_sums_buf };
        [enc memoryBarrierWithResources:bufs count:2];
    }
}
    ...
    @autoreleasepool {
        ...
        if (use_prepared_small_dense_native_quant) {
            ...
            metal_encode_specialized_native_quant(enc, ctx, ctx->x_buf,
                                                  (uint32_t)cols, 1);
            [enc setComputePipelineState:ctx->specialized_native_matvec_pipeline];
            [enc setBuffer:wbuf->buf offset:wbuf->offset atIndex:0];
            [enc setBuffer:ctx->native_quant_buf offset:0 atIndex:1];
            [enc setBuffer:ctx->native_quant_scales_buf offset:0 atIndex:2];
            [enc setBuffer:ctx->native_quant_block_sums_buf offset:0 atIndex:3];
            ...
            case BN_GPU_SHADER_MATVEC: {
                ...
                if (op->p[6] &&
                    ...
                    metal_encode_specialized_native_quant(
                        ...
                        (uint32_t)op->cols, n_tokens);
                    [enc setComputePipelineState:ctx->specialized_native_matvec_pipeline];
                    current_pso = ctx->specialized_native_matvec_pipeline;
                    [enc setBuffer:wbuf->buf offset:wbuf->offset atIndex:0];
                    [enc setBuffer:ctx->native_quant_buf offset:0 atIndex:1];
                    [enc setBuffer:ctx->native_quant_scales_buf offset:0 atIndex:2];
                    [enc setBuffer:ctx->native_quant_block_sums_buf offset:0 atIndex:3];
        ...
        if (n_native_quant_shapes > 0) {
            ...
            for (int i = 0; i < n_native_quant_shapes; i++) {
                fprintf(stderr, "  %-16s: %3d dispatches cols=%u tokens=%u\n",
                        ...
                        native_quant_shapes[i].count, native_quant_shapes[i].cols,
...
BnGPUBackend *bn_gpu_metal_create(const char *shader_dir)
{
    ...
    @autoreleasepool {
        ...
        ctx->small_dense_native_quant_enabled = bn_gpu_policy_metal_small_dense_native_quant_enabled();
        ...
        if (bn_gpu_policy_metal_specialized_native_quant_enabled()) {
            ctx->specialized_native_quant_pipeline = compile_shader(ctx, dir,
                "q8k_quantize.metal", "q8k_quantize");
            ctx->specialized_native_matvec_pipeline = compile_shader(ctx, dir,
