...
static int cuda_use_q6k_moe_float_down(void) {
    return bn_gpu_policy_cuda_q6k_moe_float_down_enabled();
}

static int cuda_use_q6k_moe_pair_down(int f32_down_default,
                                      int pair_down_f32_layer,
                                      int all_active_two_disable_pair_down) {
    return bn_gpu_policy_cuda_q6k_moe_pair_down_enabled(
        f32_down_default, pair_down_f32_layer, all_active_two_disable_pair_down);
}

static int cuda_prefer_q6k_moe_f32_down(const BnCudaBuffer *down,
                                        int hidden_dim,
                                        int all_active_two_kquant,
                                        int all_active_two_f32_down) {
    return bn_gpu_policy_cuda_q6k_moe_prefer_f32_down(
        down && down->f32_data != NULL, hidden_dim, all_active_two_kquant,
        all_active_two_f32_down);
}

static int cuda_use_q6k_moe_down_f32_pair2(int n_experts, int k) {
    return bn_gpu_policy_cuda_q6k_moe_down_f32_pair2_enabled(n_experts, k);
}

static int cuda_use_q6k_moe_down_f32_pair2_4row(void) {
    return bn_gpu_policy_cuda_q6k_moe_down_f32_pair2_4row_enabled();
}

static int cuda_use_q6k_moe_down_q8k_all_active_two_accum(
    int all_active_two_kquant) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_all_active_two_accum_enabled(
        all_active_two_kquant);
}

static int cuda_use_q6k_moe_down_q8k_pair4_sum(int all_active_two_kquant) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_pair4_sum_enabled(
        all_active_two_kquant);
}

static int cuda_use_q6k_moe_down_q8k_k8_4row_sum(int all_active_two_kquant,
                                                 int k,
                                                 int hidden_dim) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_k8_4row_sum_enabled(
        all_active_two_kquant, k, hidden_dim);
}

static int cuda_use_q6k_moe_down_q8k_k8_8row_sum(int k8_4row_sum,
                                                 int hidden_dim) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_k8_8row_sum_enabled(
        k8_4row_sum, hidden_dim);
}

static int cuda_use_q6k_moe_down_q8k_all_active_two_fixed(
    int all_active_two_kquant) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_all_active_two_fixed_enabled(
        all_active_two_kquant);
}

static int cuda_use_q6k_moe_down_resid_rmsnorm_fuse(void) {
    return bn_gpu_policy_cuda_q6k_moe_down_resid_rmsnorm_fuse_enabled();
}

static int cuda_use_q6k_moe_down_q8k_k8_exact_2048_768(int dim,
                                                       int hidden_dim,
                                                       int k) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_k8_exact_2048_768_enabled(
        dim, hidden_dim, k);
}

static int cuda_use_q6k_moe_down_q8k_all_active_two_accum_4row(void) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_all_active_two_accum_4row_enabled();
}

static int cuda_use_q6k_moe_down_q8k_pair_4row(void) {
    return bn_gpu_policy_cuda_q6k_moe_down_q8k_pair_4row_enabled();
}

static int cuda_use_q6k_moe_down_f32_cache(const BnCudaBuffer *down,
                                           int all_active_two_disable_f32_cache) {
    return bn_gpu_policy_cuda_q6k_moe_down_f32_cache_enabled(
        down && down->f32_data != NULL, all_active_two_disable_f32_cache);
}

static int cuda_use_q6k_moe_down_f16_cache(const BnCudaBuffer *down) {
    return bn_gpu_policy_cuda_q6k_moe_down_f16_cache_enabled(
...
static int cuda_matmul_device_out(BnCudaCtx *ctx, float *d_dst,
    ...
    } else if (bn_backend_quant_q4k_q8_1_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0) {
        int x_blocks = (cols + 31) / 32;
    ...
    } else if (bn_backend_quant_q5k_q8_1_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0) {
        int x_blocks = (cols + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
    ...
    } else if (bn_backend_quant_q6k_q8k_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0 &&
    ...
    } else if (bn_backend_quant_q8_0_matmul_candidate(type) &&
               (cols & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, d_dst,
                (const BnBlockQ8_0 *)w->data, d_x, rows, cols,
                n_tokens, 0) != 0)
            return -1;
    } else if (bn_backend_quant_q5_0_matmul_candidate(type) &&
               (cols & 31) == 0) {
        dim3 grid(((rows + 3) / 4 + warps - 1) / warps, n_tokens, 1);
...
static int cuda_matvec(void *vctx, float *out, void *W_buf, const float *x,
    ...
    if (bn_backend_quant_f16_float_cache_matvec_candidate(type) &&
        ...
        q4k_q8k_dot_matvec_kernel<<<blocks, threads>>>(
            ...
            rows, cols, 0);
    } else if (bn_backend_quant_q6k_q8k_matvec_candidate(type) &&
               (cols % BN_QK_K) == 0 &&
        ...
        q6k_dot_matvec_kernel<<<blocks, threads>>>(
            ...
            rows, cols, 0);
    } else if (bn_backend_quant_q6k_warp_matvec_candidate(type) &&
               (cols % BN_QK_K) == 0 &&
...
static int cuda_dense_ffn_batch_impl(void *vctx, float *out,
    ...
    if (stacked_gateup && (gate->f16_data || gate->f32_data) &&
        ...
        err = cudaSuccess;
               ...
               bn_backend_quant_q4k_q8_1_matmul_candidate(gate_type) &&
               (dim % BN_QK_K) == 0) {
        ...
        if (n_tokens >= 4) {
            ...
            q4k_dot_matmul_kernel<<<grid, threads, 0>>>(
               ...
               bn_backend_quant_q5k_q8_1_matmul_candidate(gate_type) &&
               (dim % BN_QK_K) == 0) {
        int x_blocks = (dim + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
        ...
        if (n_tokens >= 4) {
            ...
            q5k_dot_matmul_kernel<<<grid, threads, 0>>>(
               ...
               bn_backend_quant_q5_0_matmul_candidate(gate_type) &&
               (dim & 31) == 0) {
        ...
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ...
            hidden_dim * 2, dim, n_tokens, 0);
               ...
               bn_backend_quant_q8_0_matmul_candidate(gate_type) &&
               (dim & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
        ...
        bn_backend_quant_q5_0_pair_matmul(gate_type, up_type) &&
        (dim & 31) == 0) {
        ...
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ...
            hidden_dim, dim, n_tokens, (size_t)n_tokens * hidden_dim);
               ...
               bn_backend_quant_q8_0_pair_matmul(gate_type, up_type) &&
               (dim & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
                (const BnBlockQ8_0 *)gate->data, gate_input,
        ...
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
            ...
            return -1;
               ...
               bn_backend_quant_q4k_pair_matmul(gate_type, up_type) &&
               (dim % BN_QK_K) == 0) {
        int x_blocks = (dim + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
        ...
        if (n_tokens >= 4) {
            ...
            q4k_dot_matmul_kernel<<<grid, threads, 0>>>(
               ...
               bn_backend_quant_q5k_pair_matmul(gate_type, up_type) &&
               (dim % BN_QK_K) == 0) {
        int x_blocks = (dim + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
    ...
    if ((down->f16_data || down->f32_data) &&
        ...
        err = cudaSuccess;
    } else if (bn_backend_quant_q8_0_matmul_candidate(down_type) &&
               (hidden_dim & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
                (const BnBlockQ8_0 *)down->data, ctx->d_x,
                dim, hidden_dim, n_tokens, 0) != 0)
            return -1;
    } else if (bn_backend_quant_q5_0_matmul_candidate(down_type) &&
               (hidden_dim & 31) == 0) {
        dim3 grid(((dim + 3) / 4 + warps - 1) / warps, n_tokens, 1);
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ctx->d_out, (const BnBlockQ5_0 *)down->data, ctx->d_x,
            dim, hidden_dim, n_tokens, 0);
    } else if (bn_backend_quant_q5k_q8_1_matmul_candidate(down_type) &&
...
static int cuda_prefill_attention_wo(void *vctx, float *out, void *wo_buf,
    ...
    if ((wo->f16_data || wo->f32_data) &&
        cuda_cublas_matmul_f16(ctx, ctx->d_out, wo, ctx->d_x, wo_rows,
                               wo_cols, n_tokens) == 0) {
        err = cudaSuccess;
    } else if (bn_backend_quant_q4k_q8_1_matmul_candidate(wo_type) &&
               (wo_cols % BN_QK_K) == 0) {
    ...
    } else if (bn_backend_quant_q5k_q8_1_matmul_candidate(wo_type) &&
               (wo_cols % BN_QK_K) == 0) {
        int x_blocks = (wo_cols + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
    ...
    } else if (bn_backend_quant_q6k_q8k_matmul_candidate(wo_type) &&
               (wo_cols % BN_QK_K) == 0 &&
               bn_gpu_policy_cuda_q6k_dot_enabled()) {
    ...
    } else if (bn_backend_quant_q5_0_matmul_candidate(wo_type) &&
               (wo_cols & 31) == 0) {
        dim3 grid(((wo_rows + 3) / 4 + warps - 1) / warps, n_tokens, 1);
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ctx->d_out, (const BnBlockQ5_0 *)wo->data, ctx->d_x, wo_rows,
            wo_cols, n_tokens, 0);
    } else if (bn_backend_quant_q8_0_matmul_candidate(wo_type) &&
               (wo_cols & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
...
#define BN_CUDA_DENSE_PROFILE_STEP(code_) do {                         \
    ...
    int ffn_act_ready = 0;
    if (stacked_gateup &&
        bn_backend_quant_q4k_fused_gateup_q8_1_candidate(gate_type) &&
...
#define BN_CUDA_EXEC_FAIL(reason) do { \
        ...
        case BN_GPU_CODE_MATVEC: {
            ...
            if (bn_backend_quant_q5_0_matvec_candidate(op->type) &&
                ...
                BN_CUDA_LAUNCH(ctx, q5_0_matvec_warp_kernel, blocks,
                    ...
                    out, (const BnBlockQ5_0 *)w->data, in, bias,
                    op->rows, op->cols, out_offset);
            } else if (bn_backend_quant_q6k_q8k_matvec_candidate(
                           op->type) &&
                       (op->cols % BN_QK_K) == 0 &&
                       (force_q6k_dot ||
            ...
            } else if (bn_backend_quant_q6k_warp_matvec_candidate(
                           op->type) &&
                ...
                BN_CUDA_LAUNCH_STABLE(ctx, stable_decode_matvec,
                    ...
                    op->rows, op->cols, out_offset);
            } else if (bn_backend_quant_q4k_q8k_matvec_candidate(
                           op->type) &&
            ...
            } else if (bn_backend_quant_q4k_q8_1_matvec_candidate(
                           op->type) &&
                       (op->cols % BN_QK_K) == 0 && enable_q4k_dot) {
                if (cuda_ensure_q8_1(ctx, op->cols) != 0)
            ...
            } else if (bn_backend_quant_q5k_q8_1_matvec_candidate(
                           op->type) &&
                       (op->cols % BN_QK_K) == 0 && enable_q5k_dot) {
                if (cuda_ensure_q8_1(ctx, op->cols) != 0)
        ...
        case BN_GPU_CODE_Q5K_MATVEC_SPLIT: {
            ...
            } else if (bn_backend_quant_q4k_split_q8_1_candidate(
                           op->type) &&
                (cols % BN_QK_K) == 0 && split1 != 1 && enable_q4k_dot) {
                if (cuda_ensure_q8_1(ctx, cols) != 0)
            ...
            } else if (bn_backend_quant_q5k_split_q8_1_candidate(
                           op->type) &&
        ...
        case BN_GPU_CODE_FUSED_GATEUP_SILU: {
            ...
            if (bn_backend_quant_q5_0_fused_gateup_candidate(op->type) &&
                (cols & 31) == 0 &&
                ...
                BN_CUDA_LAUNCH_STATIC(ctx, q5_0_fused_gateup_silu_warp_kernel,
                    ...
                    up_rows, cols);
            } else if (bn_backend_quant_q8_0_fused_gateup_candidate(
                           op->type) &&
                       (cols & 31) == 0 &&
            ...
            } else if (bn_backend_quant_q4k_fused_gateup_q8_1_candidate(
                           op->type) &&
                       (cols % BN_QK_K) == 0 && enable_q4k_dot) {
                if (cuda_ensure_q8_1(ctx, cols) != 0) return -1;
                BnCudaBlockQ8_1 *xq =
            ...
            } else if (bn_backend_quant_q5k_fused_gateup_q8_1_candidate(
                           op->type) &&
                ...
                if (routed_q8) {
                    ...
                    int use_q4k_q8k_dot =
                        bn_gpu_policy_cuda_moe_q4k_q8k_dot_enabled(
                            use_moe_all_active_two_q8k_default,
                    ...
                    if (bn_backend_quant_moe_down_is_q6k(down_type)) {
                        ...
                        if (moe_all_active_two_f32_exact_4row_down) {
                            ...
                            cuda_use_q6k_moe_down_f32_pair2(n_experts, k)) {
                            if (!cuda_use_q6k_moe_down_f32_pair2_4row()) {
                                BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
                            ...
                            if (moe_all_active_two_q6k_ordered_down) {
                                ...
                                if (cuda_use_q6k_moe_down_q8k_all_active_two_fixed(
                                        moe_all_active_two_kquant)) {
                                ...
                                if (fuse_resid_norm && nw && nw->data &&
                                    resid && norm_out) {
                                    if (cuda_use_q6k_moe_down_q8k_k8_exact_2048_768(
                                            dim, hidden, k)) {
                                        BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
                                ...
                                if (!cuda_use_q6k_moe_down_q8k_all_active_two_accum_4row()) {
                            ...
                            } else if (cuda_use_q6k_moe_down_q8k_pair_4row()) {
                                int pair4_blocks =
                                    ((dim + 3) / 4 + warps - 1) / warps;
                                BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
                        ...
                        } else if (cuda_use_q6k_moe_down_f32_cache(
                            ...
                            BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
                                ...
                                out, (const float *)down->f32_data, mid,
                                route, dim, hidden, n_experts, k);
                        } else if (cuda_use_q6k_moe_down_f16_cache(down)) {
                            BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
