...
static int cuda_matmul_device_out(BnCudaCtx *ctx, float *d_dst,
    ...
    if (!cuda_force_quant_matmul_for_type(type) &&
        ...
        cuda_cublas_matmul_f16(ctx, d_dst, w, d_x, rows, cols,
                               n_tokens) == 0) {
        err = cudaSuccess;
    } else if (bn_backend_quant_asymmetric_kquant_dot_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0 &&
               n_tokens > 1 &&
    ...
    } else if (bn_backend_quant_asymmetric_kquant_prepared_input_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0) {
        int x_blocks = (cols + 31) / 32;
    ...
    } else if (bn_backend_quant_deinterleaved_kquant_prepared_input_matmul_candidate(type) &&
               (cols % BN_QK_K) == 0) {
        int x_blocks = (cols + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
    ...
    } else if (bn_backend_quant_native_quant_matmul_candidate(type) &&
               (cols & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, d_dst,
                (const BnBlockQ8_0 *)w->data, d_x, rows, cols,
                n_tokens, 0) != 0)
            return -1;
    } else if (bn_backend_quant_legacy_block_matmul_candidate(type) &&
               (cols & 31) == 0) {
        dim3 grid(((rows + 3) / 4 + warps - 1) / warps, n_tokens, 1);
...
static int cuda_dense_ffn_batch_impl(void *vctx, float *out,
    ...
    if (stacked_gateup && (gate->f16_data || gate->f32_data) &&
        ...
        err = cudaSuccess;
               ...
               bn_backend_quant_asymmetric_kquant_prepared_input_matmul_candidate(gate_type) &&
               (dim % BN_QK_K) == 0) {
        ...
        if (n_tokens >= 4) {
            ...
            q4k_dot_matmul_kernel<<<grid, threads, 0>>>(
               ...
               bn_backend_quant_deinterleaved_kquant_prepared_input_matmul_candidate(gate_type) &&
               (dim % BN_QK_K) == 0) {
        int x_blocks = (dim + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
        ...
        if (n_tokens >= 4) {
            ...
            q5k_dot_matmul_kernel<<<grid, threads, 0>>>(
               ...
               bn_backend_quant_legacy_block_matmul_candidate(gate_type) &&
               (dim & 31) == 0) {
        ...
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ...
            hidden_dim * 2, dim, n_tokens, 0);
               ...
               bn_backend_quant_native_quant_matmul_candidate(gate_type) &&
               (dim & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
    ...
    if ((down->f16_data || down->f32_data) &&
        ...
        err = cudaSuccess;
    } else if (bn_backend_quant_native_quant_matmul_candidate(down_type) &&
               (hidden_dim & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
                (const BnBlockQ8_0 *)down->data, ctx->d_x,
                dim, hidden_dim, n_tokens, 0) != 0)
            return -1;
    } else if (bn_backend_quant_legacy_block_matmul_candidate(down_type) &&
               (hidden_dim & 31) == 0) {
        dim3 grid(((dim + 3) / 4 + warps - 1) / warps, n_tokens, 1);
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ctx->d_out, (const BnBlockQ5_0 *)down->data, ctx->d_x,
            dim, hidden_dim, n_tokens, 0);
    } else if (bn_backend_quant_deinterleaved_kquant_prepared_input_matmul_candidate(down_type) &&
...
static int cuda_prefill_attention_wo(void *vctx, float *out, void *wo_buf,
    ...
    if ((wo->f16_data || wo->f32_data) &&
        cuda_cublas_matmul_f16(ctx, ctx->d_out, wo, ctx->d_x, wo_rows,
                               wo_cols, n_tokens) == 0) {
        err = cudaSuccess;
    } else if (bn_backend_quant_asymmetric_kquant_prepared_input_matmul_candidate(wo_type) &&
               (wo_cols % BN_QK_K) == 0) {
    ...
    } else if (bn_backend_quant_deinterleaved_kquant_prepared_input_matmul_candidate(wo_type) &&
               (wo_cols % BN_QK_K) == 0) {
        int x_blocks = (wo_cols + 31) / 32;
        if (cuda_ensure_q8_1(ctx, x_blocks * 32 * n_tokens) != 0)
            return -1;
    ...
    } else if (bn_backend_quant_legacy_block_matmul_candidate(wo_type) &&
               (wo_cols & 31) == 0) {
        dim3 grid(((wo_rows + 3) / 4 + warps - 1) / warps, n_tokens, 1);
        q5_0_matmul4_warp_kernel<<<grid, threads, 0>>>(
            ctx->d_out, (const BnBlockQ5_0 *)wo->data, ctx->d_x, wo_rows,
            wo_cols, n_tokens, 0);
    } else if (bn_backend_quant_native_quant_matmul_candidate(wo_type) &&
               (wo_cols & 31) == 0) {
        if (cuda_launch_q8_0_matmul(ctx, ctx->d_out,
                ...
                if (routed_q8) {
                    ...
                    if (bn_backend_quant_moe_down_uses_down_kquant(down_type)) {
                        ...
                        int moe_all_active_two_pair_down_f32_layer =
                            ...
                            bn_gpu_policy_all_active_two_kquant_q6k_pair_down_f32_layer_selected(moe_layer);
                        int moe_all_active_two_q6k_ordered_down =
                            moe_all_active_two_kquant &&
                            bn_gpu_policy_all_active_two_kquant_q6k_ordered_down_enabled() &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_ordered_down_disabled();
                        int moe_all_active_two_allow_f32_down_default =
                            !moe_all_active_two_fast_moe_ffn ||
                            bn_gpu_policy_all_active_two_kquant_q6k_f32_down_default_enabled();
                        int moe_all_active_two_f32_down_default =
                            moe_all_active_two_kquant &&
                            down->f32_data &&
                            !moe_all_active_two_q6k_ordered_down &&
                            !moe_all_active_two_pair_down_enabled &&
                            moe_all_active_two_allow_f32_down_default &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_f32_down_default_disabled();
                        int moe_all_active_two_f32_down =
                            (moe_all_active_two_f32_down_default ||
                             moe_all_active_two_pair_down_f32_layer) &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_f32_all_active_down_disabled();
                        ...
                        int moe_all_active_two_disable_f32_cache =
                            moe_all_active_two_kquant &&
                            moe_all_active_two_fast_moe_ffn &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_f32_cache_enabled();
                        ...
                        int moe_all_active_two_float_4row_down =
                            moe_all_active_two_kquant &&
                            moe_all_active_two_fast_moe_ffn &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_float_4row_down_default_disabled() &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_float_4row_down_disabled();
                        int moe_all_active_two_exact_down_layer_selected =
                            bn_gpu_policy_all_active_two_kquant_q6k_f32_exact_4row_down_layer_selected(
                                moe_layer);
                        int moe_all_active_two_f32_exact_4row_down =
                            ...
                            !bn_gpu_policy_all_active_two_kquant_q6k_f32_exact_4row_down_default_disabled() &&
                            !bn_gpu_policy_all_active_two_kquant_q6k_f32_exact_4row_down_disabled();
                        if (moe_all_active_two_f32_exact_4row_down) {
                            ...
                            int use_q6k_all_active_two_accum =
                            ...
                            if (moe_all_active_two_q6k_ordered_down) {
                                BN_CUDA_LAUNCH_STABLE(ctx, graph_exec,
                            ...
                            } else if (use_q6k_all_active_two_accum) {
