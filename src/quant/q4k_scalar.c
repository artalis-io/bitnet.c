...
#define Q4K_SCALAR_SDOT_TILE_T 4

void bn_quant_q4k_scalar_sdot_matmul_range(void *ctx,
                                            int row_start,
                                            int row_end) {
    ...
    for (int row = row_start; row < row_end; row++) {
        for (int t0 = 0; t0 < n_tokens; t0 += Q4K_SCALAR_SDOT_TILE_T) {
            int tile_n = t0 + Q4K_SCALAR_SDOT_TILE_T <= n_tokens
                ? Q4K_SCALAR_SDOT_TILE_T : n_tokens - t0;
            float sums[Q4K_SCALAR_SDOT_TILE_T] = {0};
                    ...
                    for (int p = 0; p < 4; p++) {
                        ...
                        for (int i = 0; i < 32; i++) {
                            ...
                            slo += (int32_t)(q & 0x0f) *
                                   (int32_t)xb[p * 64 + i];
                            shi += (int32_t)(q >> 4) *
                                   (int32_t)xb[p * 64 + 32 + i];
                        }
                        sumi += slo * (int32_t)sc[2 * p] +
                                shi * (int32_t)sc[2 * p + 1];
                    }

                    float dd = dx * d;
                    float ddmin = dx * dmin;
                    sums[ti] -= ddmin * (float)bsum_corr;
