...
static const BnQuantFormatOps g_quant_formats[] = {
    { BN_GGUF_TENSOR_F32,      "F32",      BN_QUANT_LAYOUT_DENSE,    1,   4,   BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_CPU_NATIVE_PREFILL_ORDER, BN_QUANT_CPU_HOOKS },
    ...
    { BN_GGUF_TENSOR_Q4_0,     "Q4_0",     BN_QUANT_LAYOUT_BLOCK32,  32,  18,  BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_CPU_REPACKED | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_GPU_NATIVE | BN_QUANT_CAP_GPU_REPACKED | BN_QUANT_CAP_CPU_FUSED_KQUANT_GATEUP_SILU | BN_QUANT_CAP_MOE_ROUTED_LOWBIT_BLOCK32 | BN_QUANT_CAP_CPU_X86_PACKED_BLOCK32_SUM | BN_QUANT_CAP_GPU_MMVQ_BLOCK32_NIBBLE_ORIGINAL_SUM | BN_QUANT_CAP_GPU_MMQ_BLOCK32_SIGNED_NIBBLE | BN_QUANT_CAP_GPU_ROUTED_BLOCK32_NIBBLE | BN_QUANT_CAP_GPU_MMQ_LOGICAL_PROJECTION_ROWS | BN_QUANT_CAP_GPU_ROUTED_MERGED_PROJECTION_ROWS, BN_QUANT_CPU_HOOKS },
    ...
    { BN_GGUF_TENSOR_Q5_1,     "Q5_1",     BN_QUANT_LAYOUT_BLOCK32,  32,  24,  BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_MOE_ROUTED_MIDBIT_BLOCK32_DOWN | BN_QUANT_CAP_CPU_X86_GEMV_ORDER_MATMUL | BN_QUANT_CAP_GPU_ROUTED_BLOCK32_AFFINE_MMVQ_DOWN, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_Q8_0,     "Q8_0",     BN_QUANT_LAYOUT_BLOCK32,  32,  34,  BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_CPU_REPACKED | BN_QUANT_CAP_GPU_DENSE_GRAPH_NATIVE_QUANT_FORMAT | BN_QUANT_CAP_MOE_ALL_F16_LAZY_AUX | BN_QUANT_CAP_MOE_PREFERS_QUANT_ONLY | BN_QUANT_CAP_MOE_NATIVE_QUANT_ROUTE | BN_QUANT_CAP_MOE_DIRECT_ROUTED_DOWN | BN_QUANT_CAP_CPU_X86_GEMV_ORDER_MATMUL, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_I2_S,     "I2_S",     BN_QUANT_LAYOUT_I2S,      4,   1,   BN_QUANT_CAP_LOADABLE_CPU_TIED_LOGITS, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_MXFP4,    "MXFP4",    BN_QUANT_LAYOUT_BLOCK32,  32,  17,  BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_CPU_X86_GEMV_ORDER_MATMUL | BN_QUANT_CAP_GPU_MMVQ_BLOCK32_E8M0 | BN_QUANT_CAP_GPU_MMQ_BLOCK32_FP4 | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_GPU_ROUTED_BLOCK32_E8M0, BN_QUANT_CPU_HOOKS },
    ...
    { BN_GGUF_TENSOR_Q3_K,     "Q3_K",     BN_QUANT_LAYOUT_BLOCK256, 256, 110, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_LAZY_AUX_CACHE | BN_QUANT_CAP_GPU_MMQ_F32_SCALE | BN_QUANT_CAP_GPU_MMQ_SUBBLOCK16 | BN_QUANT_CAP_GPU_MMVQ_SUBBLOCK16_INPUT_SCALE, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_Q4_K,     "Q4_K",     BN_QUANT_LAYOUT_BLOCK256, 256, 144, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_PREPARED_KQUANT_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH_KQUANT | BN_QUANT_CAP_GPU_NATIVE_QUANT_SPLIT | BN_QUANT_CAP_MOE_LARGE_AUX_CACHE | BN_QUANT_CAP_MOE_DOWN_SMALL_KQUANT_F32_CACHE | BN_QUANT_CAP_MOE_ROUTED_KQUANT_GATEUP | BN_QUANT_CAP_MOE_DIRECT_ROUTED_DOWN | BN_QUANT_CAP_GPU_MMQ_ORIGINAL_SUM | BN_QUANT_CAP_GPU_ROUTED_KQUANT_MMVQ_GATEUP, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_Q5_K,     "Q5_K",     BN_QUANT_LAYOUT_BLOCK256, 256, 176, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_PREPARED_KQUANT_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH_KQUANT | BN_QUANT_CAP_GPU_FUSED_GATEUP_BACKEND_OPT_IN | BN_QUANT_CAP_MOE_LARGE_AUX_CACHE | BN_QUANT_CAP_MOE_DIRECT_ROUTED_DOWN | BN_QUANT_CAP_MOE_ROUTED_MIDBIT_KQUANT_GATEUP | BN_QUANT_CAP_CPU_X86_GEMV_ORDER_MATMUL | BN_QUANT_CAP_GPU_MMQ_ORIGINAL_SUM | BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_DOWN | BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_GATEUP, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_Q6_K,     "Q6_K",     BN_QUANT_LAYOUT_BLOCK256, 256, 210, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_PREPARED_KQUANT_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH_KQUANT_LOGITS_REFINE | BN_QUANT_CAP_MOE_LARGE_AUX_CACHE | BN_QUANT_CAP_LOGITS_KQUANT_F32_CACHE | BN_QUANT_CAP_MOE_DOWN_KQUANT_F32_CACHE | BN_QUANT_CAP_MOE_DOWN_CUBLAS_CACHE | BN_QUANT_CAP_AUX_CACHE_F16 | BN_QUANT_CAP_MOE_DIRECT_ROUTED_DOWN | BN_QUANT_CAP_GPU_REFERENCE_PREPARED_ACCUMULATION | BN_QUANT_CAP_GPU_MMQ_F32_SCALE | BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_SIGNED_DOWN, BN_QUANT_CPU_HOOKS },
    ...
    { BN_GGUF_TENSOR_IQ4_NL,   "IQ4_NL",   BN_QUANT_LAYOUT_BLOCK32,  32,  18,  BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_GPU_MMQ_F32_SCALE | BN_QUANT_CAP_GPU_MMQ_SUBBLOCK32 | BN_QUANT_CAP_GPU_MMVQ_BLOCK32_CODEBOOK_FP16, BN_QUANT_CPU_HOOKS },
    { BN_GGUF_TENSOR_IQ4_XS,   "IQ4_XS",   BN_QUANT_LAYOUT_BLOCK256, 256, 136, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_LAZY_AUX_CACHE | BN_QUANT_CAP_CPU_NATIVE_PREFILL_ORDER | BN_QUANT_CAP_GPU_PACKED_CODEBOOK_MATVEC | BN_QUANT_CAP_GPU_MMQ_F32_SCALE | BN_QUANT_CAP_GPU_MMQ_SUBBLOCK32 | BN_QUANT_CAP_GPU_MMVQ_SUBBLOCK32_INT_SCALE, BN_QUANT_CPU_HOOKS },
    ...
    { BN_GGUF_TENSOR_IQ3_S,    "IQ3_S",    BN_QUANT_LAYOUT_BLOCK256, 256, 110, BN_QUANT_CAP_LOADABLE_CPU_EMBEDDED_TIED_LOGITS | BN_QUANT_CAP_GPU_DENSE_GRAPH | BN_QUANT_CAP_GPU_MMQ_F32_SCALE | BN_QUANT_CAP_GPU_MMQ_SUBBLOCK32 | BN_QUANT_CAP_GPU_MMVQ_SUBBLOCK32_INT_SCALE, BN_QUANT_CPU_HOOKS },
...
int bn_quant_format_cpu_matmul_matches_matvec(int type) {
    ...
    if (!bn_quant_format_has_cpu_matvec(type))
        return 0;
...
int bn_quant_format_requires_native_cpu_prefill(int type) {
    return bn_quant_format_has_cap(type, BN_QUANT_CAP_CPU_NATIVE_PREFILL_ORDER);
}

int bn_quant_format_cpu_matvec_uses_float_input(int type) {
#if defined(__AVX2__)
    if (type == BN_GGUF_TENSOR_IQ3_S || type == BN_GGUF_TENSOR_Q3_K ||
        type == BN_GGUF_TENSOR_IQ4_NL)
        return 0;
#endif
    switch (type) {
...
uint32_t bn_quant_format_gpu_matvec_native_quant_flag(int type, int enabled) {
    return enabled && bn_quant_format_supports_direct_native_quant_matvec(type)
        ? BN_QUANT_GPU_MATVEC_FLAG_KQUANT_DOT
        : 0u;
}
...
uint32_t bn_quant_format_gpu_matvec_block_q8_activation_flag(int type,
    ...
    if (type == BN_GGUF_TENSOR_Q4_0)
        return BN_QUANT_GPU_MATVEC_FLAG_BLOCK_Q8_ACTIVATION;
    if (type == BN_GGUF_TENSOR_Q8_0)
        return BN_QUANT_GPU_MATVEC_FLAG_KQUANT_DOT;
    return 0u;
}
...
int bn_quant_format_tied_logits_uses_prepared_weight(int type) {
    ...
    // An embedding used by GET_ROWS stays in the reference's raw buffer.
    return !bn_quant_format_has_cap(type,
        BN_QUANT_CAP_CPU_X86_PACKED_BLOCK32_SUM);
#else
    (void)type;
    return 1;
#endif
...
int bn_quant_format_supports_moe_direct_routed_down(int type) {
    return bn_quant_format_has_cap(type,
                                   BN_QUANT_CAP_MOE_DIRECT_ROUTED_DOWN);
}

int bn_quant_format_supports_moe_routed_midbit_block32_down(int type) {
    return bn_quant_format_has_cap(
        type, BN_QUANT_CAP_MOE_ROUTED_MIDBIT_BLOCK32_DOWN);
}

int bn_quant_format_supports_moe_routed_midbit_kquant_gateup(int gate_type,
                                                              int up_type) {
    return bn_quant_format_has_cap(
               gate_type, BN_QUANT_CAP_MOE_ROUTED_MIDBIT_KQUANT_GATEUP) &&
           bn_quant_format_has_cap(
               up_type, BN_QUANT_CAP_MOE_ROUTED_MIDBIT_KQUANT_GATEUP);
}
...
int bn_quant_format_prefill_requires_logical_rows(int type, int n_tokens) {
    return n_tokens > 8 && bn_quant_format_has_cap(type,
        BN_QUANT_CAP_GPU_MMQ_LOGICAL_PROJECTION_ROWS);
}

int bn_quant_format_supports_moe_merged_gateup(int gate_type, int up_type) {
    return gate_type == up_type && bn_quant_format_has_cap(gate_type,
        BN_QUANT_CAP_GPU_ROUTED_MERGED_PROJECTION_ROWS);
}

int bn_quant_format_supports_moe_gathered_expert_batch(int gate_type,
                                                      int up_type,
                                                      int down_type) {
    return bn_quant_format_has_cap(gate_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_NIBBLE) &&
           bn_quant_format_has_cap(up_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_NIBBLE) &&
           bn_quant_format_has_cap(down_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_NIBBLE);
}
...
int bn_quant_format_supports_moe_routed_lowbit_block32(int gate_type,
                                                        ...
                                                        int down_type) {
    return bn_quant_format_has_cap(gate_type,
                                   BN_QUANT_CAP_MOE_ROUTED_LOWBIT_BLOCK32) &&
           bn_quant_format_has_cap(up_type,
                                   BN_QUANT_CAP_MOE_ROUTED_LOWBIT_BLOCK32) &&
           bn_quant_format_has_cap(down_type,
                                   BN_QUANT_CAP_MOE_ROUTED_LOWBIT_BLOCK32);
...
BnQWeight bn_quant_f32_weight(const float *data, int rows, int cols) {
    BnQWeight weight = { data, BN_GGUF_TENSOR_F32, rows, cols, 1.0f };
    return weight;
}

int bn_quant_format_supports_moe_routed_e8m0(int gate_type, int up_type, int down_type) {
    return bn_quant_format_has_cap(gate_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_E8M0) &&
           bn_quant_format_has_cap(up_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_E8M0) &&
           bn_quant_format_has_cap(down_type, BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_DOWN);
}

int bn_quant_format_supports_moe_routed_ordered_kquant(int gate_type, int up_type, int down_type) {
    return bn_quant_format_has_cap(gate_type, BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_GATEUP) &&
           bn_quant_format_has_cap(up_type, BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_GATEUP) &&
           bn_quant_format_has_cap(down_type, BN_QUANT_CAP_GPU_ROUTED_KQUANT_ORDERED_SIGNED_DOWN);
}

int bn_quant_format_supports_moe_routed_affine_mmvq(int gate_type, int up_type, int down_type) {
    return gate_type == up_type &&
        bn_quant_format_has_cap(gate_type, BN_QUANT_CAP_GPU_ROUTED_KQUANT_MMVQ_GATEUP) &&
        bn_quant_format_has_cap(down_type, BN_QUANT_CAP_GPU_ROUTED_BLOCK32_AFFINE_MMVQ_DOWN);
}
