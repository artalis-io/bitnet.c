...
typedef struct {
    ...
    float *ffn_post_norm_1;                 // optional dense-branch post norm [dim]
...
typedef struct {
    BnQWeight inp_gate, proj;                // per-layer embedding adapters
    float *post_norm;                        // [dim]
...
typedef struct {
    ...
    float *router_scale;                    // optional [dim] router input scale
    float *expert_down_scale;               // optional [n_experts] down projection scale
