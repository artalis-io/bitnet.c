...
// Q4_K matvec using the 32-lane reference reduction order.

constant uint QK_K = 256;
constant uint BLOCK_BYTES = 144;
constant uint SIMD_GROUPS_PER_TG = 8;
constant uint ROWS_PER_SIMD_GROUP = 2;
...
kernel void q4k_matvec(
    ...
    uint first_row = tile * ROWS_PER_TG +
    ...
    device const float *y4 = x + x_base + uint(ix) * QK_K +
                             64u * uint(iq) + 8u * uint(ir);
    float sumf[ROWS_PER_SIMD_GROUP] = {0.0f, 0.0f};
    ...
    for (uint ib = ix; ib < n_blocks; ib += 4) {
        ...
        for (uint row = 0; row < n_rows; row++) {
            ...
            packed[3] = ((sc[4] >> 4) & mask2) |
                        ((sc[2] & mask3) >> 2);
            thread const uchar *scale = (thread const uchar *)packed;
    ...
    for (uint row = 0; row < n_rows; row++) {
        ...
        if (lane == 0)
            out[out_offset + token * rows + first_row + row] = sum;
    }
}
