...
kernel void q5k_q8k_matvec(device const uchar *weights [[buffer(0)]],
    ...
    if (row < rows) {
        ...
        for (uint b = 0; b < n_blocks; b++) {
            ...
            device const uchar *qh = block + 16u;
            device const uchar *q = block + 48u + group * 32u;
            uint2 sm = get_scale_min(lane, block + 4u);
            uint x_base = token * cols + b * QK_K + lane * 32u;
            int qx = 0;
            for (uint i = 0; i < 32u; i++) {
                uint low = high ? q[i] >> 4 : q[i] & 15u;
                uint high_bit = ((qh[i] >> lane) & 1u) << 4u;
                qx += int(low | high_bit) * int(xq[x_base + i]);
            }
