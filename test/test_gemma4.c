...
static void test_gemma4_dense(void) {
    ...
    assert(m.weights.layers[0].ffn_kind == BN_LAYER_FFN_DENSE);
    assert(m.weights.layers[0].attn.q_dim == 256);
...
static void test_gemma4_moe_fused_gate_up(void) {
    ...
    BnMoEExpertMap *em = &m.weights.layers[0].moe.expert_map;
    assert(m.config.n_kv_heads == 2);
    assert(m.config.kv_dim == 128);
    assert(m.weights.layers[0].ffn_kind == BN_LAYER_FFN_MOE);
    assert(m.weights.layers[1].attn.n_kv_heads == 1);
    assert(m.weights.layers[1].attn.kv_dim == 32);
