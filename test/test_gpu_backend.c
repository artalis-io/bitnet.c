...
static void test_gpu_policy_helpers(void) {
    ...
    assert(bn_gpu_policy_rope_dims_for_head(NULL, 128) == 0);
    layers.n_layers = 12;
    ...
    assert(bn_gpu_policy_rope_dims_for_head(&layers, layers.head_size) == 128);
    float rope_freqs[2] = {0.0f, 0.0f};
    bn_gpu_policy_init_rope_frequencies(&layers, rope_freqs, 2);
    ...
    assert(bn_gpu_policy_kquant_logits_refine_top_or_default(64) == 13);
    unsetenv("BN_GPU_ENABLE_Q6_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_Q6_LOGITS_REFINE");
    unsetenv("BN_GPU_ENABLE_KQUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_KQUANT_LOGITS_REFINE");
    ...
    assert(bn_gpu_policy_native_quant_logits_refine_top_or_default(16) == 7);
    unsetenv("BN_GPU_ENABLE_Q8_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_Q8_LOGITS_REFINE");
    unsetenv("BN_GPU_ENABLE_NATIVE_QUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_NATIVE_QUANT_LOGITS_REFINE");
    ...
    assert(!bn_gpu_policy_cuda_quant_matmul_preferred_for_type(
        BN_GGUF_TENSOR_Q8_0, 1));
    assert(bn_gpu_policy_cuda_quant_matmul_preferred_for_type(
    ...
    assert(!bn_gpu_policy_cuda_down_kquant_4warp_long_enabled(2560, 8192));
    assert(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled(1536, 8960));
    assert(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled(2560, 9728));
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_5WARP_1536_8960", "1", 1);
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_5WARP_2560_9728", "1", 1);
    assert(!bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled(1536, 8960));
    assert(!bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled(2560, 9728));
    assert(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled(1536, 8960));
    assert(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled(2560, 9728));
    ...
    assert(!bn_gpu_policy_all_active_two_kquant_moe_reference_attention_disabled());
    assert(!bn_gpu_policy_all_active_two_kquant_moe_cpu_route_resident_disabled());
    assert(!bn_gpu_policy_all_active_two_kquant_moe_reference_gpu_route_requested());
    assert(!bn_gpu_policy_all_active_two_kquant_moe_reference_gpu_route_disabled());
    ...
    assert(getenv("BN_GPU_SMALL_DENSE_NATIVE_QUANT_FROM_LAYER") != NULL);
    assert(bn_gpu_policy_small_dense_native_quant_from_layer_or_default(40) ==
           0);
    assert(bn_gpu_policy_small_dense_native_quant_to_layer_or_default(40, 0) ==
           6);
    ...
    assert(bn_gpu_policy_metal_native_quant_prepared_enabled());
    assert(getenv("BN_METAL_NATIVE_QUANT_PREPARED") != NULL);
    assert(getenv("BN_METAL_Q4_PREPARED") == NULL);
    unsetenv("BN_METAL_NATIVE_QUANT_PREPARED");
    unsetenv("BN_METAL_Q4_PREPARED");
