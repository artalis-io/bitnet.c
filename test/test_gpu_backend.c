...
static BnCPURuntimePolicy test_cpu_runtime_from_env(void) {
    BnCPURuntimePolicy policy;
    bn_cpu_runtime_policy_from_env(&policy);
    return policy;
}

static BnBackendRuntimePolicy test_backend_runtime_from_env(void) {
...
#define TEST_CUDA_CUBLAS_MATMUL_ENABLED() \
    bn_gpu_policy_cuda_cublas_matmul_enabled(test_current_backend_runtime())
#define TEST_CUDA_CUBLAS_GEMM_ALGO(default_index) \
    bn_gpu_policy_cuda_cublas_gemm_algo_index_or_default( \
        test_current_backend_runtime(), (default_index))
#define TEST_CUDA_CUBLAS_AUX_CACHE_MAX_MB(...) \
    bn_gpu_policy_cuda_cublas_aux_cache_max_mb( \
        test_current_backend_runtime(), __VA_ARGS__)
#define TEST_CUDA_CUBLAS_AUX_CACHE_SUPPORTED(...) \
    bn_gpu_policy_cuda_cublas_aux_cache_supported( \
        test_current_backend_runtime(), __VA_ARGS__)
#define TEST_MOE_ROUTER_GPU_ENABLED() \
    bn_gpu_policy_moe_router_gpu_enabled(test_current_backend_runtime())
#define TEST_MOE_ROUTED_FFN_BATCH_ENABLED() \
    bn_gpu_policy_moe_routed_ffn_batch_enabled(test_current_backend_runtime())
#define TEST_MOE_ROUTED_FFN_BATCH_ALLOWED(...) \
    bn_gpu_policy_moe_routed_ffn_batch_allowed( \
        test_current_backend_runtime(), __VA_ARGS__)
#define TEST_ALL_ACTIVE_TWO_ROUTE_SELECTION_ENABLED() \
    bn_gpu_policy_all_active_two_kquant_moe_route_selection_enabled( \
        test_current_backend_runtime())
#define TEST_POLICY_CALL(fn, ...) \
    fn(test_current_backend_runtime(), __VA_ARGS__)
#define TEST_POLICY_CALL0(fn) fn(test_current_backend_runtime())

static void test_moe_combined_prefill_default(void) {
    BnGPUBackend gpu = {0};
    const char *enable = "BN_CUDA_ENABLE_MOE_ROUTE_ROUTED_FFN_BATCH_LARGE";
    const char *disable = "BN_CUDA_DISABLE_MOE_ROUTE_ROUTED_FFN_BATCH";
    assert(bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 0, 1));
    assert(!bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 1));
    gpu.caps = BN_GPU_CAP_MOE_COMBINED_PREFILL_DEFAULT;
    assert(!bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 0));
    assert(bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 1));
    assert(bn_backend_runtime_policy_set(&gpu.runtime_policy, disable, "1", 1) == 0);
    assert(!bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 0, 1));
    assert(!bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 1));
    assert(bn_backend_runtime_policy_set(&gpu.runtime_policy, enable, "1", 1) == 0);
    assert(!bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 1));
    bn_backend_runtime_policy_unset(&gpu.runtime_policy, disable);
    gpu.caps = 0;
    assert(bn_gpu_policy_backend_moe_routed_ffn_batch_allowed(&gpu, 1, 1));
    bn_backend_runtime_policy_free(&gpu.runtime_policy);
    printf("test_moe_combined_prefill_default... PASSED\n");
}

static void test_backend_runtime_policy_snapshot(void) {
    printf("test_backend_runtime_policy_snapshot... ");
    char gpu_entry[] = "BN_GPU_FORCE_GRAPH=1";
    char gpu_prefix_entry[] = "BN_GPU_FORCE_GRAPH_EXEC=2";
    char gpu_digit_entry[] = "BN_GPU_FORCE_GRAPH1=3";
    char metal_entry[] = "BN_METAL_FULL_BARRIERS=0";
    char unrelated[] = "UNRELATED_VALUE=ignored";
    char *environment[] = { gpu_prefix_entry, metal_entry, unrelated,
                            gpu_digit_entry, gpu_entry, NULL };
    BnBackendRuntimePolicy policy;
    assert(bn_backend_runtime_policy_init(&policy, environment) == 0);
    assert(bn_backend_runtime_policy_enabled(&policy,
                                             "BN_GPU_FORCE_GRAPH"));
    assert(strcmp(bn_backend_runtime_policy_get(
                      &policy, "BN_METAL_FULL_BARRIERS"), "0") == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &policy, "BN_GPU_FORCE_GRAPH_EXEC"), "2") == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &policy, "BN_GPU_FORCE_GRAPH1"), "3") == 0);
    assert(bn_backend_runtime_policy_get(&policy,
                                         "BN_GPU_FORCE_GRAP") == NULL);
    assert(bn_backend_runtime_policy_get(&policy, "UNRELATED_VALUE") == NULL);
    strcpy(gpu_entry, "BN_GPU_FORCE_GRAPH=0");
    assert(strcmp(bn_backend_runtime_policy_get(
                      &policy, "BN_GPU_FORCE_GRAPH"), "1") == 0);
    BnBackendRuntimePolicy clone;
    assert(bn_backend_runtime_policy_clone(&clone, &policy) == 0);
    bn_backend_runtime_policy_free(&policy);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &clone, "BN_GPU_FORCE_GRAPH"), "1") == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &clone, "BN_GPU_FORCE_GRAPH_EXEC"), "2") == 0);
    assert(bn_backend_runtime_policy_set(&clone, "BN_GPU_FORCE_GRAPH",
                                         "4", 0) == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &clone, "BN_GPU_FORCE_GRAPH"), "1") == 0);
    assert(bn_backend_runtime_policy_set(&clone, "BN_GPU_FORCE_GRAPH",
                                         "4", 1) == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &clone, "BN_GPU_FORCE_GRAPH"), "4") == 0);
    assert(bn_backend_runtime_policy_set(&clone, "BN_CUDA_DEVICE",
                                         "auto", 1) == 0);
    assert(strcmp(bn_backend_runtime_policy_get(
                      &clone, "BN_CUDA_DEVICE"), "auto") == 0);
    assert(bn_backend_runtime_policy_set(&clone, "UNRELATED_VALUE",
                                         "bad", 1) == -1);
    bn_backend_runtime_policy_unset(&clone, "BN_GPU_FORCE_GRAPH");
    assert(!bn_backend_runtime_policy_enabled(&clone,
                                              "BN_GPU_FORCE_GRAPH"));
    bn_backend_runtime_policy_free(&clone);
    printf("PASSED\n");
}
...
typedef struct {
    int calls;
    int enabled;
} PreparedNativeQuantConfig;

static int mock_prepare_cpu_operations(void *ctx) {
    int *calls = (int *)ctx;
    (*calls)++;
    return 0;
}

static void test_prepare_cpu_operations(void) {
    printf("test_prepare_cpu_operations... ");
    int calls = 0;
    BnGPUBackend gpu = {
        .prepare_cpu_operations = mock_prepare_cpu_operations,
        .ctx = &calls,
    };
    assert(bn_gpu_backend_prepare_cpu_operations(&gpu) == 0);
    assert(calls == 1);
    gpu.prepare_cpu_operations = NULL;
    assert(bn_gpu_backend_prepare_cpu_operations(&gpu) == 0);
    assert(calls == 1);
    printf("PASSED\n");
}

static void mock_configure_prepared_native_quant(void *ctx, int enabled) {
    PreparedNativeQuantConfig *config = (PreparedNativeQuantConfig *)ctx;
    config->calls++;
    config->enabled = enabled;
}

static void test_prepared_native_quant_configuration(void) {
    printf("test_prepared_native_quant_configuration... ");
    PreparedNativeQuantConfig config = {0};
    BnGPUBackend gpu = {
        .configure_prepared_native_quant =
            mock_configure_prepared_native_quant,
        .ctx = &config,
        .caps = BN_GPU_CAP_PREPARED_NATIVE_QUANT,
    };

    assert(bn_gpu_backend_has_cap(&gpu,
    ...
    bn_gpu_backend_configure_prepared_native_quant(&gpu, 0);
    assert(config.calls == 2);
    assert(config.enabled == 0);
    printf("PASSED\n");
}

static int mock_native_borrowed_supported(void *ctx, size_t size, int type,
                                          int rows, int cols) {
    (void)ctx;
    return size == 72 && type == BN_GGUF_TENSOR_Q4_0 &&
           rows == 4 && cols == 32;
}

static size_t mock_buffer_cache_charge(void *ctx, size_t size, int type,
                                       int rows, int cols) {
    (void)ctx;
    return mock_native_borrowed_supported(NULL, size, type, rows, cols)
        ? 24 : size;
}

static void test_native_borrowed_layout_contract(void) {
    printf("test_native_borrowed_layout_contract... ");
    BnGPUBackend gpu = {0};
    assert(!bn_gpu_backend_native_matvec_borrowed_supported(
        &gpu, 72, BN_GGUF_TENSOR_Q4_0, 4, 32));
    assert(bn_gpu_backend_buffer_cache_charge(
        &gpu, 72, BN_GGUF_TENSOR_Q4_0, 4, 32) == 72);

    gpu.buffer_create_native_matvec_borrowed = mock_create;
    gpu.native_matvec_borrowed_supported = mock_native_borrowed_supported;
    gpu.buffer_cache_charge = mock_buffer_cache_charge;
    assert(bn_gpu_backend_native_matvec_borrowed_supported(
        &gpu, 72, BN_GGUF_TENSOR_Q4_0, 4, 32));
    assert(!bn_gpu_backend_native_matvec_borrowed_supported(
        &gpu, 144, BN_GGUF_TENSOR_Q4_0, 4, 32));
    assert(bn_gpu_backend_buffer_cache_charge(
        &gpu, 72, BN_GGUF_TENSOR_Q4_0, 4, 32) == 24);
    assert(bn_gpu_backend_buffer_cache_charge(
        &gpu, 144, BN_GGUF_TENSOR_Q4_0, 4, 32) == 144);
    printf("PASSED\n");
}

static void test_gpu_moe_cache_entry_cap(void) {
    printf("test_gpu_moe_cache_entry_cap... ");
    BnGPUBackend gpu = {.buffer_destroy = mock_destroy};
    BnGPUMoECache *cache = bn_gpu_moe_cache_create(4096, 24, 3, &gpu);
    assert(cache != NULL);
    assert(cache->n_slots == 3);
    assert(cache->entry_bytes == 24);
    bn_gpu_moe_cache_free(cache);
    printf("PASSED\n");
}
...
static void test_gpu_upload_weights(void) {
    ...
    model.config.hyper_connection_count = 2;
    model.config.hyper_connection_rank = 4;
    ...
    float attn_norm[128];
    float ffn_norm[128];
    float hc_output_norm[256];
    float hc_attn_norm[256];
    float hc_ffn_norm[256];
    for (int i = 0; i < 128; i++) {
        attn_norm[i] = 1.0f;
        ffn_norm[i] = 1.0f;
    }
    ...
    for (int i = 0; i < 256; i++) {
        hc_output_norm[i] = 1.0f;
        hc_attn_norm[i] = 1.0f;
        hc_ffn_norm[i] = 1.0f;
    }
    model.weights.hc_output.norm = hc_output_norm;
    model.weights.layers[0].hc_attn.norm = hc_attn_norm;
    model.weights.layers[0].hc_ffn.norm = hc_ffn_norm;
    ...
    uint8_t *hc_down_data = make_i2s_data(4, 256, scale);
    uint8_t *hc_up_data = make_i2s_data(256, 4, scale);
    uint8_t *hc_inject_data = make_i2s_data(2, 256, scale);
    model.weights.hc_output.down = (BnQWeight){
        .data = hc_down_data, .type = BN_GGUF_TENSOR_I2_S,
        .rows = 4, .cols = 256, .scale = scale,
    };
    model.weights.hc_output.up = (BnQWeight){
        .data = hc_up_data, .type = BN_GGUF_TENSOR_I2_S,
        .rows = 256, .cols = 4, .scale = scale,
    };
    model.weights.layers[0].hc_attn.inject = (BnQWeight){
        .data = hc_inject_data, .type = BN_GGUF_TENSOR_I2_S,
        .rows = 2, .cols = 256, .scale = scale,
    };
    ...
    assert(bn_backend_model_handle(bn_model_backend(&model), -1,
                                   BN_BACKEND_HANDLE_HC_OUTPUT_NORM) != NULL);
    assert(bn_backend_model_handle(bn_model_backend(&model), 0,
                                   BN_BACKEND_HANDLE_HC_ATTN_NORM) != NULL);
    assert(bn_backend_model_handle(bn_model_backend(&model), 0,
                                   BN_BACKEND_HANDLE_HC_FFN_NORM) != NULL);
    ...
    assert(bn_backend_model_qweight_buf(bn_model_backend(&model),
                                        &model.weights.hc_output.down) != NULL);
    assert(bn_backend_model_qweight_buf(bn_model_backend(&model),
                                        &model.weights.hc_output.up) != NULL);
    assert(bn_backend_model_qweight_buf(
               bn_model_backend(&model),
               &model.weights.layers[0].hc_attn.inject) != NULL);
    assert(bn_backend_model_handle(bn_model_backend(&model), 0,
...
static void test_gpu_upload_reference_attention_output_layout(void) {
    printf("test_gpu_upload_reference_attention_output_layout... ");

    BnModel model = {0};
    model.config.n_layers = 1;
    model.config.dim = 128;
    model.config.policy_flags = BN_MODEL_ARCH_POLICY_PER_LAYER_INPUT;
    model.weights.layers = calloc(1, sizeof(BnLayerWeights));
    assert(model.weights.layers);

    uint8_t *wo_data = make_i2s_data(128, 128, 1.0f);
    BnQWeight *wo = &model.weights.layers[0].attn.wo;
    wo->data = wo_data;
    wo->type = BN_GGUF_TENSOR_I2_S;
    wo->rows = 128;
    wo->cols = 128;
    wo->scale = 1.0f;
    ...
    assert(bn_backend_model_qweight_buf(backend, wo) != NULL);
    assert(bn_backend_model_handle(
               backend, 0, BN_BACKEND_HANDLE_WO_PREFILL) != NULL);
...
static void test_model_gpu_moe_prefill_resident(void) {
    ...
    BnModel model = {0};
    /* Backends without an estimate callback retain the FP16-only contract. */
    assert(bn_gpu_backend_f16_cache_extra_bytes(
               NULL, BN_GGUF_TENSOR_Q6_K, 1024, 2560) ==
           (size_t)1024 * 2560 * sizeof(uint16_t));
    assert(bn_gpu_backend_f16_cache_extra_bytes(
               NULL, BN_GGUF_TENSOR_Q6_K, -1, 2560) == 0);
...
static void test_gpu_policy_helpers(void) {
    ...
    BnConfig layers = {0};
    assert(bn_model_arch_attention_layer_count(NULL) == 0);
    assert(bn_model_arch_ssm_layer_count(NULL) == 0);
    assert(!bn_model_arch_uses_hybrid_ssm(NULL));
    assert(!bn_model_arch_uses_hybrid_moe(NULL));
    assert(bn_model_arch_rope_dims_for_head(NULL, 128) == 0);
    layers.n_layers = 12;
    layers.dim = 2048;
    layers.full_attn_interval = 4;
    layers.ssm_inner_size = 4096;
    layers.ssm_time_step_rank = 32;
    layers.ssm_group_count = 8;
    layers.ssm_state_size = 128;
    layers.hidden_dim = 4096;
    layers.hidden_dim = 0;
    layers.full_attn_interval = 0;
    layers.ssm_inner_size = 0;
    layers.ssm_time_step_rank = 0;
    layers.ssm_group_count = 0;
    layers.ssm_state_size = 0;
    layers.head_size = 128;
    layers.rope_theta = 10000.0f;
    assert(bn_model_arch_attention_layer_count(&layers) == 12);
    assert(bn_model_arch_ssm_layer_count(&layers) == 0);
    assert(!bn_model_arch_uses_hybrid_ssm(&layers));
    assert(!bn_model_arch_uses_hybrid_moe(&layers));
    assert(bn_model_arch_rope_dims_for_head(&layers, layers.head_size) == 128);
    float rope_freqs[2] = {0.0f, 0.0f};
    bn_model_arch_init_rope_frequencies(&layers, rope_freqs, 2);
    assert(rope_freqs[0] == 1.0f);
    assert(rope_freqs[1] > 0.0f && rope_freqs[1] < 1.0f);
    layers.full_attn_interval = 4;
    assert(bn_model_arch_attention_layer_count(&layers) == 3);
    assert(bn_model_arch_ssm_layer_count(&layers) == 9);
    assert(!bn_model_arch_uses_hybrid_ssm(&layers));
    assert(!bn_model_arch_uses_hybrid_moe(&layers));
    layers.ssm_inner_size = 128;
    assert(bn_model_arch_uses_hybrid_ssm(&layers));
    assert(!bn_model_arch_uses_moe(&layers));
    BnModel dense_model = {0};
    dense_model.config = layers;
    assert(!bn_model_uses_moe(&dense_model));
    assert(!bn_moe_policy_uses_all_active_two_expert_route(&layers,
                                                            layers.dim));
    assert(!bn_moe_policy_uses_grouped_expert_route(&layers));
    ...
    assert(bn_gpu_policy_cuda_moe_cublas_all_active_two_decode_enabled(test_current_backend_runtime(),
    ...
    assert(bn_model_moe_policy_total_experts(&moe) == 2);
    assert(bn_model_moe_policy_active_experts(&moe) == 2);
    assert(bn_model_moe_policy_expert_hidden_dim(&moe) == 4096);
    assert(bn_model_arch_uses_moe(&moe));
    BnModel moe_model = {0};
    moe_model.config = moe;
    assert(bn_model_uses_moe(&moe_model));
    assert(!bn_model_arch_uses_hybrid_moe(&moe));
    assert(bn_moe_policy_uses_all_active_two_expert_route(&moe, moe.dim));
    assert(bn_moe_policy_uses_all_active_two_expert_set(&moe));
    ...
    moe.n_experts_active = 2;
    assert(!bn_moe_policy_uses_all_active_two_expert_route(&moe, moe.dim));
    assert(bn_moe_policy_uses_grouped_expert_route(&moe));
    ...
    BnGPUBackend routed_metal = { .kind = BN_GPU_BACKEND_METAL };
    BnGPUBackend routed_wgpu = { .kind = BN_GPU_BACKEND_WEBGPU };
    assert(bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_cuda, 1));
    assert(!bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_cuda, 0));
    setenv("BN_CUDA_DISABLE_MOE_ROUTED_FFN", "1", 1);
    assert(bn_backend_runtime_policy_set(
               &routed_cuda.runtime_policy,
               "BN_CUDA_DISABLE_MOE_ROUTED_FFN", "1", 1) == 0);
    assert(!bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_cuda, 1));
    unsetenv("BN_CUDA_DISABLE_MOE_ROUTED_FFN");
    bn_backend_runtime_policy_unset(
        &routed_cuda.runtime_policy, "BN_CUDA_DISABLE_MOE_ROUTED_FFN");
    unsetenv("BN_GPU_DISABLE_ROUTED_MOE_DECODE");
    assert(bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_metal, 1));
    assert(!bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_wgpu, 1));
    assert(bn_gpu_policy_metal_routed_moe_decode_enabled(
        test_current_backend_runtime()));
    setenv("BN_GPU_DISABLE_ROUTED_MOE_DECODE", "1", 1);
    test_gpu_runtime_refresh(&routed_metal);
    assert(!bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_metal, 1));
    assert(!bn_gpu_policy_metal_routed_moe_decode_enabled(
        test_current_backend_runtime()));
    setenv("BN_METAL_ENABLE_CPU_ROUTE_RESIDENT_MOE", "1", 1);
    ...
    assert(bn_backend_runtime_policy_set(
               &routed_metal.runtime_policy,
               "BN_GPU_DISABLE_MOE_ALL_LAYOUT", "1", 1) == 0);
    assert(!bn_gpu_policy_moe_resident_routed_ffn_enabled(
        &routed_metal, 1));
    bn_backend_runtime_policy_free(&routed_cuda.runtime_policy);
    bn_backend_runtime_policy_free(&routed_metal.runtime_policy);
    unsetenv("BN_GPU_DISABLE_MOE_ALL_LAYOUT");
    unsetenv("BN_METAL_ENABLE_CPU_ROUTE_RESIDENT_MOE");
    unsetenv("BN_GPU_DISABLE_ROUTED_MOE_DECODE");
    unsetenv("BN_GPU_DISABLE_MOE_EXPERT_GRAPH");
    assert(bn_gpu_policy_metal_moe_expert_graph_enabled(
        test_current_backend_runtime()));
    setenv("BN_GPU_DISABLE_MOE_EXPERT_GRAPH", "1", 1);
    assert(!bn_gpu_policy_metal_moe_expert_graph_enabled(
        test_current_backend_runtime()));
    unsetenv("BN_GPU_DISABLE_MOE_EXPERT_GRAPH");
    ...
    assert(!bn_gpu_policy_moe_resident_routed_ffn_quant_eligible(
        BN_GGUF_TENSOR_F32, BN_GGUF_TENSOR_F32, BN_GGUF_TENSOR_F32));
    assert(bn_quant_format_supports_moe_routed_midbit_block32_down(
        BN_GGUF_TENSOR_Q5_1));
    assert(!bn_quant_format_supports_moe_routed_midbit_block32_down(
        BN_GGUF_TENSOR_Q8_0));
    ...
    assert(bn_moe_policy_layer_has_router(&moe_route_lw));
    assert(bn_gpu_policy_backend_moe_resident_routed_ffn_eligible(
        &gpu, 1, 1, 0, 0,
        bn_moe_policy_supports_resident_routed_ffn_layout(
            &moe_route_c, &moe_route_lw.moe.expert_map)));
    assert(!bn_gpu_policy_moe_routed_e8m0_supported(
        &gpu, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_Q5_K));
    gpu.caps |= BN_GPU_CAP_MOE_ROUTED_E8M0;
    assert(bn_gpu_policy_moe_routed_e8m0_supported(
        &gpu, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_Q5_K));
    assert(!bn_gpu_policy_moe_routed_e8m0_supported(
        &gpu, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_Q6_K));
    assert(!bn_gpu_policy_moe_routed_e8m0_supported(
        &gpu, BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_Q5_K));
    assert(!bn_gpu_policy_moe_routed_ordered_supported(&gpu, BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_Q6_K));
    gpu.caps |= BN_GPU_CAP_MOE_ROUTED_ORDERED_KQUANT;
    assert(bn_gpu_policy_moe_routed_ordered_supported(&gpu, BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_Q6_K));
    assert(!bn_gpu_policy_moe_routed_ordered_supported(&gpu, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_MXFP4, BN_GGUF_TENSOR_Q6_K));
    gpu.caps &= ~BN_GPU_CAP_MOE_ROUTED_ORDERED_KQUANT;
    gpu.caps &= ~BN_GPU_CAP_MOE_ROUTED_E8M0;
    gpu.caps |= BN_GPU_CAP_MOE_ROUTED_MIDBIT_BLOCK32_DOWN;
    assert(bn_gpu_policy_backend_moe_resident_routed_ffn_eligible(
        &gpu, 0, 0, 1, 0, 1));
    gpu.caps |= BN_GPU_CAP_MOE_ROUTED_MIDBIT_KQUANT_GATEUP;
    assert(bn_gpu_policy_backend_moe_resident_routed_ffn_eligible(
        &gpu, 0, 0, 0, 1, 1));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_all_f16_cache_forced(&gpu));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_gateup_f16_cache_enabled(&gpu, 1));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_partial_moe_f16_cache_enabled(&gpu, 1));
    setenv("BN_CUDA_ENABLE_PARTIAL_MOE_F16_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_partial_moe_f16_cache_enabled(&gpu, 1));
    assert(!bn_gpu_policy_partial_moe_f16_cache_enabled(&gpu, 0));
    unsetenv("BN_CUDA_ENABLE_PARTIAL_MOE_F16_CACHE");

    unsetenv("BN_CUDA_DEBUG_MOE_FIT");
    unsetenv("BN_CUDA_KEEP_INDIVIDUAL_F16_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_residency_fit_debug_enabled(&gpu));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_lazy_aux_cache_enabled(&gpu));
    ...
    assert(bn_gpu_policy_cuda_prefill_batched_gemm_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_gemm_debug_enabled(test_current_backend_runtime()));
    ...
    assert(TEST_CUDA_CUBLAS_GEMM_ALGO(24) == 24);
    setenv("BN_CUDA_CUBLAS_GEMM_ALGO", "-1", 1);
    assert(TEST_CUDA_CUBLAS_GEMM_ALGO(24) == -1);
    setenv("BN_CUDA_CUBLAS_GEMM_ALGO", "0", 1);
    assert(TEST_CUDA_CUBLAS_GEMM_ALGO(24) == 0);
    setenv("BN_CUDA_CUBLAS_GEMM_ALGO", "23", 1);
    assert(TEST_CUDA_CUBLAS_GEMM_ALGO(24) == 23);
    ...
    assert(bn_gpu_policy_cuda_readback_debug_enabled(test_current_backend_runtime()));
    unsetenv("BN_CUDA_DEBUG_READBACK");

    unsetenv("BN_CUDA_DEBUG_CUBLAS_CACHE");
    const BnBackendRuntimePolicy *cuda_cache_policy =
        test_current_backend_runtime();
    assert(!bn_gpu_policy_cuda_cublas_cache_debug_enabled(cuda_cache_policy));
    assert(bn_gpu_policy_cuda_cublas_cache_reserve_mb_or_default(
               cuda_cache_policy, 4096) ==
           4096);
    assert(bn_gpu_policy_cuda_cublas_workspace_mb_or_default(
               cuda_cache_policy, 32) == 32);
    setenv("BN_CUDA_DEBUG_CUBLAS_CACHE", "1", 1);
    setenv("BN_CUDA_CUBLAS_CACHE_RESERVE_MB", "128", 1);
    setenv("BN_CUDA_CUBLAS_WORKSPACE_MB", "64", 1);
    cuda_cache_policy = test_current_backend_runtime();
    assert(bn_gpu_policy_cuda_cublas_cache_debug_enabled(cuda_cache_policy));
    assert(bn_gpu_policy_cuda_cublas_cache_reserve_mb_or_default(
               cuda_cache_policy, 4096) ==
           128);
    assert(bn_gpu_policy_cuda_cublas_workspace_mb_or_default(
               cuda_cache_policy, 32) == 64);
    unsetenv("BN_CUDA_DEBUG_CUBLAS_CACHE");
    unsetenv("BN_CUDA_CUBLAS_CACHE_RESERVE_MB");
    unsetenv("BN_CUDA_CUBLAS_WORKSPACE_MB");

#define bn_gpu_policy_cuda_moe_cublas_gateup_f16_out_enabled() \
    bn_gpu_policy_cuda_moe_cublas_gateup_f16_out_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_cublas_grouped_variable_enabled() \
    bn_gpu_policy_cuda_moe_cublas_grouped_variable_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_cublas_grouped_enabled(...) \
    bn_gpu_policy_cuda_moe_cublas_grouped_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_cublas_gateup_only_enabled(...) \
    bn_gpu_policy_cuda_moe_cublas_gateup_only_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_cublas_all_active_two_fixed_enabled(...) \
    bn_gpu_policy_cuda_moe_cublas_all_active_two_fixed_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_sorted_slots_enabled(...) \
    bn_gpu_policy_cuda_moe_sorted_slots_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_prefill_internal_profile_enabled() \
    bn_gpu_policy_cuda_moe_prefill_internal_profile_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_prefill_profile_every_or_default(...) \
    bn_gpu_policy_cuda_moe_prefill_profile_every_or_default(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_prefill_direct_resid_out_enabled(...) \
    bn_gpu_policy_cuda_moe_prefill_direct_resid_out_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_batch_fused_route_topk_enabled(...) \
    bn_gpu_policy_cuda_moe_batch_fused_route_topk_enabled(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_route_dist_profile_enabled() \
    bn_gpu_policy_cuda_moe_route_dist_profile_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_route_dist_profile_every_or_default(...) \
    bn_gpu_policy_cuda_moe_route_dist_profile_every_or_default(          \
        test_current_backend_runtime(), __VA_ARGS__)
#define bn_gpu_policy_cuda_moe_cublas_grouped_debug_enabled() \
    bn_gpu_policy_cuda_moe_cublas_grouped_debug_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_cublas_gateup_debug_enabled() \
    bn_gpu_policy_cuda_moe_cublas_gateup_debug_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_ffn_batch_enabled() \
    bn_gpu_policy_cuda_moe_ffn_batch_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_ffn_batch_profile_enabled() \
    bn_gpu_policy_cuda_moe_ffn_batch_profile_enabled(       \
        test_current_backend_runtime())
#define bn_gpu_policy_cuda_moe_ffn_batch_profile_every_or_default(...) \
...
#undef bn_gpu_policy_cuda_moe_cublas_gateup_f16_out_enabled
#undef bn_gpu_policy_cuda_moe_cublas_grouped_variable_enabled
#undef bn_gpu_policy_cuda_moe_cublas_grouped_enabled
#undef bn_gpu_policy_cuda_moe_cublas_gateup_only_enabled
#undef bn_gpu_policy_cuda_moe_cublas_all_active_two_fixed_enabled
#undef bn_gpu_policy_cuda_moe_sorted_slots_enabled
#undef bn_gpu_policy_cuda_moe_prefill_internal_profile_enabled
#undef bn_gpu_policy_cuda_moe_prefill_profile_every_or_default
#undef bn_gpu_policy_cuda_moe_prefill_direct_resid_out_enabled
#undef bn_gpu_policy_cuda_moe_batch_fused_route_topk_enabled
#undef bn_gpu_policy_cuda_moe_route_dist_profile_enabled
#undef bn_gpu_policy_cuda_moe_route_dist_profile_every_or_default
#undef bn_gpu_policy_cuda_moe_cublas_grouped_debug_enabled
#undef bn_gpu_policy_cuda_moe_cublas_gateup_debug_enabled
#undef bn_gpu_policy_cuda_moe_ffn_batch_enabled
#undef bn_gpu_policy_cuda_moe_ffn_batch_profile_enabled
#undef bn_gpu_policy_cuda_moe_ffn_batch_profile_every_or_default

    unsetenv("BN_CUDA_ENABLE_DENSE_FFN");
    assert(!bn_gpu_policy_cuda_dense_ffn_enabled(test_current_backend_runtime()));
    setenv("BN_CUDA_ENABLE_DENSE_FFN", "1", 1);
    assert(bn_gpu_policy_cuda_dense_ffn_enabled(test_current_backend_runtime()));
    unsetenv("BN_CUDA_ENABLE_DENSE_FFN");

    unsetenv("BN_CUDA_DISABLE_DENSE_FFN_BATCH");
    assert(bn_gpu_policy_cuda_dense_ffn_batch_enabled(test_current_backend_runtime()));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_cuda_cublas_logits_enabled(&gpu.runtime_policy));
    assert(!bn_gpu_policy_cuda_f32_logits_matvec_enabled(&gpu.runtime_policy));
    assert(!bn_gpu_policy_cuda_f16_logits_matvec_enabled(&gpu.runtime_policy));
    ...
    assert(bn_gpu_policy_backend_kquant_logits_refine_enabled(&gpu, 0));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_logits_refine_top_or_default, 64) == 64);
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_logits_refine_top_or_default, 64) == 11);
    unsetenv("BN_GPU_DISABLE_KQUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_KQUANT_LOGITS_REFINE_TOP");
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_logits_refine_top_or_default, 64) == 13);
    unsetenv("BN_GPU_ENABLE_Q6_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_Q6_LOGITS_REFINE");
    unsetenv("BN_GPU_ENABLE_KQUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_KQUANT_LOGITS_REFINE");
    ...
    assert(bn_gpu_policy_backend_native_quant_logits_refine_enabled(&gpu, 0));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(TEST_POLICY_CALL(bn_gpu_policy_native_quant_logits_refine_top_or_default, 16) == 16);
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_native_quant_logits_refine_top_or_default, 16) == 5);
    unsetenv("BN_GPU_DISABLE_NATIVE_QUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_NATIVE_QUANT_LOGITS_REFINE_TOP");
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_native_quant_logits_refine_top_or_default, 16) == 7);
    unsetenv("BN_GPU_ENABLE_Q8_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_Q8_LOGITS_REFINE");
    unsetenv("BN_GPU_ENABLE_NATIVE_QUANT_LOGITS_REFINE");
    unsetenv("BN_GPU_DISABLE_NATIVE_QUANT_LOGITS_REFINE");
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_moe_down_kquant_f32_cache_forced));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_moe_down_kquant_f32_cache_default_for_cols,
        1024));
    assert(TEST_POLICY_CALL(bn_gpu_policy_moe_down_kquant_f32_cache_default_for_cols, 1025));
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_down_kquant_f32_cache_forced));
    assert(TEST_POLICY_CALL(bn_gpu_policy_moe_down_kquant_f32_cache_default_for_cols, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_moe_down_kquant_f32_cache_requires_full_buffer,
        BN_GGUF_TENSOR_Q6_K));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_moe_down_kquant_f32_cache_requires_full_buffer,
        BN_GGUF_TENSOR_Q4_K));
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_KQUANT_F32_CACHE");
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_layout_reserve_bytes) ==
           512u * 1024u * 1024u);
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_full_reserve_bytes) ==
           512u * 1024u * 1024u);
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_full_reserve_bytes) == SIZE_MAX);
    unsetenv("BN_CUDA_LAYOUT_RESERVE_MB");
    unsetenv("BN_CUDA_MOE_FULL_RESERVE_MB");

    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE");
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_SMALL_KQUANT_F32_CACHE");
    unsetenv("BN_CUDA_ENABLE_Q4K_MOE_DOWN_F32_CACHE");
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE");
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_KQUANT_F32_CACHE");
    unsetenv("BN_CUDA_DISABLE_Q4K_MOE_DOWN_F32_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_down_small_expert_f32_cache_enabled(
        &gpu, BN_GGUF_TENSOR_Q4_K));
    setenv("BN_CUDA_ENABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_down_small_expert_f32_cache_enabled(
        &gpu, BN_GGUF_TENSOR_Q4_K));
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE");
    ...
    assert(bn_gpu_policy_moe_down_small_expert_f32_cache_enabled(
        &gpu, BN_GGUF_TENSOR_Q4_K));
    setenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_down_small_expert_f32_cache_enabled(
        &gpu, BN_GGUF_TENSOR_Q4_K));
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE");
    ...
    unsetenv("BN_CUDA_DISABLE_LEGACY_5BIT_MATVEC");
    ...
    assert(!bn_gpu_policy_matvec_disabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q8_0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q5_0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q4_K));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q5_K));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q6_K));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q8_K));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_disabled, BN_GGUF_TENSOR_Q4_0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q8_0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q5_0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q4_K));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q5_K));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q6_K));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q8_K));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_Q4_0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_IQ4_NL));
    assert(TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_IQ3_S));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_matvec_type_supported, BN_GGUF_TENSOR_I2_S));
    ...
    setenv("BN_CUDA_DISABLE_LEGACY_5BIT_MATVEC", "1", 1);
    ...
    assert(bn_gpu_policy_matmul_batch_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_matvec_batch_enabled(test_current_backend_runtime()));
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_small_state_native_quant_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_small_state_native_quant_enabled, 1));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_state_native_quant_disabled));
    ...
    assert(!bn_gpu_policy_prefill_matmul_disabled(&gpu));
    assert(!bn_gpu_policy_prefill_matmul_enabled(&gpu));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_prefill_direct_kv_disabled(&gpu));
    assert(!bn_gpu_policy_prefill_direct_kv_with_cpu_fallback_enabled(&gpu));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_cpu_decode_fallback_requested(&gpu));
    assert(bn_gpu_policy_cpu_fallback_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_fallback_from_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_attention_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_attention_from_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_ffn_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_ffn_from_layer_or_default(&gpu, -1) == -1);
    assert(bn_gpu_policy_cpu_ffn_down_from_layer_or_default(&gpu, -1) == -1);
    setenv("BN_GPU_CPU_FALLBACK_FROM_LAYER", "0", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_cpu_decode_fallback_requested(&gpu));
    assert(bn_gpu_policy_cpu_fallback_from_layer_or_default(&gpu, -1) == 0);
    unsetenv("BN_GPU_CPU_FALLBACK_FROM_LAYER");
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_cpu_ffn_layer_or_default(&gpu, -1) == 2);
    assert(bn_gpu_policy_cpu_ffn_from_layer_or_default(&gpu, -1) == 3);
    assert(bn_gpu_policy_cpu_ffn_down_from_layer_or_default(&gpu, -1) == 4);
    unsetenv("BN_GPU_CPU_FFN_LAYER");
    unsetenv("BN_GPU_CPU_FFN_FROM_LAYER");
    unsetenv("BN_GPU_CPU_FFN_DOWN_FROM_LAYER");
    ...
    unsetenv("BN_METAL_ENABLE_REFERENCE_ATTENTION");
    BnBackendRuntimePolicy attention_policy = {0};
    assert(!bn_gpu_policy_metal_reference_attention_enabled(
        &attention_policy));
    setenv("BN_METAL_ENABLE_REFERENCE_ATTENTION", "1", 1);
    assert(bn_backend_runtime_policy_set(
               &attention_policy, "BN_METAL_ENABLE_REFERENCE_ATTENTION",
               "1", 1) == 0);
    assert(bn_gpu_policy_metal_reference_attention_enabled(
        &attention_policy));
    bn_backend_runtime_policy_unset(
        &attention_policy, "BN_METAL_ENABLE_REFERENCE_ATTENTION");
    unsetenv("BN_METAL_ENABLE_REFERENCE_ATTENTION");
    unsetenv("BN_METAL_DISABLE_PREPARED_NATIVE_QUANT_ATTENTION");
    assert(bn_gpu_policy_metal_prepared_native_quant_attention_enabled(
        &attention_policy));
    setenv("BN_METAL_DISABLE_PREPARED_NATIVE_QUANT_ATTENTION", "1", 1);
    assert(bn_backend_runtime_policy_set(
               &attention_policy,
               "BN_METAL_DISABLE_PREPARED_NATIVE_QUANT_ATTENTION",
               "1", 1) == 0);
    assert(!bn_gpu_policy_metal_prepared_native_quant_attention_enabled(
        &attention_policy));
    bn_backend_runtime_policy_unset(
        ...
        "BN_METAL_DISABLE_PREPARED_NATIVE_QUANT_ATTENTION");
    unsetenv("BN_METAL_DISABLE_PREPARED_NATIVE_QUANT_ATTENTION");
    unsetenv("BN_METAL_REFERENCE_ATTENTION_STAGES");
    assert(bn_gpu_policy_metal_reference_attention_stage_mask(
               &attention_policy) == 7u);
    setenv("BN_METAL_REFERENCE_ATTENTION_STAGES", "5", 1);
    assert(bn_backend_runtime_policy_set(
               &attention_policy, "BN_METAL_REFERENCE_ATTENTION_STAGES",
               "5", 1) == 0);
    assert(bn_gpu_policy_metal_reference_attention_stage_mask(
               &attention_policy) == 5u);
    setenv("BN_METAL_REFERENCE_ATTENTION_STAGES", "15", 1);
    assert(bn_backend_runtime_policy_set(
               &attention_policy, "BN_METAL_REFERENCE_ATTENTION_STAGES",
               "15", 1) == 0);
    assert(bn_gpu_policy_metal_reference_attention_stage_mask(
               &attention_policy) == 7u);
    bn_backend_runtime_policy_free(&attention_policy);
    unsetenv("BN_METAL_REFERENCE_ATTENTION_STAGES");
    assert(!bn_gpu_policy_ssm_graph_disabled(&gpu));
    ...
    unsetenv("BN_CUDA_DISABLE_LEGACY_BLOCK_GATEUP_WARP");
    ...
    assert(!bn_gpu_policy_cuda_qkv_mixed_fuse_disabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_qkv_key_cache_fuse_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_qkv_kpair_opt_enabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_legacy_block_gateup_warp_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_gateup_warp_disabled));
    assert(!bn_gpu_policy_cuda_graph_exec_requested(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_moe_graph_max_experts_or_default(test_current_backend_runtime(), 128) == 128);
    assert(bn_gpu_policy_cuda_decode_graph_default_enabled(test_current_backend_runtime(), 0, 0));
    assert(!bn_gpu_policy_cuda_decode_graph_default_enabled(test_current_backend_runtime(), 1, 0));
    assert(bn_gpu_policy_cuda_decode_graph_default_enabled(test_current_backend_runtime(), 1, 1));
    ...
    setenv("BN_CUDA_DISABLE_LEGACY_BLOCK_GATEUP_WARP", "1", 1);
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_gateup_warp_disabled));
    assert(bn_gpu_policy_cuda_graph_exec_requested(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_moe_graph_max_experts_or_default(test_current_backend_runtime(), 128) == 7);
    unsetenv("BN_CUDA_ENABLE_GRAPH_EXEC");
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_max_storage_binding_bytes, 0) ==
           128u * 1024u * 1024u);
    assert(TEST_POLICY_CALL(bn_gpu_policy_max_storage_binding_bytes, 7u * 1024u * 1024u) ==
           7u * 1024u * 1024u);
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_max_storage_binding_bytes, 7u * 1024u * 1024u) == 0);
    unsetenv("BN_GPU_MAX_STORAGE_BINDING_MB");

    unsetenv("BN_CUDA_DISABLE_CUBLAS_MATMUL");
    unsetenv("BN_CUDA_ENABLE_EAGER_KQUANT_CACHE");
    ...
    assert(TEST_CUDA_CUBLAS_MATMUL_ENABLED());
    assert(!TEST_POLICY_CALL0(
        bn_gpu_policy_cuda_eager_kquant_aux_cache_enabled));
    setenv("BN_CUDA_ENABLE_EAGER_KQUANT_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(TEST_POLICY_CALL0(
        bn_gpu_policy_cuda_eager_kquant_aux_cache_enabled));
    unsetenv("BN_CUDA_ENABLE_EAGER_KQUANT_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_cublas_f16_cache_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_matmul_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_f16_native_quant_matmul_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_split_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_split_default_allowed));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_all_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_default_allowed));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_logits_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_native_quant_prepared_input_logits_default_enabled, 0));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_prepared_kquant_input_cache_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_all_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_prepared_input_logits_disabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_native_quant_prepared_input_logits_default_enabled, 1));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_prepared_kquant_input_cache_enabled));
    unsetenv("BN_CUDA_ENABLE_NATIVE_QUANT_MATMUL");
    unsetenv("BN_CUDA_DISABLE_NATIVE_QUANT_MATMUL");
    unsetenv("BN_CUDA_DISABLE_F16_NATIVE_QUANT_MATMUL");
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_quant_matmul_preferred_for_type,
        BN_GGUF_TENSOR_Q8_0, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_quant_matmul_preferred_for_type,
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_4warp_long_enabled, 1536, 8960));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_4warp_long_enabled, 2560, 8192));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_4warp_long_enabled, 2560, 6144));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_4warp_long_enabled, 2560, 8192));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled, 1536, 8960));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled, 2560, 9728));
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_5WARP_1536_8960", "1", 1);
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_5WARP_2560_9728", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled, 1536, 8960));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_5warp_shape_enabled, 2560, 9728));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled, 1536, 8960));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled, 2560, 9728));
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_3WARP_1536_8960", "1", 1);
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_3WARP_2560_9728", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled, 1536, 8960));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_3warp_shape_enabled, 2560, 9728));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_2warp_long_enabled, 2560, 8192));
    setenv("BN_CUDA_ENABLE_DOWN_KQUANT_2WARP_LONG", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_2warp_long_enabled, 2560, 8192));
    setenv("BN_CUDA_DISABLE_DOWN_KQUANT_2WARP_LONG", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_2warp_long_enabled, 2560, 8192));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_matvec4_shape_disabled, 1024, 2560));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_matvec4_shape_disabled, 512, 2048));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_matvec4_shape_disabled, 768, 2048));
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_KQUANT_F32_CACHE");
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_CACHE");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_quant_path_preferred,
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_quant_path_preferred,
        1, BN_GGUF_TENSOR_Q6_K, 1024, 4, 2));
    unsetenv("BN_CUDA_ENABLE_Q6K_MOE_DOWN_F32_CACHE");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_path_enabled,
    ...
    assert(bn_gpu_policy_cuda_moe_down_4row_enabled(test_current_backend_runtime(), 1024));
    assert(!bn_gpu_policy_cuda_moe_down_4row_enabled(test_current_backend_runtime(), 1025));
    assert(bn_gpu_policy_cuda_moe_down_8row_enabled(test_current_backend_runtime(), 1024));
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_4ROW");
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_8ROW");
    assert(!bn_gpu_policy_cuda_moe_down_halfwarp_enabled(test_current_backend_runtime(),
    ...
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_HALFWARP");
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_HALFWARP");
    assert(!bn_gpu_policy_cuda_moe_down_split4_enabled(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q6_K, 1, 4, 2));
    assert(!bn_gpu_policy_cuda_moe_down_split4_enabled(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q6_K, 0, 4, 2));
    ...
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_SPLIT4");
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SPLIT4");
    assert(bn_gpu_policy_cuda_moe_down_scatter_enabled(test_current_backend_runtime(),
    ...
    assert(!bn_gpu_policy_cuda_moe_down_scatter_enabled(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q6_K, 0, 0));
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SCATTER");
    assert(!bn_gpu_policy_cuda_moe_down_scatter_16row_enabled(test_current_backend_runtime(), 1, 1024));
    setenv("BN_CUDA_ENABLE_MOE_DOWN_SCATTER_16ROW", "1", 1);
    assert(bn_gpu_policy_cuda_moe_down_scatter_16row_enabled(test_current_backend_runtime(), 1, 768));
    assert(!bn_gpu_policy_cuda_moe_down_scatter_16row_enabled(test_current_backend_runtime(), 0, 768));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_float_path_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_float_path_enabled));
    unsetenv("BN_CUDA_DISABLE_Q6K_FLOAT_MOE_DOWN");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_pair_path_enabled, 0, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_pair_path_enabled, 1, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_pair_path_enabled, 0, 1, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_pair_path_enabled, 0, 0, 1));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_pair_path_enabled, 0, 0, 0));
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_PAIR_DOWN");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 4096, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 4096, 1, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 0, 4096, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 2048, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 4096, 1, 0));
    setenv("BN_CUDA_DISABLE_MOE_DOWN_KQUANT_F32_CACHE", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 4096, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_enabled, 1, 0));
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_KQUANT_F32_CACHE");
    setenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_CACHE", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prefers_f32_cache, 1, 4096, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_enabled, 1, 0));
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_CACHE");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_pair2_enabled, 2, 2));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_pair2_enabled, 4, 2));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_pair2_enabled, 2, 2));
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_PAIR2");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_f32_pair2_4row_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_f32_pair2_4row_enabled));
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_4ROW");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_accum_enabled, 1));
    setenv("BN_CUDA_ENABLE_ALL_ACTIVE_TWO_KQUANT_MOE_DOWN_ACCUM", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_accum_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_accum_enabled, 0));
    ...
    unsetenv("BN_CUDA_ENABLE_MOE_Q6K_ALL2_ACCUM");
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_ALL2_ACCUM");
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair4_sum_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair4_sum_enabled, 0));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair4_sum_enabled, 1));
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_PAIR4_SUM");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_4row_sum_enabled,
        0, 8, 1024));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_4row_sum_enabled,
        1, 8, 1024));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_4row_sum_enabled,
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_4row_sum_enabled,
        0, 8, 1024));
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_K8_4ROW_SUM");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_8row_sum_enabled,
        1, 1024));
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_8row_sum_enabled,
        1, 1024));
    unsetenv("BN_CUDA_ENABLE_MOE_Q6K_K8_8ROW_SUM");
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_fixed_enabled, 1));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_fixed_enabled, 1));
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_ALL2_FIXED");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_resid_rmsnorm_fuse_enabled));
    setenv("BN_CUDA_DISABLE_MOE_DOWN_RESID_RMSNORM_FUSE", "1", 1);
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_resid_rmsnorm_fuse_enabled));
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_RESID_RMSNORM_FUSE");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_shape_2048_768_enabled,
        2048, 768, 8));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_shape_2048_768_enabled,
        2048, 1024, 8));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_native_quant_shape_2048_768_enabled,
        2048, 768, 8));
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_K8_EXACT_2048_768");
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_accum_4row_enabled));
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_accum_4row_enabled));
    unsetenv("BN_CUDA_ENABLE_MOE_Q6K_ALL2_ACCUM_4ROW");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_down_prepared_pair_4row_enabled));
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_Q6K_PAIR_DOWN_4ROW");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_enabled, 1, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_enabled, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f32_cache_enabled, 1, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_f16_cache_enabled, 1));
    ...
    unsetenv("BN_CUDA_ENABLE_Q6K_MOE_DOWN_F16_CACHE");
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F16_CACHE");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_aux_f32_cache_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_aux_f32_cache_enabled, 0));
    setenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_aux_f32_cache_enabled, 1));
    unsetenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_EXPERT_F32_CACHE");
    setenv("BN_CUDA_DISABLE_MOE_DOWN_SMALL_KQUANT_F32_CACHE", "1", 1);
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_aux_f32_cache_enabled, 1));
    unsetenv("BN_CUDA_DISABLE_Q4K_MOE_DOWN_F32_CACHE");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_pair8_enabled, 2, 2, 4096));
    setenv("BN_CUDA_ENABLE_MOE_ASYMMETRIC_KQUANT_PAIR_DOWN", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_pair8_enabled, 2, 2, 4096));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_pair8_enabled, 4, 2, 4096));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_pair8_enabled, 2, 2, 2048));
    ...
    unsetenv("BN_CUDA_ENABLE_MOE_Q4K_PAIR_DOWN");
    unsetenv("BN_CUDA_DISABLE_MOE_Q4K_PAIR_DOWN");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_8row_enabled, 1024));
    setenv("BN_CUDA_ENABLE_MOE_ASYMMETRIC_KQUANT_DOWN_8ROW", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_8row_enabled, 1024));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_prepared_8row_enabled, 1025));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 1, 4096, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 2, 2048, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 2, 2048, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 2, 4096, 1));
    setenv("BN_CUDA_ENABLE_MOE_ASYMMETRIC_KQUANT_GATEUP_DOT", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 2, 4096, 0));
    setenv("BN_CUDA_DISABLE_Q4K_Q8K_DOT", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 1, 2048, 1));
    unsetenv("BN_CUDA_DISABLE_Q4K_Q8K_DOT");
    unsetenv("BN_CUDA_ENABLE_MOE_ASYMMETRIC_KQUANT_GATEUP_DOT");
    setenv("BN_CUDA_ENABLE_Q4K_Q8K_MOE_GATEUP", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_dot_enabled, 2, 4096, 0));
    unsetenv("BN_CUDA_ENABLE_Q4K_Q8K_MOE_GATEUP");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_8row_enabled, 2048));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_8row_enabled, 2049));
    setenv("BN_CUDA_DISABLE_MOE_GATEUP_8ROW", "1", 1);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_8row_enabled, 2048));
    unsetenv("BN_CUDA_DISABLE_MOE_GATEUP_8ROW");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_split_enabled, 2048, 2));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_split_enabled, 2049, 4));
    setenv("BN_CUDA_ENABLE_MOE_GATEUP_SPLIT", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_prepared_split_enabled, 2048, 4));
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_block_prepared_batch_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_block_prepared_batch_enabled, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_block_2row_enabled, 1024));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_gateup_block_2row_enabled, 1025));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_block_4row_enabled, 1024));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_down_block_2row_enabled, 1024));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_decode_logits_cache_enabled(&gpu, 0));
    setenv("BN_CUDA_ENABLE_LOGITS_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_decode_logits_cache_enabled(&gpu, 0));
    assert(!bn_gpu_policy_decode_logits_cache_enabled(&gpu, 1));
    unsetenv("BN_CUDA_ENABLE_MOE_DECODE_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_decode_cache_enabled(&gpu));
    setenv("BN_CUDA_ENABLE_MOE_DECODE_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_decode_cache_enabled(&gpu));
    unsetenv("BN_CUDA_DISABLE_MOE_DECODE_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_decode_cache_disabled(&gpu));
    setenv("BN_CUDA_DISABLE_MOE_DECODE_CACHE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_decode_cache_disabled(&gpu));
    unsetenv("BN_CUDA_DISABLE_DECODE_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_decode_cache_disabled(&gpu));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_native_quant_decode_cache_disabled));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_logits_argmax_disabled(&gpu));
    setenv("BN_CUDA_DISABLE_LOGITS_ARGMAX", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_logits_argmax_disabled(&gpu));
    unsetenv("BN_CUDA_ENABLE_DENSE_LOGITS_ARGMAX");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_dense_logits_argmax_enabled(&gpu));
    setenv("BN_CUDA_ENABLE_DENSE_LOGITS_ARGMAX", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_dense_logits_argmax_enabled(&gpu));
    unsetenv("BN_CUDA_ENABLE_MOE_LOGITS_MMVQ_ARGMAX");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_logits_mmvq_argmax_enabled(&gpu));
    setenv("BN_CUDA_ENABLE_MOE_LOGITS_MMVQ_ARGMAX", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_logits_mmvq_argmax_enabled(&gpu));
    unsetenv("BN_CUDA_DISABLE_MOE_LOGITS_MMVQ_ARGMAX");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_logits_mmvq_argmax_disabled(&gpu));
    setenv("BN_CUDA_DISABLE_MOE_LOGITS_MMVQ_ARGMAX", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_logits_mmvq_argmax_disabled(&gpu));
    unsetenv("BN_CUDA_ENABLE_MOE_LOGITS_MMVQ_ARGMAX");
    unsetenv("BN_CUDA_DISABLE_MOE_LOGITS_MMVQ_ARGMAX");
    assert(bn_gpu_policy_cuda_moe_logits_mmvq_argmax_path_enabled(test_current_backend_runtime(), 50000,
                                                                  1536));
    assert(!bn_gpu_policy_cuda_moe_logits_mmvq_argmax_path_enabled(test_current_backend_runtime(), 49999,
                                                                   1536));
    assert(!bn_gpu_policy_cuda_moe_logits_mmvq_argmax_path_enabled(test_current_backend_runtime(), 50000,
                                                                   2048));
    setenv("BN_CUDA_ENABLE_MOE_LOGITS_MMVQ_ARGMAX", "1", 1);
    ...
    assert(bn_gpu_policy_cuda_moe_logits_mmvq_1warp8_1536_enabled(test_current_backend_runtime(), 1, 151936,
                                                                  1536));
    assert(!bn_gpu_policy_cuda_moe_logits_mmvq_1warp8_1536_enabled(test_current_backend_runtime(), 1, 151936,
                                                                   2048));
    ...
    assert(!bn_gpu_policy_cuda_moe_logits_mmvq_1warp16_1536_enabled(test_current_backend_runtime(), 1));
    setenv("BN_CUDA_ENABLE_MOE_LOGITS_MMVQ_1WARP16_1536", "1", 1);
    assert(bn_gpu_policy_cuda_moe_logits_mmvq_1warp16_1536_enabled(test_current_backend_runtime(), 1));
    unsetenv("BN_CUDA_DISABLE_MOE_LOGITS_MMVQ_1WARP8_1536_UNROLL");
    assert(bn_gpu_policy_cuda_moe_logits_mmvq_1warp8_1536_unroll_enabled(test_current_backend_runtime(), 1,
                                                                         0));
    assert(!bn_gpu_policy_cuda_moe_logits_mmvq_1warp8_1536_unroll_enabled(test_current_backend_runtime(), 1,
                                                                          1));
    ...
    assert(bn_gpu_policy_cuda_argmax_fast_enabled(test_current_backend_runtime()));
    setenv("BN_CUDA_DISABLE_ARGMAX_FAST", "1", 1);
    assert(!bn_gpu_policy_cuda_argmax_fast_enabled(test_current_backend_runtime()));
    unsetenv("BN_CUDA_ENABLE_OPTIMISTIC_ARGMAX_PENALTY");
    assert(!bn_gpu_policy_cuda_optimistic_argmax_penalty_enabled(test_current_backend_runtime()));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_legacy_block_matvec4_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_legacy_block_warp_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_deinterleaved_kquant_pair_matvec_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, 8192));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, 21504));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, 65536));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, -256));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_4warp_enabled, 8193));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_split_4warp_enabled, 8192));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_deinterleaved_kquant_gateup_2warp_enabled));
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_split_4warp_enabled, 8192));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_deinterleaved_kquant_split_4warp_enabled, 21504));
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_symmetric_kquant_dot_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_deinterleaved_kquant_dot_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_4warp_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_out_residual_rmsnorm_fuse_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_split_k_rope_cache_fuse_enabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_qwarp4_enabled, 4096));
    unsetenv("BN_CUDA_DISABLE_Q4K_4WARP");
    unsetenv("BN_CUDA_ENABLE_Q4K_OUT_RESID_RMSNORM_FUSE");
    unsetenv("BN_CUDA_ENABLE_Q4K_SPLIT_K_ROPE_CACHE_FUSE");
    unsetenv("BN_CUDA_DISABLE_Q4K_GATEUP_QWARP4");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_4warp_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 1536, 8960));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 2560, 9728));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 3072, 9728));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 5376, 16384));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 5376, 21504));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 37, 65536));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 0, 8192));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 37, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_4warp_shape_enabled, 37, 8193));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_out_residual_rmsnorm_fuse_enabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_qkv_mixed_fuse_enabled,
        BN_GGUF_TENSOR_Q4_K));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_qkv_mixed_fuse_enabled,
        BN_GGUF_TENSOR_Q5_K));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_split_k_rope_cache_fuse_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_split_qk_rope_cache_fuse_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 2048));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 2560));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 5376));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 65536));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 5377));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_5warp_enabled, 2560));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_5warp_enabled, 2048));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_rows, 4608, 2048) == 512);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_rows, 2304, 2048) == 256);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_rows, 1792, 1536) == 0);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_fuse_enabled, 256));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_fuse_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_kquant_gateup_prepared_path_enabled, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_gateup_prepared_path_enabled, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_qwarp4_enabled, 4096));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_qwarp4_enabled, 4097));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_5warp_enabled, 1, 2560));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_5warp_enabled, 0, 2560));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_2warp_enabled, 1, 5120));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_2warp_enabled, 1, 5121));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_warp_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_ssm_matvec_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_native_quant_ssm_prepared_input_enabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_native_quant_mixed_prepared_input_enabled,
        BN_GGUF_TENSOR_Q8_0, BN_GGUF_TENSOR_Q4_K, 2048));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_route_dot_prepared_input_enabled, 2048, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_route_dot_prepared_input_enabled, 2049, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_route_dot_prepared_input_enabled, 2048, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_route_block_prepared_input_enabled, 2048, 1, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_route_block_prepared_input_enabled, 2048, 1, 1));
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_ROUTE_DOT_PREPARED_INPUT");
    unsetenv("BN_CUDA_ENABLE_MOE_ROUTE_BLOCK_PREPARED_INPUT");
    assert(!bn_gpu_policy_cuda_moe_router_fused_topk_enabled(test_current_backend_runtime(), 256, 0));
    assert(!bn_gpu_policy_cuda_moe_router_warp_disabled(test_current_backend_runtime(), 0));
    assert(bn_gpu_policy_cuda_moe_router_warp_disabled(test_current_backend_runtime(), 1));
    assert(bn_gpu_policy_cuda_moe_router_4warp_enabled(test_current_backend_runtime(), 2048));
    assert(!bn_gpu_policy_cuda_moe_router_4warp_enabled(test_current_backend_runtime(), 2047));
    assert(bn_gpu_policy_cuda_moe_router_2warp_enabled(test_current_backend_runtime(), 2048));
    assert(!bn_gpu_policy_cuda_moe_router_2warp_enabled(test_current_backend_runtime(), 2047));
    assert(bn_gpu_policy_cuda_moe_router_warp_topk_enabled(test_current_backend_runtime(), 256));
    assert(!bn_gpu_policy_cuda_moe_router_warp_topk_enabled(test_current_backend_runtime(), 257));
    assert(bn_gpu_policy_cuda_moe_router_warp_topk_enabled(test_current_backend_runtime(), 512));
    assert(!bn_gpu_policy_cuda_moe_router_warp_topk_enabled(test_current_backend_runtime(), 513));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_block_prepared_decode_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fast_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fast_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_prepared_dot_enabled, 0, 0, 0, 2048,
                                                       2048));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_prepared_dot_enabled, 0, 0, 0, 2049,
                                                      2049));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_internal_profile_enabled, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fixed_prepared_4row_enabled, 1, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fixed_prepared_4row_enabled, 0, 1));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_gateup_prepared_4row_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_f16_native_quant_ssm_matvec_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_f16_native_quant_matvec_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_f16_packed_kquant_matvec_enabled));
    ...
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_4warp_enabled, 5376));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_5warp_enabled, 2560));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_rows, 1792, 1536) == 256);
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_split_value_fuse_enabled, 256));
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_gateup_prepared_path_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_qwarp4_enabled, 4096));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_gateup_5warp_enabled, 1, 2560));
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_ROUTER_FUSED_TOPK");
    assert(bn_gpu_policy_cuda_moe_router_fused_topk_enabled(test_current_backend_runtime(), 256, 0));
    assert(!bn_gpu_policy_cuda_moe_router_fused_topk_enabled(test_current_backend_runtime(), 256, 1));
    assert(bn_gpu_policy_cuda_moe_router_warp_disabled(test_current_backend_runtime(), 0));
    assert(!bn_gpu_policy_cuda_moe_router_4warp_enabled(test_current_backend_runtime(), 2048));
    assert(!bn_gpu_policy_cuda_moe_router_2warp_enabled(test_current_backend_runtime(), 2048));
    assert(!bn_gpu_policy_cuda_moe_router_warp_topk_enabled(test_current_backend_runtime(), 256));
    ...
    unsetenv("BN_CUDA_DISABLE_Q8_MOE_Q8X");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fast_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_prepared_dot_enabled, 1, 1, 1, 2048,
                                                       2048));
    unsetenv("BN_CUDA_DISABLE_MOE_ASYMMETRIC_KQUANT_DOT");
    setenv("BN_CUDA_ENABLE_ALL_ACTIVE_TWO_MOE_ASYMMETRIC_KQUANT_DOT_INPUT", "1", 1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_prepared_dot_enabled, 0, 0, 1, 2048,
                                                      2048));
    unsetenv("BN_CUDA_ENABLE_ALL_ACTIVE_TWO_MOE_ASYMMETRIC_KQUANT_DOT_INPUT");
    ...
    unsetenv("BN_CUDA_DISABLE_MOE_Q4K_Q8K_DOT");
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_internal_profile_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_internal_profile_enabled, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fixed_prepared_4row_enabled, 1, 1));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_moe_gateup_prepared_4row_disabled));
    unsetenv("BN_CUDA_DISABLE_ALL_ACTIVE_TWO_MOE_ASYMMETRIC_KQUANT_FIXED");
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_moe_all_active_two_fixed_prepared_4row_enabled, 1, 1));
    ...
    unsetenv("BN_CUDA_DISABLE_ASYMMETRIC_KQUANT_MATMUL8");
    unsetenv("BN_CUDA_DISABLE_Q4K_MATMUL8");
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_kquant_dot_enabled));
    assert(!TEST_POLICY_CALL0(
        bn_gpu_policy_cuda_asymmetric_kquant_batch_q8k_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_kquant_dot_forced));
    setenv("BN_CUDA_ENABLE_ASYMMETRIC_KQUANT_NATIVE_DOT", "1", 1);
    assert(TEST_POLICY_CALL0(bn_gpu_policy_kquant_dot_enabled));
    assert(TEST_POLICY_CALL0(
        bn_gpu_policy_cuda_asymmetric_kquant_batch_q8k_enabled));
    unsetenv("BN_CUDA_ENABLE_ASYMMETRIC_KQUANT_NATIVE_DOT");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_symmetric_kquant_pair_matvec_enabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_kquant_matvec4_enabled, 8192));
    assert(TEST_POLICY_CALL(bn_gpu_policy_kquant_matvec4_enabled, 16384));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_matmul8_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_sharedx_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_batch_sharedx_enabled));
    setenv("BN_CUDA_DISABLE_Q4K_Q8K_DOT", "1", 1);
    ...
    unsetenv("BN_CUDA_DISABLE_Q4K_PAIR_MATVEC");
    unsetenv("BN_CUDA_ENABLE_Q4K_MATMUL8");
    setenv("BN_CUDA_DISABLE_Q4K_MATMUL8", "1", 1);
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_matmul8_enabled));
    setenv("BN_CUDA_ENABLE_Q4K_MATMUL8", "1", 1);
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_asymmetric_kquant_matmul8_enabled));
    unsetenv("BN_CUDA_ENABLE_Q4K_MATMUL8");
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_dot_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_dot_forced));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_warp_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_pair_matvec_enabled, 4096));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_pair_matvec_enabled, 5120));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_prepared_dot_enabled, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 5120, 4096, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 1024, 2560, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 1024, 2560, 0, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 0, 2560, 0, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 1024, 2559, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 512, 2048, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 1536, 8960, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 2560, 9728, 0, 0));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 50000, 1536, 1, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_enabled, 5120, 4096, 0, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_2warp_logits_enabled, 50000, 1536, 1));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_2warp_logits_enabled, 50000, 2048, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_mmvq_2warp_logits_enabled, 50000, 1536, 0));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_residual_rmsnorm_fuse_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_f16_down_kquant_matvec_enabled, 2048, 8192, 0));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_f16_down_kquant_matvec_enabled, 2048, 8192, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_f16_down_kquant_matvec_enabled, 2048, 4096, 0));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_matmul8_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_matmul4_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_matvec4_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_batch_warp_enabled));
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_asymmetric_kquant_pair_matvec_enabled, 5120));
    assert(TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_prepared_dot_enabled, 1));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_cuda_down_kquant_prepared_dot_enabled, 0));
    ...
    const BnBackendRuntimePolicy *cuda_fuse_policy =
        test_current_backend_runtime();
    assert(bn_gpu_policy_cuda_fuse_bias_enabled(cuda_fuse_policy));
    assert(bn_gpu_policy_cuda_rope_flash_fuse_enabled(cuda_fuse_policy));
    assert(!bn_gpu_policy_cuda_bias_rope_flash_fuse_enabled(cuda_fuse_policy));
    assert(bn_gpu_policy_cuda_qk_norm_rope_flash_fuse_enabled(cuda_fuse_policy));
    assert(bn_gpu_policy_cuda_qk_norm_rope_fuse_enabled(cuda_fuse_policy));
    ...
    cuda_fuse_policy = test_current_backend_runtime();
    assert(!bn_gpu_policy_cuda_fuse_bias_enabled(cuda_fuse_policy));
    assert(!bn_gpu_policy_cuda_rope_flash_fuse_enabled(cuda_fuse_policy));
    assert(bn_gpu_policy_cuda_bias_rope_flash_fuse_enabled(cuda_fuse_policy));
    assert(!bn_gpu_policy_cuda_qk_norm_rope_flash_fuse_enabled(cuda_fuse_policy));
    ...
    const BnBackendRuntimePolicy *cuda_debug_policy =
        test_current_backend_runtime();
    assert(!bn_gpu_policy_cuda_nan_verbose_debug_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_stream_exec_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_profile_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_profile_every_or_default(
               cuda_debug_policy, 1) == 1);
    assert(!bn_gpu_policy_cuda_wall_profile_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_wall_profile_detail_limit_or_default(
               cuda_debug_policy, 0) == 0);
    assert(bn_gpu_policy_cuda_wall_profile_every_or_default(
               cuda_debug_policy, 16) == 16);
    assert(!bn_gpu_policy_cuda_profile_shapes_enabled(cuda_debug_policy));
    BnBackendRuntimePolicy empty_backend_policy = {0};
    assert(bn_gpu_policy_cuda_device_selector(&empty_backend_policy) == NULL);
    assert(!bn_gpu_policy_cuda_exec_fail_debug_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_sync_each_op_debug_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_nan_debug_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_dump_ops_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_dump_ops_every_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_dump_ops_limit_or_default(
               cuda_debug_policy, 256) == 256);
    ...
    cuda_debug_policy = test_current_backend_runtime();
    assert(bn_gpu_policy_cuda_nan_verbose_debug_enabled(cuda_debug_policy));
    assert(!bn_gpu_policy_cuda_stream_exec_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_profile_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_profile_every_or_default(
               cuda_debug_policy, 1) == 7);
    ...
    assert(bn_gpu_policy_cuda_wall_profile_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_wall_profile_detail_limit_or_default(
               cuda_debug_policy, 0) == 3);
    assert(bn_gpu_policy_cuda_wall_profile_every_or_default(
               cuda_debug_policy, 16) == 11);
    setenv("BN_CUDA_PROFILE_WALL_EVERY", "-4", 1);
    cuda_debug_policy = test_current_backend_runtime();
    assert(bn_gpu_policy_cuda_wall_profile_every_or_default(
               cuda_debug_policy, 16) == 16);
    assert(bn_gpu_policy_cuda_profile_shapes_enabled(cuda_debug_policy));
    char cuda_device_entry[] = "BN_CUDA_DEVICE=auto";
    char *cuda_device_environment[] = { cuda_device_entry, NULL };
    BnBackendRuntimePolicy cuda_device_policy;
    assert(bn_backend_runtime_policy_init(&cuda_device_policy,
                                          cuda_device_environment) == 0);
    assert(strcmp(bn_gpu_policy_cuda_device_selector(&cuda_device_policy),
                  "auto") == 0);
    bn_backend_runtime_policy_free(&cuda_device_policy);
    assert(bn_gpu_policy_cuda_exec_fail_debug_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_sync_each_op_debug_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_nan_debug_enabled(cuda_debug_policy));
    assert(bn_gpu_policy_cuda_dump_ops_enabled(cuda_debug_policy));
    ...
    unsetenv("BN_CUDA_ENABLE_NATIVE_QUANT_SSM_PREFILL");
    ...
    assert(!bn_gpu_policy_cuda_prefill_moe_layer_disabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_dense_layer_disabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_dense_debug_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_dense_profile_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_dense_profile_every_or_default(test_current_backend_runtime(), 36) ==
           36);
    assert(!bn_gpu_policy_cuda_prefill_ssm_layer_disabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_ssm_layer_disabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_prefill_ssm_quant_path_enabled(
        &gpu, BN_GGUF_TENSOR_Q8_0, BN_GGUF_TENSOR_Q8_0,
        BN_GGUF_TENSOR_Q8_0));
    ...
    gpu.caps &= ~BN_GPU_CAP_REFERENCE_RECURRENT_PREFILL;
    assert(bn_gpu_policy_prefill_ssm_quant_path_enabled(
        &gpu, BN_GGUF_TENSOR_Q4_K, BN_GGUF_TENSOR_Q4_K,
        BN_GGUF_TENSOR_Q4_K));
    setenv("BN_CUDA_ENABLE_NATIVE_QUANT_SSM_PREFILL", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_prefill_ssm_quant_path_enabled(
        &gpu, BN_GGUF_TENSOR_Q8_0, BN_GGUF_TENSOR_Q8_0,
        BN_GGUF_TENSOR_Q8_0));
    unsetenv("BN_CUDA_ENABLE_NATIVE_QUANT_SSM_PREFILL");
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_cuda_prefill_fused_asymmetric_kquant_gateup_batch_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_ssm_fused_asymmetric_kquant_gateup_batch_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_ssm_profile_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_stacked_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_stream_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_input_alias_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_f32_ab_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_scan_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_ssm_delta_128_warp_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_ssm_ffn_profile_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_prefill_ssm_ffn_gateup_f16_out_enabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_backend_opt_in_fused_gateup_enabled));
    ...
    assert(bn_gpu_policy_fused_gateup_silu_allowed(
        &gpu, BN_GGUF_TENSOR_Q5_K));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(TEST_POLICY_CALL0(bn_gpu_policy_shared_kquant_dot_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_shared_expert_gate_enabled));
    ...
    unsetenv("BN_CUDA_ENABLE_ALL_ACTIVE_TWO_KQUANT_MOE_REFERENCE_GPU_ROUTE");
    ...
    unsetenv("BN_CUDA_DISABLE_ALL_ACTIVE_TWO_KQUANT_MOE_REFERENCE_GPU_ROUTE");
    ...
    unsetenv("BN_CUDA_DISABLE_PREFILL_MMA_ATTN");
    ...
    gpu.kind = BN_GPU_BACKEND_CUDA;
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_cuda_prefill_mma_attention_disabled(
        test_current_backend_runtime()));
    setenv("BN_CUDA_DISABLE_PREFILL_MMA_ATTN", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_cuda_prefill_mma_attention_disabled(
        test_current_backend_runtime()));
    unsetenv("BN_CUDA_DISABLE_PREFILL_MMA_ATTN");
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_cuda_prefill_attention_min_tokens_configured(test_current_backend_runtime()));
    assert(!bn_gpu_policy_prefill_attention_min_tokens_configured(&gpu));
    assert(bn_gpu_policy_cuda_prefill_attention_min_tokens_or_default(test_current_backend_runtime(), 16) ==
           16);
    assert(bn_gpu_policy_prefill_attention_min_tokens_or_default(&gpu, 16) == 16);
    ...
    assert(bn_gpu_policy_cuda_prefill_attention_min_tokens_or_default(test_current_backend_runtime(), 16) ==
           9);
    assert(bn_gpu_policy_prefill_attention_min_tokens_or_default(&gpu, 16) == 9);
    gpu.kind = BN_GPU_BACKEND_METAL;
    assert(!bn_gpu_policy_prefill_attention_min_tokens_configured(&gpu));
    assert(bn_gpu_policy_prefill_attention_min_tokens_or_default(&gpu, 16) == 16);
    gpu.kind = BN_GPU_BACKEND_CUDA;
    ...
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_min_tokens_or_default(
               test_current_backend_runtime(), 256) == 256);
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 255, 512));
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 256, 512));
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 513, 512));
    setenv("BN_CUDA_ENABLE_PREFILL_GEMM_ATTN", "1", 1);
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 1, 512));
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 513, 512));
    unsetenv("BN_CUDA_ENABLE_PREFILL_GEMM_ATTN");
    setenv("BN_CUDA_PREFILL_GEMM_ATTN_MIN_TOKENS", "64", 1);
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_min_tokens_or_default(
               test_current_backend_runtime(), 256) == 64);
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 63, 512));
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 64, 512));
    ...
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled(test_current_backend_runtime(), 256, 512));
    unsetenv("BN_CUDA_DISABLE_PREFILL_GEMM_ATTN");
    assert(bn_gpu_policy_cuda_prefill_gemm_attention_enabled_for_shape(
        test_current_backend_runtime(), 8, 512, 1, 24, 4, 256, 6));
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled_for_shape(
        test_current_backend_runtime(), 8, 512, 1, 32, 8, 128, 4));
    setenv("BN_CUDA_DISABLE_PREFILL_GEMM_ATTN", "1", 1);
    assert(!bn_gpu_policy_cuda_prefill_gemm_attention_enabled_for_shape(
        test_current_backend_runtime(), 8, 512, 1, 24, 4, 256, 6));
    unsetenv("BN_CUDA_DISABLE_PREFILL_GEMM_ATTN");
    unsetenv("BN_CUDA_PREFILL_GEMM_ATTN_MIN_TOKENS");
    assert(bn_gpu_policy_cuda_prefill_attention_wo_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_cuda_prefill_qkv_attention_wo_enabled(test_current_backend_runtime()));
    ...
    unsetenv("BN_CUDA_DISABLE_PREFILL_QKV_ATTN_WO");
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(bn_gpu_policy_prefill_dense_chain_enabled(&gpu));
    assert(bn_gpu_policy_prefill_hybrid_chain_enabled(&gpu));
    assert(bn_gpu_policy_cuda_prefill_attention_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_prefill_attention_enabled(&gpu));
    assert(bn_gpu_policy_prefill_ssm_run_chain_enabled(&gpu));
    assert(bn_gpu_policy_prefill_ssm_ffn_fuse_allowed(&gpu));
    assert(!bn_gpu_policy_prefill_moe_chain_debug_enabled(&gpu));
    assert(!bn_gpu_policy_prefill_hybrid_chain_debug_enabled(&gpu));
    assert(bn_gpu_policy_moe_prefill_enabled(&gpu));
    assert(bn_gpu_policy_prefill_moe_layer_enabled(&gpu));
    assert(!bn_gpu_policy_moe_prefill_min_tokens_configured(&gpu));
    assert(bn_gpu_policy_moe_prefill_min_tokens_or_default(&gpu, 1) == 1);
    assert(bn_gpu_policy_moe_cache_prefill_enabled(&gpu));
    assert(bn_gpu_policy_moe_prefill_shared_fuse_enabled(&gpu));
    assert(bn_gpu_policy_moe_route_batch_enabled(&gpu));
    assert(!bn_gpu_policy_moe_route_batch_debug_enabled(&gpu));
    ...
    assert(bn_gpu_policy_moe_prefill_enabled(&gpu));
    assert(bn_gpu_policy_moe_prefill_min_tokens_configured(&gpu));
    assert(bn_gpu_policy_moe_prefill_min_tokens_or_default(&gpu, 1) == 7);
    gpu.kind = BN_GPU_BACKEND_METAL;
    assert(!bn_gpu_policy_moe_prefill_min_tokens_configured(&gpu));
    assert(bn_gpu_policy_moe_prefill_min_tokens_or_default(&gpu, 1) == 1);
    gpu.kind = BN_GPU_BACKEND_CUDA;
    ...
    assert(!bn_gpu_policy_moe_route_batch_debug_enabled(&gpu));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_large_hybrid_attention_enabled(&gpu));
    assert(!bn_gpu_policy_large_hybrid_cpu_attention_safe_enabled(&gpu));
    assert(!bn_gpu_policy_large_hybrid_cpu_attention_safe_disabled(&gpu));
    assert(!bn_gpu_policy_large_hybrid_cpu_attention_safe_forced(&gpu));
    assert(bn_gpu_policy_large_hybrid_prefill_enabled(&gpu));
    assert(bn_gpu_policy_large_hybrid_prefill_chain_enabled(&gpu));
    assert(!bn_gpu_policy_large_hybrid_prefill_disabled(&gpu));
    assert(!bn_gpu_policy_large_hybrid_argmax_enabled(&gpu));
    ...
    assert(bn_gpu_policy_large_hybrid_prefill_disabled(&gpu));
    assert(bn_gpu_policy_large_hybrid_argmax_enabled(&gpu));
    assert(bn_gpu_policy_fused_gateup_enabled(&gpu));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_fused_gateup_enabled));
    assert(bn_gpu_policy_gateup_split_enabled(&gpu));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_ffn_down_enabled));
    assert(bn_gpu_policy_qkv_split_enabled(&gpu));
    assert(!bn_gpu_policy_qkv_split_debug_enabled(&gpu));
    assert(bn_gpu_policy_ssm_qkvz_split_enabled(&gpu));
    assert(bn_gpu_policy_ssm_ab_stack_enabled(&gpu));
    assert(!bn_gpu_policy_split_residual_rmsnorm_enabled(&gpu));
    assert(!bn_gpu_policy_debug_fallback_enabled(&gpu));
    assert(!bn_gpu_policy_force_graph_enabled(&gpu));
    assert(bn_gpu_policy_flash_min_kv_or_default(&gpu, 0) == 0);
    ...
    assert(!bn_gpu_policy_backend_reference_attention_supported(&gpu));
    assert(!bn_gpu_policy_backend_reference_recurrent_supported(&gpu));
    assert(!bn_gpu_policy_backend_reference_recurrent_prefill_supported(
        &gpu));
    ...
    assert(bn_gpu_policy_backend_reference_attention_supported(&gpu));
    gpu.caps &= ~BN_GPU_CAP_REFERENCE_ATTENTION;
    assert(!bn_gpu_policy_backend_reference_attention_native_graph_supported(
        &gpu));
    gpu.caps |= BN_GPU_CAP_REFERENCE_ATTENTION_NATIVE_GRAPH;
    assert(bn_gpu_policy_backend_reference_attention_native_graph_supported(
        &gpu));
    gpu.caps &= ~BN_GPU_CAP_REFERENCE_ATTENTION_NATIVE_GRAPH;
    assert(!bn_gpu_policy_backend_reference_attention_token_fallback_supported(
        &gpu));
    ...
    gpu.kind = BN_GPU_BACKEND_METAL;
    assert(!bn_gpu_policy_backend_cpu_attention_fallback_supported(&gpu));
    assert(!bn_gpu_policy_backend_reference_attention_fallback_supported(
        &gpu));
    ...
    assert(bn_gpu_policy_backend_moe_reference_attention_supported(&gpu));
    ...
    assert(bn_gpu_policy_backend_weighted_add_sigmoid_supported(&gpu));
    assert(bn_gpu_policy_backend_moe_route_topk_supported(&gpu));
    ...
    assert(!bn_gpu_policy_backend_all_active_two_moe_direct_route_supported(
        &gpu));
    assert(!bn_gpu_policy_backend_resident_moe_ffn_supported(&gpu));
    assert(!bn_gpu_policy_backend_moe_expert_graph_supported(&gpu));
    gpu.caps |= BN_GPU_CAP_MOE_EXPERT_GRAPH;
    assert(bn_gpu_policy_backend_moe_expert_graph_supported(&gpu));
    assert(!bn_gpu_policy_backend_resident_moe_ffn_supported(&gpu));
    gpu.caps &= ~BN_GPU_CAP_MOE_EXPERT_GRAPH;
    gpu.caps |= BN_GPU_CAP_HYPER_CONNECTION_GRAPH;
    assert(bn_gpu_backend_has_cap(&gpu, BN_GPU_CAP_HYPER_CONNECTION_GRAPH));
    gpu.caps &= ~BN_GPU_CAP_HYPER_CONNECTION_GRAPH;
    gpu.caps |= BN_GPU_CAP_MOE_ROUTED_FFN;
    assert(bn_gpu_policy_backend_resident_moe_ffn_supported(&gpu));
    assert(bn_gpu_policy_backend_moe_expert_graph_supported(&gpu));
    ...
    assert(!bn_gpu_policy_backend_moe_expert_graph_supported(NULL));
    assert(!bn_gpu_policy_backend_weighted_add_sigmoid_supported(NULL));
    assert(!bn_gpu_policy_backend_moe_route_topk_supported(NULL));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_cpu_logits_enabled(&gpu));
    assert(!bn_gpu_policy_compare_logits_enabled(&gpu));
    assert(!bn_gpu_policy_debug_argmax_compare_enabled(&gpu));
    assert(!bn_gpu_policy_moe_ffn_disabled(&gpu));
    assert(bn_gpu_policy_moe_router_topk_enabled(&gpu, 1));
    assert(!bn_gpu_policy_moe_router_topk_enabled(&gpu, 0));
    assert(bn_gpu_policy_native_quant_moe_cpu_route_resident_enabled(&gpu, 1));
    assert(!bn_gpu_policy_native_quant_moe_cpu_route_resident_enabled(&gpu, 0));
    assert(!TEST_MOE_ROUTER_GPU_ENABLED());
    assert(bn_gpu_policy_moe_router_diff2_enabled(&gpu));
    assert(TEST_MOE_ROUTED_FFN_BATCH_ENABLED());
    assert(TEST_MOE_ROUTED_FFN_BATCH_ALLOWED(0));
    assert(!TEST_MOE_ROUTED_FFN_BATCH_ALLOWED(1));
    assert(!bn_gpu_policy_moe_cpu_actual_override_enabled(&gpu));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_cpu_attention_safe_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_ffn_down_requested));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_small_dense_prefill_disabled(&gpu));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_native_quant_logits_refine_requested));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_native_quant_logits_refine_disabled));
    ...
    unsetenv("BN_CUDA_ENABLE_SMALL_QWEN_Q8_LOGITS_REFINE");
    unsetenv("BN_CUDA_DISABLE_SMALL_QWEN_Q8_LOGITS_REFINE");
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_fast_ffn_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_fast_graph_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_cublas_decode_enabled));
    assert(bn_gpu_policy_cuda_moe_cublas_decode_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_cuda_moe_cublas_decode_debug_enabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_fast_route_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_dot_prepared_input_default_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_route_dot_prepared_input_default_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_route_block_prepared_input_enabled));
    ...
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_fast_prepared_gateup_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_fast_prepared_gateup_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layers_disabled));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layer_selected, 4));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_all_active_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_default_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_layer_selected, 4));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_default_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_down_skip_eps_or_default, 0.25f) ==
           0.25f);
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_cpu_attention_safe_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_logits_refine_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_cpu_moe_safe_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_reference_attention_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_cpu_route_resident_disabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_reference_gpu_route_requested));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_reference_gpu_route_disabled));
    assert(!TEST_ALL_ACTIVE_TWO_ROUTE_SELECTION_ENABLED());
    int route_from_layer = 99;
    int route_to_layer = 99;
    bn_gpu_policy_all_active_two_kquant_moe_route_layer_range(test_current_backend_runtime(), &route_from_layer,
                                                  &route_to_layer);
    ...
    setenv("BN_CUDA_DISABLE_ALL_ACTIVE_TWO_KQUANT_MOE_CPU_ROUTE_RESIDENT",
           "1", 1);
    setenv("BN_CUDA_ENABLE_ALL_ACTIVE_TWO_KQUANT_MOE_REFERENCE_GPU_ROUTE", "1", 1);
    setenv("BN_CUDA_DISABLE_ALL_ACTIVE_TWO_KQUANT_MOE_REFERENCE_GPU_ROUTE", "1", 1);
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layers_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layer_selected, 4));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layer_selected, 6));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_disabled));
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_default_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_layer_selected, 6));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_layer_selected, 4));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_default_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_down_skip_eps_or_default, 0.25f) >
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layers_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layer_selected, 7));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_pair_path_f32_layer_selected, 9));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_ordered_quant_path_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_enabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_cache_default_disabled));
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_default_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_float_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_layer_selected, 9));
    assert(!TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_layer_selected, 7));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_default_disabled));
    assert(TEST_POLICY_CALL0(bn_gpu_policy_all_active_two_kquant_moe_down_f32_4row_disabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_all_active_two_kquant_down_skip_eps_or_default, 0.25f) >
           0.062f);
    ...
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_attention_layer_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_attention_pos_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_gqa_layer_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_gqa_pos_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_qkv_layer_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_qkv_pos_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_down_layer_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_down_pos_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_state_layer_or_default, -1) == -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_state_pos_or_default, -1) == -1);
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 1));
    unsetenv("BN_METAL_ENABLE_PREPARED_F32");
    assert(!bn_gpu_policy_metal_prepared_f32_enabled(test_current_backend_runtime()));
    setenv("BN_METAL_ENABLE_PREPARED_F32", "1", 1);
    assert(bn_gpu_policy_metal_prepared_f32_enabled(test_current_backend_runtime()));
    unsetenv("BN_METAL_ENABLE_PREPARED_F32");
    assert(bn_gpu_policy_moe_gateup_split_enabled(&gpu, 1));
    assert(!bn_gpu_policy_moe_gateup_split_enabled(&gpu, 0));
    assert(!bn_gpu_policy_moe_route_profile_enabled(&gpu));
    assert(bn_gpu_policy_moe_route_profile_every_or_default(&gpu, 28) == 28);
    setenv("BN_GPU_DISABLE_FUSED_GATEUP", "1", 1);
    ...
    assert(bn_gpu_policy_debug_fallback_enabled(&gpu));
    assert(bn_gpu_policy_force_graph_enabled(&gpu));
    assert(bn_gpu_policy_flash_min_kv_or_default(&gpu, 0) == 32);
    ...
    gpu.kind = BN_GPU_BACKEND_METAL;
    assert(bn_gpu_policy_moe_router_topk_enabled(&gpu, 1));
    assert(!bn_gpu_policy_native_quant_moe_cpu_route_resident_enabled(
        &gpu, 1));
    setenv("BN_METAL_ENABLE_CPU_ROUTE_RESIDENT_MOE", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_native_quant_moe_cpu_route_resident_enabled(
        &gpu, 1));
    assert(bn_gpu_policy_lowbit_block32_moe_cpu_route_resident_enabled(
        &gpu, 1));
    assert(!bn_gpu_policy_native_quant_moe_cpu_route_resident_enabled(
        &gpu, 0));
    assert(!bn_gpu_policy_lowbit_block32_moe_cpu_route_resident_enabled(
    ...
    assert(!TEST_MOE_ROUTED_FFN_BATCH_ENABLED());
    assert(TEST_MOE_ROUTED_FFN_BATCH_ALLOWED(1));
    setenv("BN_GPU_DISABLE_MOE_ROUTER_GPU", "1", 1);
    assert(!TEST_MOE_ROUTER_GPU_ENABLED());
    ...
    assert(bn_gpu_policy_moe_compare_shared_down_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_moe_compare_norm_enabled(test_current_backend_runtime()));
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_attention_layer_or_default, -1) == 10);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_attention_pos_or_default, -1) == 11);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_gqa_layer_or_default, -1) == 12);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_gqa_pos_or_default, -1) == 13);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_qkv_layer_or_default, -1) == 14);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_qkv_pos_or_default, -1) == 15);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_down_layer_or_default, -1) == 16);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_down_pos_or_default, -1) == 17);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_state_layer_or_default, -1) == 18);
    assert(TEST_POLICY_CALL(bn_gpu_policy_compare_ffn_state_pos_or_default, -1) == 19);
    assert(!bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 0));
    assert(bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 1));
    gpu.kind = BN_GPU_BACKEND_METAL;
    assert(!bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 1));
    setenv("BN_METAL_ENABLE_MOE_SHARED_CPU_FALLBACK", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 0));
    assert(bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 1));
    setenv("BN_METAL_DISABLE_MOE_SHARED_CPU_FALLBACK", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_moe_shared_cpu_fallback_enabled(&gpu, 1));
    unsetenv("BN_METAL_DISABLE_MOE_SHARED_CPU_FALLBACK");
    unsetenv("BN_METAL_ENABLE_MOE_SHARED_CPU_FALLBACK");
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_gateup_split_enabled(&gpu, 1));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_route_profile_enabled(&gpu));
    assert(bn_gpu_policy_moe_route_profile_every_or_default(&gpu, 28) == 5);
    ...
    assert(bn_gpu_policy_cuda_cublas_cache_max_mb(test_current_backend_runtime(), 128, 0) == 128);
    assert(bn_gpu_policy_cuda_cublas_cache_max_mb(test_current_backend_runtime(), 128, 1) == 512);
    assert(TEST_CUDA_CUBLAS_AUX_CACHE_MAX_MB(
        BN_GGUF_TENSOR_Q8_0, 0, 0) == 128);
    assert(TEST_CUDA_CUBLAS_AUX_CACHE_MAX_MB(
        BN_GGUF_TENSOR_Q4_K, 0, 0) == 512);
    assert(TEST_CUDA_CUBLAS_AUX_CACHE_MAX_MB(
        BN_GGUF_TENSOR_Q6_K, 0, 1) == 0);
    assert(TEST_POLICY_CALL0(bn_gpu_policy_cuda_down_kquant_f16_cache_adds_f32_down_cache));
    ...
    assert(bn_gpu_policy_moe_down_aux_cache_bytes(
        &gpu, BN_GGUF_TENSOR_Q6_K, 8, 16) == 128 * sizeof(uint16_t));
    assert(bn_gpu_policy_aux_cache_bytes(&gpu,
    ...
    assert(TEST_CUDA_CUBLAS_AUX_CACHE_SUPPORTED(
    ...
    assert(bn_gpu_policy_cuda_cublas_cache_max_mb(test_current_backend_runtime(), 128, 1) == 7);
    assert(TEST_CUDA_CUBLAS_AUX_CACHE_MAX_MB(
    ...
    unsetenv("BN_CUDA_DISABLE_Q6K_MOE_DOWN_F32_CACHE");
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_moe_auto_resident_enabled(&gpu));
    gpu.kind = BN_GPU_BACKEND_CUDA;
    assert(!bn_gpu_policy_duplicate_moe_cache_enabled(&gpu));
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_duplicate_moe_cache_enabled(&gpu));
    unsetenv("BN_METAL_DISABLE_MMAP_ZERO_COPY");
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_cache_reserve_bytes) ==
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_cache_reserve_bytes) == 0);
    ...
    assert(TEST_POLICY_CALL0(bn_gpu_policy_moe_cache_reserve_bytes) == SIZE_MAX);
    unsetenv("BN_GPU_MOE_CACHE_RESERVE_MB");
    BnBackendRuntimePolicy mmap_policy = {0};
    assert(bn_gpu_policy_metal_mmap_zero_copy_enabled(&mmap_policy));
    setenv("BN_METAL_DISABLE_MMAP_ZERO_COPY", "1", 1);
    assert(bn_backend_runtime_policy_set(
               &mmap_policy, "BN_METAL_DISABLE_MMAP_ZERO_COPY", "1", 1) == 0);
    assert(!bn_gpu_policy_metal_mmap_zero_copy_enabled(&mmap_policy));
    unsetenv("BN_METAL_DISABLE_MMAP_ZERO_COPY");
    bn_backend_runtime_policy_unset(&mmap_policy,
                                    "BN_METAL_DISABLE_MMAP_ZERO_COPY");
    assert(bn_gpu_policy_metal_mmap_zero_copy_enabled(&mmap_policy));
    bn_backend_runtime_policy_free(&mmap_policy);
    unsetenv("BN_METAL_SHARED_WEIGHTS");
    BnBackendRuntimePolicy shared_policy = {0};
    assert(!bn_gpu_policy_metal_shared_weights_enabled(&shared_policy));
    setenv("BN_METAL_SHARED_WEIGHTS", "1", 1);
    assert(bn_backend_runtime_policy_set(
               &shared_policy, "BN_METAL_SHARED_WEIGHTS", "1", 1) == 0);
    assert(bn_gpu_policy_metal_shared_weights_enabled(&shared_policy));
    bn_backend_runtime_policy_free(&shared_policy);
    ...
    unsetenv("BN_METAL_DISABLE_Q6_Q8K");
    assert(!bn_gpu_policy_metal_specialized_native_quant_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_specialized_native_quant_decode_path_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_metal_specialized_native_quant_matvec_supported(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q6_K, 256, 1, 1));
    assert(bn_gpu_policy_metal_specialized_native_quant_shape_default_enabled(test_current_backend_runtime(),
    ...
    assert(bn_gpu_policy_metal_specialized_native_quant_enabled(test_current_backend_runtime()));
    assert(bn_gpu_policy_specialized_native_quant_decode_path_enabled(test_current_backend_runtime()));
    unsetenv("BN_METAL_NATIVE_QUANT_BARRIERS");
    unsetenv("BN_METAL_Q8_BARRIERS");
    assert(!bn_gpu_policy_metal_native_quant_barriers_enabled(test_current_backend_runtime()));
    setenv("BN_METAL_NATIVE_QUANT_BARRIERS", "1", 1);
    assert(bn_gpu_policy_metal_native_quant_barriers_enabled(test_current_backend_runtime()));
    unsetenv("BN_METAL_NATIVE_QUANT_BARRIERS");
    setenv("BN_METAL_Q8_BARRIERS", "1", 1);
    assert(bn_gpu_policy_metal_native_quant_barriers_enabled(test_current_backend_runtime()));
    unsetenv("BN_METAL_CPU_ORDER_RMSNORM");
    BnBackendRuntimePolicy metal_policy = test_backend_runtime_from_env();
    assert(!bn_gpu_policy_metal_cpu_order_rmsnorm_enabled(&metal_policy));
    bn_backend_runtime_policy_free(&metal_policy);
    setenv("BN_METAL_CPU_ORDER_RMSNORM", "1", 1);
    metal_policy = test_backend_runtime_from_env();
    assert(bn_gpu_policy_metal_cpu_order_rmsnorm_enabled(&metal_policy));
    bn_backend_runtime_policy_free(&metal_policy);
    ...
    metal_policy = test_backend_runtime_from_env();
    assert(!bn_gpu_policy_metal_full_barriers_enabled(&metal_policy));
    assert(!bn_gpu_policy_metal_barriers_enabled(&metal_policy));
    assert(bn_gpu_policy_metal_barriers_disabled(&metal_policy));
    bn_backend_runtime_policy_free(&metal_policy);
    metal_policy = (BnBackendRuntimePolicy){0};
    assert(bn_gpu_policy_apply_metal_barrier_disable_override(
               &metal_policy) == 0);
    assert(bn_gpu_policy_metal_barriers_disabled(&metal_policy));
    bn_backend_runtime_policy_free(&metal_policy);
    unsetenv("BN_METAL_DISABLE_BARRIERS");
    ...
    unsetenv("BN_METAL_DISABLE_SMALL_DENSE_NATIVE_QUANT_DEFAULT");
    ...
    unsetenv("BN_METAL_ENABLE_DENSE_RESIDUAL_GRAPH_DIAGNOSTIC");
    assert(!bn_gpu_policy_metal_dense_residual_graph_diagnostic_enabled(
        test_current_backend_runtime()));
    setenv("BN_METAL_ENABLE_DENSE_RESIDUAL_GRAPH_DIAGNOSTIC", "1", 1);
    assert(bn_gpu_policy_metal_dense_residual_graph_diagnostic_enabled(
        test_current_backend_runtime()));
    unsetenv("BN_METAL_ENABLE_DENSE_RESIDUAL_GRAPH_DIAGNOSTIC");
    assert(!bn_gpu_policy_metal_small_dense_native_quant_enabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_attn_only_enabled));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_ffn_only_enabled));
    assert(TEST_POLICY_CALL(bn_gpu_policy_small_dense_native_quant_from_layer_or_default, 40) ==
           -1);
    assert(TEST_POLICY_CALL(bn_gpu_policy_small_dense_native_quant_to_layer_or_default, 40, 0) ==
           -1);
    assert(!bn_gpu_policy_metal_native_quant_prepared_enabled(test_current_backend_runtime()));
    assert(!TEST_POLICY_CALL0(bn_gpu_policy_small_dense_native_quant_prepared_layer_default_enabled));
    assert(!bn_gpu_policy_metal_native_quant_prepared_upload_enabled(test_current_backend_runtime()));
    ...
    assert(bn_gpu_policy_metal_repacked_buffer_type(
        BN_GGUF_TENSOR_Q8_0) == -1);
    assert(!bn_gpu_policy_metal_prepared_stacked_upload_blocked(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q4_0));
    assert(!bn_gpu_policy_metal_small_dense_native_quant_matvec_supported(
        BN_GGUF_TENSOR_Q4_0, 0, 0, 1, 1, 0));
    BnBackendRuntimePolicy default_policy = test_backend_runtime_from_env();
    assert(bn_gpu_policy_metal_apply_small_dense_native_quant_default(
               &default_policy) == 0);
    ...
    assert(bn_gpu_policy_metal_small_dense_native_quant_graph_path_supported(
    ...
    assert(strcmp(bn_backend_runtime_policy_get(
                  ...
                  "0") == 0);
    assert(bn_backend_runtime_policy_enabled(
        &default_policy, "BN_GPU_SMALL_DENSE_NATIVE_QUANT_FFN_ONLY"));
    bn_backend_runtime_policy_free(&default_policy);
    setenv("BN_GPU_SMALL_DENSE_NATIVE_QUANT", "1", 1);
    setenv("BN_GPU_SMALL_DENSE_NATIVE_QUANT_FROM_LAYER", "0", 1);
    ...
    setenv("BN_GPU_SMALL_DENSE_EXACT_NATIVE_FROM_LAYER", "11", 1);
    setenv("BN_GPU_SMALL_DENSE_EXACT_NATIVE_TO_LAYER", "19", 1);
    ...
    unsetenv("BN_GPU_Q4_Q8_ATTN_ONLY");
    unsetenv("BN_GPU_Q4_Q8_FFN_ONLY");
    BnBackendRuntimePolicy override_policy = {0};
    assert(bn_gpu_policy_apply_native_quant_prepared_override(
               &override_policy) == 0);
    assert(bn_backend_runtime_policy_enabled(
        &override_policy, "BN_METAL_NATIVE_QUANT_PREPARED"));
    assert(!bn_backend_runtime_policy_enabled(
        &override_policy, "BN_METAL_Q4_PREPARED"));
    bn_backend_runtime_policy_free(&override_policy);
    unsetenv("BN_METAL_NATIVE_QUANT_PREPARED");
    unsetenv("BN_METAL_Q4_PREPARED");
    ...
    assert(!bn_gpu_policy_metal_prepared_stacked_upload_blocked(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q8_0));
    setenv("BN_GPU_SMALL_DENSE_NATIVE_QUANT_FROM_LAYER", "1", 1);
    assert(!bn_gpu_policy_metal_native_quant_prepared_upload_enabled(test_current_backend_runtime()));
    assert(!bn_gpu_policy_metal_prepared_stacked_upload_blocked(test_current_backend_runtime(),
        BN_GGUF_TENSOR_Q4_0));
    setenv("BN_GPU_SMALL_DENSE_NATIVE_QUANT_FROM_LAYER", "0", 1);
    ...
    default_policy = test_backend_runtime_from_env();
    assert(bn_gpu_policy_metal_apply_small_dense_native_quant_default(
               &default_policy) == 0);
    assert(!bn_gpu_policy_metal_small_dense_native_quant_enabled(
        &default_policy));
    ...
    bn_backend_runtime_policy_free(&default_policy);
    unsetenv("BN_METAL_DISABLE_Q4_Q8_DEFAULT");
    override_policy = (BnBackendRuntimePolicy){0};
    assert(bn_gpu_policy_apply_metal_small_dense_native_quant_default_disable_override(
               &override_policy) == 0);
    assert(bn_backend_runtime_policy_enabled(
        &override_policy,
        "BN_METAL_DISABLE_SMALL_DENSE_NATIVE_QUANT_DEFAULT"));
    assert(bn_gpu_policy_apply_metal_private_weights_override(
               &override_policy) == 0);
    assert(bn_backend_runtime_policy_enabled(
        &override_policy, "BN_METAL_PRIVATE_WEIGHTS"));
    unsetenv("BN_METAL_ENABLE_SPECIALIZED_NATIVE_QUANT");
    unsetenv("BN_METAL_ENABLE_Q6_Q8K");
    assert(bn_gpu_policy_apply_specialized_native_quant_decode_override(
               &override_policy) == 0);
    assert(bn_backend_runtime_policy_enabled(
        &override_policy, "BN_METAL_ENABLE_SPECIALIZED_NATIVE_QUANT"));
    assert(!bn_backend_runtime_policy_enabled(
        &override_policy, "BN_METAL_DISABLE_SPECIALIZED_NATIVE_QUANT"));
    bn_backend_runtime_policy_free(&override_policy);
    unsetenv("BN_METAL_ENABLE_SPECIALIZED_NATIVE_QUANT");
    unsetenv("BN_METAL_ENABLE_Q6_Q8K");
    unsetenv("BN_METAL_DISABLE_Q4_Q8_DEFAULT");
    ...
    test_gpu_runtime_refresh(&gpu);
    assert(!bn_gpu_policy_argmax_debug_enabled(&gpu));
    setenv("BN_GPU_DEBUG_ARGMAX", "1", 1);
    test_gpu_runtime_refresh(&gpu);
    assert(bn_gpu_policy_argmax_debug_enabled(&gpu));
    unsetenv("BN_GPU_PROFILE");
    BnBackendRuntimePolicy profile_policy = {0};
    assert(bn_gpu_policy_profile_level(&profile_policy) == 0);
    setenv("BN_GPU_PROFILE", "4", 1);
    assert(bn_backend_runtime_policy_set(&profile_policy, "BN_GPU_PROFILE",
                                         "4", 1) == 0);
    assert(bn_gpu_policy_profile_level(&profile_policy) == 4);
    bn_backend_runtime_policy_free(&profile_policy);
    unsetenv("BN_CUDA_DISABLE_CUBLAS_MATMUL");
    unsetenv("BN_CUDA_DISABLE_Q6K_CUBLAS_F16");
    unsetenv("BN_CUDA_CUBLAS_CACHE_MAX_MB");
    unsetenv("BN_CUDA_ENABLE_MOE_DOWN_KQUANT_F32_CACHE");
...
static void test_quant_registry(void) {
    ...
    assert(bn_backend_quant_dense_graph_supported(BN_GGUF_TENSOR_Q5_1));
    ...
    BnCPURuntimePolicy cpu_runtime = test_cpu_runtime_from_env();
    assert(cpu_runtime.tied_kquant_refine_top == 0);
    assert(cpu_runtime.tied_kquant_hybrid_top == 0);
    setenv("BN_CPU_TIED_KQUANT_REFINE_TOP", "1", 1);
    setenv("BN_CPU_TIED_KQUANT_HYBRID_TOP", "2", 1);
    cpu_runtime = test_cpu_runtime_from_env();
    assert(cpu_runtime.tied_kquant_refine_top == 1);
    assert(cpu_runtime.tied_kquant_hybrid_top == 2);
    ...
    cpu_runtime = test_cpu_runtime_from_env();
    assert(cpu_runtime.tied_kquant_refine_top == 128);
    assert(cpu_runtime.tied_kquant_hybrid_top == 128);
    unsetenv("BN_CPU_TIED_KQUANT_REFINE_TOP");
    unsetenv("BN_CPU_TIED_KQUANT_HYBRID_TOP");
    ...
    assert(bn_backend_quant_gpu_fused_gateup_silu_cap(BN_GGUF_TENSOR_Q4_K) == 0);
    ...
    assert(bn_quant_format_gpu_matvec_kquant_dot_flag(BN_GGUF_TENSOR_Q5_K, 1) ==
           BN_QUANT_GPU_MATVEC_FLAG_KQUANT_DOT);
    ...
    assert(!bn_backend_quant_moe_routed_native_quant(BN_GGUF_TENSOR_Q8_0,
                                           BN_GGUF_TENSOR_Q8_0,
                                           BN_GGUF_TENSOR_Q4_K));
    assert(bn_quant_format_supports_moe_routed_lowbit_block32(
        BN_GGUF_TENSOR_Q4_0, BN_GGUF_TENSOR_Q4_0,
        BN_GGUF_TENSOR_Q4_0));
    assert(!bn_quant_format_supports_moe_routed_lowbit_block32(
        BN_GGUF_TENSOR_Q4_0, BN_GGUF_TENSOR_Q4_0,
        BN_GGUF_TENSOR_Q8_0));
    assert(bn_backend_quant_moe_routed_lowbit_block32(
        BN_GGUF_TENSOR_Q4_0, BN_GGUF_TENSOR_Q4_0,
        BN_GGUF_TENSOR_Q4_0));
    assert(!bn_backend_quant_moe_routed_lowbit_block32(
        BN_GGUF_TENSOR_Q4_0, BN_GGUF_TENSOR_Q4_K,
        BN_GGUF_TENSOR_Q4_0));
    ...
    assert(!bn_backend_quant_supports_f16_float_cache_matvec(
        BN_GGUF_TENSOR_Q8_0));
    assert(bn_backend_quant_supports_packed_kquant_f16_cache_matvec(
        BN_GGUF_TENSOR_Q5_K));
    assert(!bn_backend_quant_supports_packed_kquant_f16_cache_matvec(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_down_kquant_f16_cache_matvec(
        BN_GGUF_TENSOR_Q6_K));
    assert(!bn_backend_quant_supports_down_kquant_f16_cache_matvec(
        BN_GGUF_TENSOR_Q4_K));
    ...
    assert(!bn_backend_quant_supports_native_quant_warp_matvec(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_asymmetric_kquant_dot_matmul(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_dot_matmul(
        BN_GGUF_TENSOR_Q6_K));
    assert(bn_backend_quant_supports_asymmetric_kquant_prepared_input_matmul(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_prepared_input_matmul(
        BN_GGUF_TENSOR_Q6_K));
    assert(bn_backend_quant_supports_deinterleaved_kquant_prepared_input_matmul(
        BN_GGUF_TENSOR_Q5_K));
    assert(!bn_backend_quant_supports_deinterleaved_kquant_prepared_input_matmul(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_down_kquant_dot_matmul(
        BN_GGUF_TENSOR_Q6_K));
    assert(!bn_backend_quant_supports_down_kquant_dot_matmul(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_native_quant_matmul(
        BN_GGUF_TENSOR_Q8_0));
    assert(!bn_backend_quant_supports_native_quant_matmul(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_legacy_block_matmul(
        BN_GGUF_TENSOR_Q5_0));
    assert(!bn_backend_quant_supports_legacy_block_matmul(
        BN_GGUF_TENSOR_Q5_K));
    assert(bn_backend_quant_supports_legacy_block_fused_gateup(
        BN_GGUF_TENSOR_Q5_0));
    assert(!bn_backend_quant_supports_legacy_block_fused_gateup(
        BN_GGUF_TENSOR_Q8_0));
    assert(bn_backend_quant_supports_native_quant_fused_gateup(
        BN_GGUF_TENSOR_Q8_0));
    assert(!bn_backend_quant_supports_native_quant_fused_gateup(
        BN_GGUF_TENSOR_Q5_0));
    assert(bn_backend_quant_supports_asymmetric_kquant_dot_fused_gateup(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_dot_fused_gateup(
        BN_GGUF_TENSOR_Q6_K));
    assert(bn_backend_quant_supports_asymmetric_kquant_prepared_input_fused_gateup(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_prepared_input_fused_gateup(
        BN_GGUF_TENSOR_Q5_K));
    assert(bn_backend_quant_supports_deinterleaved_kquant_prepared_input_fused_gateup(
        BN_GGUF_TENSOR_Q5_K));
    assert(!bn_backend_quant_supports_deinterleaved_kquant_prepared_input_fused_gateup(
        BN_GGUF_TENSOR_Q4_K));
    ...
    assert(!bn_backend_quant_split_allows_fused_bias(
        BN_GGUF_TENSOR_Q8_0));
    assert(bn_backend_quant_supports_asymmetric_kquant_dot_split(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_dot_split(
        BN_GGUF_TENSOR_Q6_K));
    assert(bn_backend_quant_supports_asymmetric_kquant_prepared_input_split(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_asymmetric_kquant_prepared_input_split(
        BN_GGUF_TENSOR_Q5_K));
    assert(bn_backend_quant_supports_deinterleaved_kquant_prepared_input_split(
        BN_GGUF_TENSOR_Q5_K));
    assert(!bn_backend_quant_supports_deinterleaved_kquant_prepared_input_split(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_native_quant_split(
        BN_GGUF_TENSOR_Q8_0));
    assert(!bn_backend_quant_supports_native_quant_split(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_split_value_4warp_dot(
        BN_GGUF_TENSOR_Q4_K));
    assert(!bn_backend_quant_supports_split_value_4warp_dot(
        BN_GGUF_TENSOR_Q6_K));
    assert(bn_backend_quant_supports_split_value_mmvq(
        BN_GGUF_TENSOR_Q6_K));
    assert(!bn_backend_quant_supports_split_value_mmvq(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_split_value_fuse(
        BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_split_value_fuse(
        BN_GGUF_TENSOR_Q6_K));
    assert(!bn_backend_quant_supports_split_value_fuse(
    ...
    assert(!bn_backend_quant_deinterleaved_kquant_pair_matmul(
        BN_GGUF_TENSOR_Q5_K, BN_GGUF_TENSOR_Q4_K));
    assert(bn_backend_quant_supports_kquant_logits_argmax(
        BN_GGUF_TENSOR_Q6_K));
    assert(!bn_backend_quant_supports_kquant_logits_argmax(
        BN_GGUF_TENSOR_Q4_K));
    ...
    assert(bn_quant_format_gpu_matvec_reference_kquant_flag(BN_GGUF_TENSOR_Q4_K, 1) ==
           BN_QUANT_GPU_MATVEC_FLAG_REFERENCE_KQUANT);
    ...
    assert(bn_backend_quant_dense_graph_model_supported(
        &dense_w, &dense_c, BN_BACKEND_QUANT_DENSE_GRAPH_ANY));
    assert(!bn_backend_quant_dense_graph_model_supported(
        &dense_w, &dense_c, BN_BACKEND_QUANT_DENSE_GRAPH_NATIVE_QUANT));
    ...
    assert(bn_backend_quant_dense_graph_model_supported(
        &dense_w, &dense_c, BN_BACKEND_QUANT_DENSE_GRAPH_ANY));
    assert(!bn_backend_quant_dense_graph_model_supported(
        NULL, &dense_c, BN_BACKEND_QUANT_DENSE_GRAPH_ANY));
    assert(!bn_backend_quant_dense_graph_model_supported(
        &dense_w, NULL, BN_BACKEND_QUANT_DENSE_GRAPH_ANY));
    ...
    assert(bn_backend_quant_supports_packed_codebook_matvec(
        BN_GGUF_TENSOR_IQ4_XS));
...
static void test_backend_layout_prepared_qweights(void) {
    ...
    /* Every prepared format must contribute to the shared arena budget.
     * Q5_K layouts precede Q4_K here, reproducing later weights losing their
     * prepared buffers when Q5_K bytes are omitted from the accounting. */
    BnBlockQ5K mixed_q5[8] = {0};
    BnBlockQ4K mixed_q4[8] = {0};
    BnLayerWeights mixed_layer = {0};
    BnWeights mixed_weights = {0};
    mixed_weights.layers = &mixed_layer;
    mixed_layer.attn.wq = (BnQWeight){
        mixed_q5, BN_GGUF_TENSOR_Q5_K, 8, BN_QK_K, 1.0f
    };
    mixed_layer.ffn.ffn_down = (BnQWeight){
        mixed_q4, BN_GGUF_TENSOR_Q4_K, 8, BN_QK_K, 1.0f
    };
    size_t q5_bytes = bn_quant_prepared_qweight_size(&mixed_layer.attn.wq, NULL);
    size_t q4_bytes = bn_quant_prepared_qweight_size(&mixed_layer.ffn.ffn_down, NULL);
    BnBackendLayoutPreparedStats mixed_stats = {0};
    size_t mixed_bytes = bn_backend_layout_prepared_qweights_size(
        &config, &mixed_weights, &mixed_stats);
    assert(q5_bytes > 0 && q4_bytes > 0);
    assert(mixed_bytes == q5_bytes + q4_bytes);
    assert(mixed_stats.lowbit_repack_bytes == q5_bytes);
    assert(mixed_stats.kquant_scale_table_bytes == q4_bytes);
    SHArena *mixed_arena = sh_arena_create(mixed_bytes);
    BnBackendModel *mixed_backend = bn_backend_model_create();
    assert(mixed_arena && mixed_backend);
    bn_backend_layout_prepare_qweights(mixed_backend, &config, &mixed_weights,
                                       mixed_arena, NULL);
    const BnPreparedWeight *mixed_q5_prepared =
        bn_backend_model_prepared_qweight(mixed_backend, &mixed_layer.attn.wq);
    const BnPreparedWeight *mixed_q4_prepared =
        bn_backend_model_prepared_qweight(mixed_backend, &mixed_layer.ffn.ffn_down);
    assert(mixed_q5_prepared && mixed_q5_prepared->aux);
    assert(mixed_q4_prepared && mixed_q4_prepared->aux);
    assert(mixed_q5_prepared->kind == BN_PREPARED_WEIGHT_Q5_K_X8);
    assert(mixed_q4_prepared->kind == BN_PREPARED_WEIGHT_Q4_K_SCALES);
    bn_backend_model_free(mixed_backend);
    sh_arena_free(mixed_arena);
#else
    assert(q4k_bytes == 0);
    assert(q4k_stats.lowbit_repack_bytes == 0);
...
typedef struct {
    int batch_calls;
    int single_calls;
    const void *reject_weight;
} RetryMatmulState;

static int retry_matmul_batch(void *ctx, const BnGPUMatvecOp *ops, int n_ops,
                              const float *x, int n_tokens, int cols) {
    RetryMatmulState *state = ctx;
    (void)ops; (void)n_ops; (void)x; (void)n_tokens; (void)cols;
    state->batch_calls++;
    return -1;
}

static int retry_matmul_single(void *ctx, float *out, void *weight,
                               const float *x, int rows, int cols,
                               int n_tokens, int type) {
    RetryMatmulState *state = ctx;
    state->single_calls++;
    if (weight == state->reject_weight) return -1;
    assert(type == BN_GGUF_TENSOR_F32 && rows == 1 && cols == 4);
    const float *w = weight;
    for (int t = 0; t < n_tokens; t++) {
        out[t] = 0;
        for (int c = 0; c < cols; c++) out[t] += w[c] * x[t * cols + c];
    }
    return 0;
}

static void test_gpu_matmul_batch_retries_single(void) {
    float cpu_weights[4] = {1, 1, 1, 1};
    float gpu_weights[2][4] = {{2, 2, 2, 2}, {3, 3, 3, 3}};
    const float x[8] = {1, 2, 3, 4, 2, 3, 4, 5};
    BnQWeight weight = {0};
    weight.data = cpu_weights; weight.rows = 1; weight.cols = 4;
    weight.type = BN_GGUF_TENSOR_F32;
    float out[2][2];
    BnMatvecTask tasks[2] = {{out[0], &weight, NULL, 0},
                             {out[1], &weight, NULL, 0}};
    const void *buffers[2] = {gpu_weights[0], gpu_weights[1]};
    for (int batch_available = 0; batch_available < 2; batch_available++) {
        for (int reject_single = 0; reject_single < 2; reject_single++) {
            RetryMatmulState state = {0};
            state.reject_weight = reject_single ? gpu_weights[1] : NULL;
            BnGPUBackend gpu = {0}; gpu.ctx = &state;
            gpu.matmul_batch = batch_available ? retry_matmul_batch : NULL; gpu.matmul = retry_matmul_single;
            bn_backend_quant_matmul_batch_gpu_buf(tasks, buffers, 2, x, 2, 4,
                                                  NULL, NULL, &gpu);
            assert(state.batch_calls == batch_available && state.single_calls == 2);
            assert(out[0][0] == 20 && out[0][1] == 28);
            assert(out[1][0] == (reject_single ? 10 : 30));
            assert(out[1][1] == (reject_single ? 14 : 42));
        }
    }
    printf("test_gpu_matmul_batch_retries_single... PASSED\n");
}

static int check_rope_activation_plan(void *ctx, const BnGPUActivationPlan *plan) {
    assert(plan->separate_rope_norm == (*(int *)ctx == 1));
    assert(plan->rope_frequency_count == 128);
    assert(plan->rope_frequency_plan_count == 2);
    const BnGPURopeFrequencyPlan *a = &plan->rope_frequency_plans[0];
    const BnGPURopeFrequencyPlan *b = &plan->rope_frequency_plans[1];
    assert(a->offset == 0 && a->pair_count == 64 && a->rotary_dims == 128);
    assert(a->theta == 1000000.0f && !a->factors);
    assert(a->factor_mode == BN_GPU_ROPE_FACTOR_NONE);
    assert(b->offset == 64 && b->pair_count == 16 && b->rotary_dims == 32);
    assert(b->theta == 10000.0f && !b->factors);
    assert(plan->rope_frequencies[0] == 1 && plan->rope_frequencies[64] == 1);
    for (int i = 80; i < 128; i++) assert(plan->rope_frequencies[i] == 0);
    (*(int *)ctx)++;
    return 0;
}

static void test_rope_activation_plan(void) {
    BnModel model = {0};
    BnLayerWeights layers[2] = {0};
    model.config.dim = 256; model.config.n_layers = 2;
    model.config.n_heads = 2; model.config.n_kv_heads = 1;
    model.config.head_size = 128; model.config.kv_dim = 128;
    model.config.rope_dim_count = 128; model.config.rope_theta = 1000000.0f;
    model.config.rope_theta_swa = 10000.0f; model.config.rope_dim_count_swa = 32;
    layers[1].attn.head_size = 64;
    model.weights.layers = layers;
    int calls = 0;
    BnGPUBackend gpu = {0};
    gpu.ctx = &calls; gpu.init_activations = check_rope_activation_plan;
    assert(bn_model_init_gpu_activations(&model, &gpu) == 0);
    assert(calls == 1);
    model.config.policy_flags |= BN_MODEL_ARCH_POLICY_SEPARATE_ROPE_NORM;
    assert(bn_model_init_gpu_activations(&model, &gpu) == 0);
    model.config.policy_flags &= ~BN_MODEL_ARCH_POLICY_SEPARATE_ROPE_NORM;
    assert(bn_model_init_gpu_activations(&model, &gpu) == 0);
    assert(calls == 3);
    printf("test_rope_activation_plan... PASSED\n");
}

int main(void) {
    test_rope_activation_plan();
    test_gpu_matmul_batch_retries_single();
    test_moe_combined_prefill_default();
    test_backend_runtime_policy_snapshot();
    test_data_size();
    test_quant_registry();
    test_gpu_policy_helpers();
    test_backend_layout_reasons();
    test_backend_layout_prepared_qweights();
    test_prepared_native_quant_configuration();
    test_prepare_cpu_operations();
    test_native_borrowed_layout_contract();
