...
if grep -n 'config->n_experts\|config->n_experts_active\|config->moe_intermediate_size\|config->moe_norm_topk_prob\|config->moe_expert_weights_scale\|config->moe_uses_reference_silu\|config->has_shared_expert\|config->shared_expert_intermediate_size' src/model_policy.c >/dev/null 2>&1; then
    echo "model config policy must delegate MoE config semantics to model_arch helpers"
    fail=1
fi
...
if rg -n 'bn_model_arch_cpu_prefill_uses_decode_for_parity' \
    src/transformer \
    src/transformer.c \
    src/gpu_policy.c >/dev/null 2>&1; then
    echo "Transformer/GPU policy must use backend-neutral prefill parity model_arch helpers"
    fail=1
fi
...
if grep -n 'bn_transformer_cpu_prefill_force_float_kquant_enabled' \
    ...
    test/test_transformer.c >/dev/null 2>&1; then
    echo "CPU prefill float K-quant fallback policy must use fallback behavior names"
    fail=1
fi
...
if grep -n 'bn_backend_quant_moe_route_asymmetric_kquant_down\|bn_backend_quant_moe_route_native_quant' src/gpu_policy.c >/dev/null 2>&1; then
    echo "GPU policy resident routed FFN eligibility must compose backend quant behavior helpers"
    fail=1
fi
...
if grep -n 'c->norm_eps\|m->config\.norm_eps' src/transformer/logits.c >/dev/null 2>&1; then
    echo "src/transformer/logits.c must use logits execution policy for output norm config"
    fail=1
fi

if grep -n 'c->norm_eps\|m->config\.norm_eps' src/transformer.c >/dev/null 2>&1; then
    echo "Top-level transformer execution must use model norm policy helpers"
    fail=1
fi
...
if grep -n 'bn_model_arch_uses_all_active_two_expert_moe\|bn_model_arch_uses_two_expert_all_active_moe\|bn_model_arch_uses_more_than_two_expert_moe' src/transformer/gpu_policy.c >/dev/null 2>&1; then
    echo "src/transformer/gpu_policy.c must use MoE policy helpers for MoE route-shape policy"
    fail=1
fi

if grep -n 'bn_model_arch_uses_all_active_two_expert_moe\|bn_model_arch_uses_two_expert_all_active_moe\|bn_model_arch_uses_more_than_two_expert_moe' src/transformer/prefill_policy.c >/dev/null 2>&1; then
    echo "src/transformer/prefill_policy.c must use MoE policy helpers for MoE route-shape policy"
    fail=1
fi
...
do
    if grep -n 'bn_quant_format_gpu_split_cap\|bn_backend_quant_gpu_split_cap\|bn_backend_quant_gpu_fused_gateup_silu_cap\|bn_quant_format_gpu_matvec_q8k_dot_flag\|bn_backend_quant_gpu_matvec_q8k_dot_flag\|bn_quant_format_gpu_matvec_kquant_dot_flag\|bn_backend_quant_gpu_matvec_kquant_dot_flag\|bn_quant_format_gpu_matvec_exact_q6k_flag\|bn_backend_quant_gpu_matvec_exact_q6k_flag\|bn_quant_format_gpu_matvec_exact_kquant_flag\|bn_backend_quant_gpu_matvec_exact_kquant_flag\|bn_quant_format_gpu_float_buffer_type\|bn_backend_quant_gpu_float_buffer_type\|BN_BACKEND_QUANT_GPU_MATVEC_FLAG_' "$file" >/dev/null 2>&1; then
...
if [ "$(grep -c 'buffer_destroy' src/gpu_moe_bridge.c)" -gt 2 ]; then
    echo "src/gpu_moe_bridge.c must centralize MoE expert backend buffer destruction"
    fail=1
fi
...
if grep -n 'buffer_destroy' src/model_gpu.c >/dev/null 2>&1; then
    echo "src/model_gpu.c must use GPU backend helpers for uploaded model buffer destruction"
    fail=1
fi
...
if grep -n 'gpu->buffer_create' src/backend_layout.c >/dev/null 2>&1; then
    echo "src/backend_layout.c must use GPU backend helpers for layout buffer uploads"
    fail=1
fi
...
if grep -n '!gpu || !gpu->argmax_activation' src/transformer/gpu.c >/dev/null 2>&1; then
    echo "src/transformer/gpu.c must use GPU policy helpers for argmax availability"
    fail=1
fi
...
if grep -n 'gpu->read_activation\|gpu->write_activation' src/transformer/gpu_fallback.c >/dev/null 2>&1; then
    echo "src/transformer/gpu_fallback.c must use GPU read/write helpers instead of raw backend hooks"
    fail=1
fi
...
if grep -n 'c->norm_eps\|m->config\.norm_eps' src/transformer/gpu_fallback.c >/dev/null 2>&1; then
    echo "GPU fallback execution must use model norm policy helpers"
    fail=1
fi
...
if grep -n 'bn_model_config_norm_epsilon' src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU decode execution must compose norm policy through GPU policy helpers"
    fail=1
fi
...
if grep -n 'kv_tq_bits\|kv_f16' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "src/transformer/prefill.c must use planned KV mode policy helpers instead of raw KV config checks"
    fail=1
fi
...
if grep -n 'BN_DUMP_LAYER_INP\|BN_DUMP_LAYER_POS\|BN_DUMP_ALL_HEADS\|BN_CPU_REFERENCE_DOT\|BN_CPU_REFERENCE_BLOCK_QUANT_DOT\|BN_CPU_REFERENCE_Q4_DOT\|BN_CPU_LLAMA_DOT\|BN_CPU_LLAMA_Q4_DOT' src/transformer/cpu.c src/transformer/cpu_backend.c >/dev/null 2>&1; then
...
if grep -n 'bn_model_config_norm_epsilon' src/transformer/cpu.c >/dev/null 2>&1; then
    echo "Transformer CPU execution must compose norm policy through CPU policy helpers"
    fail=1
fi
...
if grep -n 'bn_transformer_cpu_quantize_q8k_activation\|prepared_kquant_bsums\|prepared_kquant_d\|\bx_d\b\|\bx_bsums\b' \
    include/transformer_cpu_backend_internal.h \
    src/transformer/cpu_backend.c \
    src/transformer/cpu.c \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "Transformer prepared K-quant activation helpers and buffers must use behavior names"
    fail=1
fi
...
if grep -n '#include "quant.h"\|bn_quant_' src/transformer/gpu_fallback.c >/dev/null 2>&1; then
    echo "src/transformer/gpu_fallback.c must use fallback/backend helpers, not quant internals directly"
    fail=1
fi
...
if grep -n 'bn_model_gpu(m)->matmul_batch' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "Prefill execution code must use prefill policy helpers for GPU-resident batched matmul availability"
    fail=1
fi
...
if awk '/^int bn_transformer_gpu_batch_prefill_enabled/{flag=1} /^int bn_transformer_gpu_large_hybrid_cpu_attn_fallback_enabled/{flag=0} flag{print}' \
    src/transformer/gpu_policy.c | grep -n 'gpu->prefill_ssm_layer' >/dev/null 2>&1; then
    echo "Transformer GPU batch prefill policy must use GPU backend helpers for SSM layer availability"
    fail=1
fi
...
if grep -n '!gpu || !gpu->matmul ||' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "Prefill execution code must use prefill policy helpers for single matmul availability"
    fail=1
fi

if grep -n '!gpu || !gpu->matmul_batch ||' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "Prefill execution code must use prefill policy helpers for batched matmul availability"
    fail=1
fi
...
if grep -n 'bn_backend_quant_asymmetric_kquant_dot_matmul_candidate\|bn_backend_quant_asymmetric_kquant_prepared_input_matmul_candidate\|bn_backend_quant_deinterleaved_kquant_prepared_input_matmul_candidate\|bn_backend_quant_down_kquant_dot_matmul_candidate\|bn_backend_quant_native_quant_matmul_candidate\|bn_backend_quant_legacy_block_matmul_candidate\|bn_backend_quant_legacy_block_fused_gateup_candidate\|bn_backend_quant_native_quant_fused_gateup_candidate\|bn_backend_quant_asymmetric_kquant_dot_fused_gateup_candidate\|bn_backend_quant_asymmetric_kquant_prepared_input_fused_gateup_candidate\|bn_backend_quant_deinterleaved_kquant_prepared_input_fused_gateup_candidate\|bn_backend_quant_asymmetric_kquant_dot_split_candidate\|bn_backend_quant_asymmetric_kquant_prepared_input_split_candidate\|bn_backend_quant_deinterleaved_kquant_prepared_input_split_candidate\|bn_backend_quant_native_quant_split_candidate\|bn_backend_quant_split_value_4warp_dot_candidate\|bn_backend_quant_split_value_mmvq_candidate\|bn_backend_quant_split_value_fuse_candidate\|bn_backend_quant_kquant_logits_argmax_candidate' include/backend_quant.h src/gpu_policy.c src/gpu_cuda.cu test/test_gpu_backend.c >/dev/null 2>&1; then
    echo "backend quant matmul/fused/split/logits helpers must use supports behavior names"
    fail=1
fi
...
if grep -n 'gpu->execute\|gpu->read_activation\|gpu->write_activation\|gpu->max_storage_binding_size' src/transformer/gpu_policy.c >/dev/null 2>&1; then
    echo "Transformer GPU policy must use backend GPU helpers for execution, activation transfer, and storage binding capability"
    fail=1
fi
...
if grep -n 'q8_quant_pipeline\|q8_buf\|q8_scales_buf\|q8_bsums_buf\|q8_need\|bsums_need\|ensure_q8_scratch\|ensure_q8k_scratch\|metal_encode_q8_quant' src/gpu_metal.m >/dev/null 2>&1; then
    echo "Metal native-quant scratch state must use behavior names, not Q8/Q8K implementation names"
    fail=1
fi
...
if awk '/^int bn_gpu_policy_cuda_legacy_block_matvec4_enabled/{flag=1} /^int bn_gpu_policy_cuda_native_quant_warp_disabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1; then
    echo "src/gpu_policy.c public legacy/asymmetric K-quant policy helpers must compose local env policy helpers"
    fail=1
fi

if awk '/^int bn_gpu_policy_cuda_native_quant_warp_disabled/{flag=1} /^int bn_gpu_policy_kquant_matvec4_enabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1; then
    echo "src/gpu_policy.c public native/F16 K-quant policy helpers must compose local env policy helpers"
    fail=1
fi

if awk '/^int bn_gpu_policy_kquant_matvec4_enabled/{flag=1} /^int bn_gpu_policy_cuda_down_kquant_mmvq_enabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1; then
    echo "src/gpu_policy.c public K-quant matvec/dot policy helpers must compose local env policy helpers"
    fail=1
fi
...
if awk '/^int bn_gpu_policy_cuda_fuse_bias_enabled/{flag=1} /^int bn_gpu_policy_cuda_prefill_moe_layer_disabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1; then
    echo "src/gpu_policy.c public CUDA fusion/debug policy helpers must compose local env policy helpers"
    fail=1
fi
...
if awk '/^BnTransformerPrefillSSMStateUploadPolicy/{flag=1} /^BnTransformerPrefillEntryPolicy/{flag=0} flag{print}' src/transformer/prefill_policy.c | grep -n 'bn_model_arch_uses_hybrid_ssm' >/dev/null 2>&1; then
    echo "Prefill SSM state upload policy must use prefill sequence policy helpers"
    fail=1
fi
...
if grep -n 'gpu->dense_ffn_batch\|gpu->moe_route_batch\|gpu->moe_route_routed_ffn_batch\|gpu_batch->moe_routed_ffn_batch\|gpu_batch->moe_ffn_batch' src/moe_prefill.c >/dev/null 2>&1; then
...
if grep -n 'gpu->buffer_create' src/gpu_moe_bridge.c >/dev/null 2>&1; then
    echo "src/gpu_moe_bridge.c must use GPU backend helpers for buffer uploads"
    fail=1
fi
...
if grep -n 'router_weight != NULL' src/model_gpu.c >/dev/null 2>&1; then
    echo "src/model_gpu.c must compose MoE router policy helpers"
    fail=1
fi
...
if grep -n 'shared_gate\.data != NULL' src/transformer/gpu_policy.c >/dev/null 2>&1; then
    echo "src/transformer/gpu_policy.c must compose loaded shared MoE expert path helpers"
    fail=1
fi

if grep -n 'lw->shared\.shared_gate\.data && shared && shared->shared_gate' src/transformer/gpu_emit.c >/dev/null 2>&1; then
    echo "src/transformer/gpu_emit.c must compose shared expert path availability helpers"
    fail=1
fi
...
if grep -n 'bn_transformer_gpu_all_active_two_kquant_moe_exact_gpu_route_enabled\|\bexact_gpu_route\b\|exact route' src/transformer/gpu_policy.c src/transformer/gpu_internal.h src/gpu_policy.c src/gpu_cuda.cu include/gpu_policy.h test/test_transformer.c test/test_gpu_backend.c >/dev/null 2>&1; then
...
if grep -n '#include "quant.h"\|bn_quant_' src/model.c >/dev/null 2>&1; then
    echo "src/model.c must use model quant policy helpers, not quant internals directly"
    fail=1
fi

if grep -n '#include "quant.h"\|bn_quant_' src/model_embed.c >/dev/null 2>&1; then
    echo "src/model_embed.c must use model quant policy helpers, not quant internals directly"
    fail=1
fi
...
if grep -n 'router_weight != NULL' src/main.c >/dev/null 2>&1; then
    echo "src/main.c must compose MoE router policy helpers for loaded-layer decisions"
    fail=1
fi
...
if grep -n '^[[:space:]]*setenv("BN_CPU_LLAMA' test/test_transformer.c >/dev/null 2>&1; then
    echo "Transformer policy tests must use behavior-named CPU reference env vars; legacy LLAMA aliases belong in quant policy alias tests"
    fail=1
fi
...
if awk '/^void bn_transformer_gpu_emit_context_attention_finish/{flag=1} /^void bn_transformer_gpu_emit_context_ssm/{flag=0} flag{print}' \
    src/transformer/gpu_emit.c | grep -n 'lw->attn\.wo\.\(type\|rows\|cols\)' >/dev/null 2>&1; then
    echo "Attention output GPU emission must use resolved projection layout metadata"
    fail=1
fi
...
if grep -n 'int did_gpu_route_topk\|float cpu_weights\[BN_MAX_MOE_K\]\|gpu moe route readback failed' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate host MoE route resolution to GPU fallback"
    fail=1
fi
...
if grep -n 'moe_raw_gate_compare\|moe_raw_up_compare\|gate_raw_compare_flags\|up_raw_compare_flags' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate routed MoE raw projection comparison to GPU fallback"
    fail=1
fi

if grep -n 'moe_mid_compare\|float \*moe_cpu_mid\|float \*moe_gpu_mid' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate routed MoE intermediate comparison to GPU fallback"
    fail=1
fi
...
if grep -n 'shared_cpu_xb\|shared_cpu_out\|shared_gpu_out' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate shared expert CPU residual fallback to GPU fallback"
    fail=1
fi

if grep -n 'moe_cpu_shared_mid\|moe_gpu_shared_mid\|moe_cpu_shared_down\|moe_gpu_shared_down\|moe_cpu_norm\|moe_gpu_norm' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate routed MoE post-layer diagnostics to GPU fallback"
    fail=1
fi

if grep -n 'moe_cpu_x\|moe_gpu_x\|moe_override_x\|bn_transformer_gpu_fallback_moe_output_from_state\|bn_transformer_gpu_fallback_moe_output(' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "GPU orchestration must delegate routed MoE debug state lifecycle to GPU fallback"
    fail=1
fi
...
if grep -n 'bn_transformer_cpu_\(refine_q6_logits_row\|refine_q6_logits_prepared_activation_row\)' include/transformer_cpu_backend_internal.h src/transformer/gpu.c src/transformer/logits.c >/dev/null 2>&1; then
    echo "Transformer logits callers must use K-quant refine CPU helpers, not Q6 helper names"
    fail=1
fi
...
if ! grep -n 'BN_CUDA_ENABLE_ALL_ACTIVE_TWO_KQUANT_ROUTE_BLOCK_PREPARED_INPUT' test/test_gpu_backend.c >/dev/null 2>&1; then
    echo "GPU policy tests must cover canonical all-active-two route block prepared-input env policy"
    fail=1
fi
...
if grep -n 'gpu->matmul_batch(gpu->ctx' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "Transformer prefill execution must use GPU policy helpers for quant matmul-batch backend calls"
    fail=1
fi

if awk '/^int bn_transformer_gpu_prefill_quant_matmul_backend_available/{flag=1} /^int bn_transformer_gpu_prefill_dense_ffn_batch_backend_available/{flag=0} flag{print}' \
    src/transformer/gpu_policy.c | grep -n 'gpu->matmul\|gpu->matmul_batch' >/dev/null 2>&1; then
    echo "Transformer GPU prefill quant matmul policy must use GPU backend helpers"
    fail=1
fi

if awk '/^int bn_transformer_gpu_prefill_dense_ffn_batch_backend_available/{flag=1} /^int bn_transformer_gpu_prefill_qkv_attention_wo_backend_available/{flag=0} flag{print}' \
    src/transformer/gpu_policy.c | grep -n 'gpu->dense_ffn_batch' >/dev/null 2>&1; then
    echo "Transformer GPU prefill dense FFN batch policy must use GPU backend helpers"
    fail=1
fi
...
if grep -n 'bn_transformer_gpu_matvec_exact_kquant_flags' include/transformer_plan_internal.h src/transformer/gpu_policy.c src/transformer/gpu_internal.h src/transformer/gpu_emit.c test/test_transformer.c >/dev/null 2>&1; then
    echo "Transformer GPU reference K-quant matvec flag policy must use reference behavior names"
    fail=1
fi
...
if grep -n 'BN_QUANT_GPU_MATVEC_FLAG_EXACT_KQUANT\|BN_BACKEND_QUANT_GPU_MATVEC_FLAG_EXACT_KQUANT\|BN_GPU_OP_FLAG_MATVEC_EXACT_KQUANT\|bn_quant_format_gpu_matvec_exact_kquant_flag\|bn_backend_quant_gpu_matvec_exact_kquant_flag' include src test/test_transformer.c test/test_gpu_backend.c >/dev/null 2>&1; then
    echo "Reference K-quant matvec flag policy must use reference behavior names"
    fail=1
fi
...
if grep -n 'bn_transformer_gpu_dense_gateup_exact_split_supported' src/transformer/gpu_internal.h src/transformer/gpu_policy.c src/transformer/gpu_emit.c test/test_transformer.c >/dev/null 2>&1; then
    echo "Dense gate/up transformer GPU split policy must use reference-activation behavior names"
    fail=1
fi
...
if awk '/^static int gpu_policy_cuda_moe_routed_ffn_enabled/{flag=1} /^int bn_gpu_policy_moe_resident_routed_ffn_enabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1 ||
   awk '/^int bn_gpu_policy_cpu_logits_enabled/{flag=1} /^int bn_gpu_policy_small_dense_native_quant_cpu_attention_safe_disabled/{flag=0} flag{print}' src/gpu_policy.c | grep -n 'getenv(' >/dev/null 2>&1; then
    echo "src/gpu_policy.c public logits/MoE policy helpers must compose local env policy helpers"
    fail=1
fi
...
for file in \
    ...
    src/transformer/plan.c
do
    if grep -n 'bn_backend_quant_can_gpu_native\|bn_backend_quant_can_gpu_repack\|bn_backend_quant_gpu_float_buffer_type\|bn_backend_quant_dense_float_type\|bn_backend_quant_uses_dense_float\|bn_backend_quant_can_convert_dense_to_float\|bn_backend_quant_convert_dense_to_float' "$file" >/dev/null 2>&1; then
        echo "$file must use quant format dense/GPU type helpers for quant-format policy"
        fail=1
    fi
done

if grep -n 'bn_backend_quant_already_f32\|bn_backend_quant_dense_f32_type\|bn_backend_quant_can_convert_dense_to_f32\|bn_backend_quant_convert_dense_to_f32\|bn_model_quant_is_dense_f32\|bn_model_quant_dense_f32_type\|bn_model_quant_can_convert_dense_to_f32\|bn_model_quant_convert_dense_to_f32' \
...
if grep -n 'debug_compare_q8_activation' src/transformer/gpu_fallback.c >/dev/null 2>&1; then
    echo "Transformer GPU fallback native-quant activation comparison must use behavior names"
    fail=1
fi
...
if grep -n 'BN_BACKEND_QUANT_AUX_CACHE_DEQUANT_\(BF16\|Q8_0\|Q5_0\|Q3K\|Q4K\|Q5K\|Q6K\)_TO' include/backend_quant.h src/gpu_cuda.cu test/test_gpu_backend.c >/dev/null 2>&1; then
    echo "Backend quant aux-cache dequant routes must use behavior names, not raw tensor-format names"
    fail=1
fi
...
if grep -n 'bn_model_config_' src/model.c >/dev/null 2>&1; then
    echo "src/model.c must compose load-time behavior through model-load policy helpers"
    fail=1
fi
...
if grep -n 'c->moe_intermediate_size\|c->n_experts_active\|c->n_experts' src/model_session.c >/dev/null 2>&1; then
    echo "src/model_session.c must use MoE policy helpers for routed session buffer shape"
    fail=1
fi
...
if grep -n 'c->rope_dim_count\|c->rope_text_dims\|c->rope_theta\|m->config\.rope_dim_count\|m->config\.rope_text_dims\|m->config\.rope_theta' src/model_session.c src/transformer.c src/transformer/gpu.c src/transformer/gpu_fallback.c src/transformer/prefill.c src/gpu_wgpu.c src/gpu_metal.m src/gpu_cuda.cu >/dev/null 2>&1; then
...
if grep -n 'arch_per_layer' src/transformer.c src/transformer/cpu.c >/dev/null 2>&1; then
    echo "Transformer execution must use behavior-named per-layer input helpers"
    fail=1
fi
...
if grep -n 'bn_model_config_' src/transformer/plan.c >/dev/null 2>&1; then
    echo "Transformer planning must use model-transformer policy helpers for model-family policy"
    fail=1
fi
...
if grep -n 'c->qk_norm_per_head\|m->config\.qk_norm_per_head' \
    ...
    echo "Transformer prefill/GPU paths must use attention Q/K norm layout policy helpers"
    fail=1
fi

if grep -n 'bn_transformer_attention_uses_per_head_qk_norm(c)\|bn_transformer_attention_uses_per_head_qk_norm(&m->config)\|bn_transformer_attention_qk_stride(' \
    src/transformer/prefill.c \
    src/transformer/gpu_emit.c >/dev/null 2>&1; then
    echo "Transformer prefill/GPU emit must use planned Q/K norm layout"
    fail=1
fi
...
if grep -n 'c->has_shared_expert.*shared_expert_gate\|shared_expert_gate.*c->has_shared_expert' src/transformer/plan.c >/dev/null 2>&1; then
    echo "Transformer MoE planning must compose shared-expert presence through policy helpers"
    fail=1
fi
...
if grep -n 'bn_transformer_gpu_can_use_stacked_alpha_beta' src/transformer/gpu_internal.h src/transformer/gpu_policy.c src/transformer/gpu_emit.c test/test_transformer.c >/dev/null 2>&1; then
    echo "Transformer SSM alpha/beta stack helper must name same-quant-format behavior explicitly"
    fail=1
fi
...
if grep -n '#include "model_arch.h"\|bn_model_arch_' src/transformer/plan.c >/dev/null 2>&1; then
    echo "src/transformer/plan.c must use model-policy helpers instead of reaching into model_arch"
    fail=1
fi
...
if grep -n 'bn_model_config_attention_layer_count\|bn_model_config_uses_hybrid_layer_layout' src/prompt_cache.c >/dev/null 2>&1; then
    echo "src/prompt_cache.c must compose prompt-cache model policy helpers"
    fail=1
fi
...
if grep -n 'c->moe_intermediate_size\|c->n_experts_active\|c->n_experts\|c->moe_norm_topk_prob\|c->moe_expert_weights_scale' src/moe_prefill.c >/dev/null 2>&1; then
    echo "src/moe_prefill.c must use MoE policy helpers for routed expert shape and weighting"
    fail=1
fi
...
if grep -n 'c->moe_intermediate_size\|c->n_experts_active\|c->n_experts\|c->moe_norm_topk_prob\|c->moe_expert_weights_scale' src/transformer/plan.c >/dev/null 2>&1; then
    echo "src/transformer/plan.c must use MoE policy helpers for routed expert shape and weighting"
    fail=1
fi

if grep -n 'c->moe_intermediate_size\|c->n_experts_active\|c->n_experts\|c->moe_norm_topk_prob\|c->moe_expert_weights_scale\|m->config\.n_experts\|m->config\.moe_intermediate_size\|m->config\.moe_norm_topk_prob\|m->config\.moe_expert_weights_scale' src/transformer/prefill.c >/dev/null 2>&1; then
    echo "src/transformer/prefill.c must use MoE policy helpers for routed expert shape and weighting"
    fail=1
fi
...
if awk '/^int bn_moe_can_batch_shared_gateup/{flag=1} /^void bn_moe_quant_matvec/{flag=0} flag{print}' src/moe_cpu_kernels.c | grep -n 'BN_TRANSFORMER_CPU_HAS_\|__AVX\|__ARM_NEON' >/dev/null 2>&1; then
    echo "src/moe_cpu_kernels.c shared gateup capability must come from local MoE CPU ops"
    fail=1
fi
...
if grep -n 'bn_backend_model_qweight_buf' \
    src/transformer/cpu.c >/dev/null 2>&1; then
    echo "Transformer CPU orchestration must compose backend quant resources through CPU policy"
    fail=1
fi

if grep -n 'bn_backend_model_prepared_qweight\|bn_backend_model_qweight_buf' \
    src/transformer/gpu_fallback.c >/dev/null 2>&1; then
    echo "Transformer GPU fallback must compose backend quant resources through CPU policy"
    fail=1
fi
...
if grep -n '#include "backend_model.h"' \
    ...
    src/transformer/logits.c \
    src/transformer/prefill.c >/dev/null 2>&1; then
    echo "Transformer orchestration must depend on composed policies, not backend model APIs"
    fail=1
fi

if grep -n 'BnBackendModel\|bn_model_backend\|bn_transformer_gpu_resolve_\(initial_norm\|next_norm\|dense_ffn_resources\|qkv_resources\|attention_resources\|ssm_resources\|moe_shared_resources\|moe_decode_resources\)' \
    src/transformer/gpu.c >/dev/null 2>&1; then
    echo "Transformer GPU orchestration must consume model-owned composed resources"
    fail=1
fi
...
if grep -n 'bn_backend_model_handle' src/main.c >/dev/null 2>&1; then
    echo "CLI orchestration must compose backend MoE residency through backend-model resources"
    fail=1
fi
...
if grep -n '#include "backend_model.h"' \
    ...
    echo "MoE execution interfaces must keep backend models opaque"
    fail=1
fi
...
if grep -n '#include "backend_model.h"' \
    include/backend_layout.h >/dev/null 2>&1; then
    echo "Backend layout interface must keep backend models opaque"
    fail=1
fi
...
if grep -n 'bn_backend_model_raw_gpu\|bn_backend_model_clear_gpu\|bn_backend_session_set_gpu_graph' \
    ...
    src/backend_model.c \
    src/backend_session.c >/dev/null 2>&1; then
    echo "Backend ownership APIs must not expose raw GPU or graph injection"
    fail=1
fi
